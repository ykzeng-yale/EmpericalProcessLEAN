...
Current frontier: Theorem 5.41 now has a compiled Gaussian-limit law vector
mean source endpoint for the centered-product covariance route
...
It replaces coordinatewise Gaussian-limit law mean-zero by the single vector
mean-zero statement under `Q.map Z`, deriving coordinate means using Gaussian
`MemLp` and `vaart1998_coordinate_mean_zero_of_vector_integral_zero`. Its
immediate predecessor is the score-law vector mean source endpoint
