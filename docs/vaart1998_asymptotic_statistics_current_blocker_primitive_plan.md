...
integrability hypotheses, or explicit nonnegative-envelope hypotheses, unless a
...
probability, but still consumes signed-envelope integrability
`Integrable envelope observationLaw`. The current endpoint derives that field
from integrability of `fun x => |envelope x|`.
