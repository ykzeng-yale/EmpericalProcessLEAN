...
Route from `Live In-Thread Goal Prompt V26` in
...
Jensen.  The next frontier is the remaining Theorem 4.2.6 `|X_n|^p`
consequence, then Theorem 4.2.7 if the power-convexity route is not immediate.
...
Next proof packet: finish the remaining Theorem 4.2.6 `|X_n|^p` consequence
for `p ≥ 1` by locating or adding a reusable convexity wrapper for
`x ↦ |x|^p`/`x ↦ ‖x‖^p`, then feed it into the compiled generic convex-image
submartingale theorem.  If that power-convexity route is not immediate, move to
Theorem 4.2.7 increasing-convex transforms.
