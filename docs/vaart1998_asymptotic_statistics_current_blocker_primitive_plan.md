...
The newest observation-estimating-map layer removes the sample-indexed
...
estimating-map `ContDiffOn` fields, or sample-indexed Taylor fderiv fields,
...
source or a remaining score/observation covariance identification source; the
sample-indexed estimating-map plumbing is solved and should not be replayed.
Reuse local `StatInference` and mathlib APIs before adding primitives.
...
This predecessor still asks directly for sample-indexed `estimatingMap`,
