...
## Live In-Thread Goal Prompt V32
...
Theorems 4.2.4-4.2.10 as closed dependencies.  Treat Theorem 4.2.11's
mathlib L1/eLpNorm-bounded convergence wrappers as compiled support, while the
exact Durrett positive-part hypothesis bridge remains the active target.
Theorem 4.1.16 remains deferred unless a future targeted kernel search finds a
...
integrability of the limit, and martingale specializations.

Next theorem-sized packet: close the exact Durrett positive-part hypothesis
bridge for Theorem 4.2.11.  Prove a source-facing lemma that converts a
submartingale bound on the positive parts, together with the submartingale
expectation lower bound from time zero, into the mathlib hypothesis
`∀ n, eLpNorm (X n) 1 μ ≤ R`.  Then add a final wrapper whose assumption
matches `sup_n E X_n^+ < ∞` as closely as the local integral API allows.
Search only the relevant positive/negative part and L1 APIs before editing:
`integral_eq_norm_posPart_sub`, `memLp_one_iff_integrable`,
`eLpNorm_one_eq_lintegral_enorm`, `Submartingale.ae_le_condExp`, and
monotonicity of expectations for submartingales.  Do not revisit solved Chapter
...
The next likely packet should close the positive-part-to-L1 bridge for the
exact Durrett Theorem 4.2.11 hypothesis, reusing the compiled eLpNorm
convergence wrappers in `StatInference/ProbabilityTheory/Martingale.lean`.
...
Use `Live In-Thread Goal Prompt V32` at the top of this file.  Historical route
