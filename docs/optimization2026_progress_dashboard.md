...
Next theorem-sized packet: discharge the sampled stochastic model fields above
the Bochner transport and smooth L2-noise wrappers.  Prove raw `ψ_x`
inequalities from `mirrorProximalGradientModel`, growth from
...
strong-convexity, Lipschitz, and bounded-gradient estimates from (12.2).  Then
instantiate the compiled smooth L2-noise and non-smooth integral-component
final rate wrappers.
...
component-hcore, finite-support, Bochner-integral, and smooth L2/Hölder noise
packets reuse local Gronwall/geometric weight infrastructure and leave only the
concrete sampled probability/model estimates as the active formal blocker.
