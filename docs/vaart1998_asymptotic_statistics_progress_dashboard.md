...
Latest verified packet: fixed true-parameter source endpoint for the
...
It replaces coordinate derivative-transform integrability by vector-valued
integrability and derives each matrix-entry field with `Integrable.eval`.
...
Current next blocker: continue from the fixed-`theta0` Theorem 5.41 endpoint
toward a concrete model-specialized instantiation, or package only a still-live
source field that the current endpoint explicitly exposes.  Do not replay the
centered derivative-coordinate residual algebra,
