...
| Chapter 4 convex/strongly-convex reductions | local-layer | `StatInference/Optimization/Reductions.lean` | Chewi Lemma 4.2's regularization spine now compiles. `quadraticRegularizedAround` and `regularizedGradient` model `f_delta = f + delta / 2 * ‖· - x0‖^2`; `quadraticRegularizedAround_firstOrderStrongConvexOn` shifts first-order lower-model parameter `alpha` to `alpha + delta`, and `quadraticRegularizedAround_smoothWithGradientOn` shifts smoothness `beta` to `beta + delta`. Source-shaped value and complexity hooks compile: `le_quadraticRegularizedAround`, `quadraticRegularizedAround_near_min_le_base_add_penalty`, `quadraticRegularizedAround_near_min_gap_le_eps`, `regularization_penalty_le_of_norm_le`, `regularized_minimizer_dist_le_of_base_min`, and `regularized_smoothness_le_two_beta`. Search-first result: no local regularization layer existed; reused `FirstOrderStrongConvexOn`, `SmoothWithGradientOn`, `norm_add_sq_real`, `real_inner_smul_left`, `sq_le_sq₀`, and `mul_le_mul_of_nonneg_left`. Next target is a source-shaped Lemma 4.2 complexity wrapper and then Theorem 4.5. |
...
Latest proof target: Chapter 4 Lemma 4.2 reductions and Theorem 4.5.  The new
...
regularized-minimizer distance estimate, and the `beta + delta <= 2 beta`
smoothness arithmetic hook.  Do not redo the Theorem 3.4/Chapter 4 lower-bound
foundation work; the next aggressive step is to package Lemma 4.2's condition
number/complexity arithmetic around these hooks, then connect the compiled
Theorem 4.4 lower-bound lane to Theorem 4.5.
...
Next high-value tasks: finish the source-shaped Lemma 4.2 arithmetic wrapper,
then use it to derive the strongly-convex lower-bound Theorem 4.5 from the
compiled Theorem 4.4 lane.  Reuse `Reductions.lean` for regularization,
`LowerBounds.lean` for gradient-span lower bounds, and search mathlib/local
arithmetic APIs before adding condition-number primitives.  Keep any exercise
work opportunistic and centralized in `StatInference/Optimization/Exercises.lean`;
the main theorem lane remains the priority.
