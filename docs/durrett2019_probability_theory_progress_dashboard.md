...
Route from `Live In-Thread Goal Prompt V119` in
...
The next target is coefficient extraction and division algebra: pull out
`(p - 1)⁻¹`, convert the kernel to the exact Hölder integrand, and feed the
compiled `lintegral_rpow` hypothesis.
...
threshold bound now compile as well.  The weighted/Fubini identification now
also compiles through withDensity and base-measure forms.  Aim at one forward
textbook packet: coefficient extraction, exact Hölder-kernel conversion, and
division algebra for the actual positive-part p-th-power estimate.
