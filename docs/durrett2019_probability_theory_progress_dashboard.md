...
Route from `Live In-Thread Goal Prompt V157` in
...
bounded-variance Exercise 4.4.11 corollary route.  The next target is the
reciprocal deterministic-transform side-condition packaging: direct
`H n = (b n)⁻¹`, positivity/nonzero, boundedness, predictability, and transform
`MemLp` bridges for a positive monotone normalizer.
The V143-V157 packets added `durrett2019_exercise_4_4_6_varianceClock_succ`,
...
Next proof packet: continue Exercise 4.4.11 from the compiled zero-initial
normalized-process and bounded-variance corollary route.  Exercise 4.4.10 now
supplies the almost-sure martingale convergence handoff, and Exercise 4.4.11
now feeds scaled-transform almost-sure convergence through Kronecker pathwise
all the way to the `X_n / b_n` display.  Aim at one forward textbook packet:
package the reciprocal deterministic-transform side conditions for a positive
monotone normalizer, especially direct `H n = (b n)⁻¹`, positivity/nonzero,
boundedness, predictability, and transform `MemLp` bridges.  If that is not
immediate, move to a nearby Chapter 4.4 theorem-sized packet.  Do not re-prove
the compiled Chapter 4.3.5/4.3.8 assembly, Theorem 4.4.1 optional stopping,
Exercises 4.4.5-4.4.10, the Exercise 4.4.11 transform convergence bridge, or
the deterministic Abel/Toeplitz/Kronecker/normalized-increment/zero-initial/
bounded-variance route.
