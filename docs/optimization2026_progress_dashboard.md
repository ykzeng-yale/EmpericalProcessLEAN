...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after the 2026-05-05 Chapter 6 PSD packet:
  ...
  `chewi614_average_gap_bound`.  The active target is now a theorem-sized
  Chapter 6 packet: source-complete Theorem 6.14 by adding the valid
  Lipschitz-to-subgradient-norm bridge and displayed `h = R / sqrt N`
  corollary, then continue directly to a source-shaped Theorem 6.16 functional
  constraints interface and recurrence package.
...
| Chapter 6 nonsmooth convex optimization | local-layer | `StatInference/Optimization/ProjectedSubgradient.lean` | The first Chapter 6 packet now compiles. It defines `IsSubgradientAt`, `ProjectionCharacterizationOn`, `ProjectionOracleOn`, `normalizedSubgradient`, `projectedSubgradientStep`, `IsProjectedSubgradientTrajectory`, and `iterateAverage`; proves `IsSubgradientAt.gap_le_inner`, `ProjectionCharacterizationOn.dist_to_set_le`, `ProjectionCharacterizationOn.fixed`, `ProjectionCharacterizationOn.nonexpansive`, `ProjectionCharacterizationOn.toProjectionOracleOn`, `normalizedSubgradient_norm`, Jensen wrappers for the averaged iterate, the PSD squared-distance recurrence `projectedSubgradient_sqdist_recurrence`, the finite telescope `projectedSubgradient_gap_average_bound_of_recurrence`, and Theorem 6.14 supplied-interface wrapper `chewi614_average_gap_bound`. Search-first result: reused mathlib `ConvexOn.map_sum_le`, local `sum_range_sub_succ`, `norm_add_sq_real`/`norm_sub_sq_real`, `real_inner_le_norm`, and scalar `field_simp`/`nlinarith`; the route records mathlib projection-minimal and Lipschitz APIs for the next exact-source bridge. Current aggressive packet: prove the correct interior/ray-form Lipschitz-to-subgradient-norm bridge, source-package Theorem 6.14 under Chewi's Lipschitz assumption where valid, add the displayed `h = R / sqrt N` rate, and then start Theorem 6.16 functional constraints without waiting for report packaging. |
...
Latest proof target: Chapter 6 nonsmooth convex optimization.  The next manual
goal packet should source-complete Theorem 6.14 and immediately begin Theorem
6.16, not loop on tiny wrappers.  First, search mathlib/local APIs and prove a
correct interior/ray-form bridge from `LipschitzOnWith` to `‖p_n‖ <= L` for
the selected subgradients.  Then package the existing PSD average-gap theorem
under this bridge and add the source displayed step-size corollary
`h = R / sqrt N`.  If that closes cleanly, define the functional-constraint
PSD interface and recurrence for Theorem 6.16 in the same batch.
