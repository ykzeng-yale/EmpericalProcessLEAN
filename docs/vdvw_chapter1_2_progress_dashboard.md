...
The canonical Rademacher-product specializations of that fixed-sample tail are
now compiled as
`vdVWTheorem243_rademacherFiniteCenterHoeffding_failure_real_le_vdVWRademacher`,
`vdVWTheorem243_rademacherFiniteCenterHoeffding_failure_le_ofReal_tail_vdVWRademacher`,
`vdVWTheorem243_rademacherFiniteCenterHoeffding_failure_real_le_neg_vdVWRademacher`,
and
`vdVWTheorem243_rademacherFiniteCenterHoeffding_failure_le_ofReal_tail_neg_vdVWRademacher`.
The sign-only and product-pair failure-tail consumers use these wrappers
directly, so the dashboard source API no longer requires each consumer to
repeat the canonical Rademacher independence and sub-Gaussian setup.
...
The inverse-square source route now feeds that consumer through
`vdVWTheorem243_selectedFailureTail_tendsto_zero_of_invSq_lintegral`,
`VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_productPairChebyshev_countable_finiteCenter_failure_tails_halfScale_of_invSq_lintegral`, and
...
This replaces the raw displayed failure-tail convergence input by explicit
inverse-square selected-cardinality lintegral convergence.
The deterministic selected-cardinality lower-growth bridge is now compiled as
...
so the inverse-square selected-cardinality lintegral can be discharged from an
eventual selected-cardinality lower bound tending to infinity.
The lower-growth route now has probability-facing forms:
...
`VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_productPairChebyshev_countable_finiteCenter_failure_tails_halfScale_of_selected_truncated_quarterRadius_firstLevel_of_eventually_ae_cardinality_ge`, and
...
This admits product-pair a.e. or ordinary single-sample a.e. selected-cover
cardinality growth inputs.
Those inputs now also package directly to the untruncated centered conclusion
via
...
The single-sample inverse-square lintegral bridge is now compiled via
...
2026-05-11 follow-up: the Theorem 2.4.3/Lemma 2.4.5 final-output shape is now
named by `VdVWTheorem243TextbookAlignedConclusion`.  It bundles the exact
outputs currently available from the theorem-facing routes: Definition 2.3.3
`P`-measurability, finite outer envelope expectation, outer-probability and
outer-a.s. GC endpoints, the local book-style `P`-GC predicate, in-mean
centered-supremum convergence, and the Lemma 2.4.5 a.s. centered-supremum
endpoint.  Constructors now feed this conclusion from the full-subgraph
countable route, original full-subgraph countable route, fixed-degree original
full-subgraph VC route, and finite-class SLLN route.  The exact textbook
statement remains blocked on the arbitrary `P`-measurable/asymptotic-measurable
class layer and the book random entropy-to-selected finite-net/full-subgraph
bridge.
The same conclusion bundle now has the generic centered-convergence constructor
...
so future entropy or structural-cardinality source proofs can target the
single centered untruncated outer-probability endpoint and obtain the full
Theorem 2.4.3/Lemma 2.4.5 output package.
The proof-carrying full-subgraph side-condition record now feeds that same
bundle through `VdVWTheorem243TextbookAlignedConclusion.of_fullSubgraphSideConditions`.
The selected entropy-to-finite-net-mean primitive also now feeds the bundle
directly through
`VdVWTheorem243TextbookAlignedConclusion.of_selectedEntropyFiniteNetMeanPrimitive`.
The fixed-fiber pair-difference Chebyshev route now has the canonical
first-level selected-cover source wrapper
...
so event measurability and selected-cover construction are no longer exposed
in that branch.
The canonical Rademacher `Phi(x)=x` route now has the matching first-level
selected-cover fixed-`M` source wrapper
...
so its branch exposes only the canonical finite-center maximal event and
canonical `Phi(x)=x` comparison.
...
CURRENT    The active /goal frontier is no longer the reverse/cofiltration
           ...
           The exact-output proposition is now
           `VdVWTheorem243TextbookAlignedConclusion`; future source bridges
           should target constructors into that proposition instead of
           expanding seven-part conjunctions.  The generic constructor from
           centered untruncated convergence is
           `VdVWTheorem243TextbookAlignedConclusion.of_centered_untruncated_convergesInOuterProbabilityConst_zero_countable_integrable`.
           The compact side-condition record constructor is
           `VdVWTheorem243TextbookAlignedConclusion.of_fullSubgraphSideConditions`.
           The direct selected-entropy primitive constructor is
           `VdVWTheorem243TextbookAlignedConclusion.of_selectedEntropyFiniteNetMeanPrimitive`.
           The canonical fixed-fiber pair-difference selected-cover source
           wrapper is
           `VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_pairDifferenceChebyshev_signSwap_ae_finiteCenter_halfScale_of_selected_truncated_quarterRadius_firstLevel`.
           The canonical Rademacher `Phi(x)=x` first-level selected-cover
           source wrapper is
           `VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_canonicalRademacher_countable_hphi_id_firstLevel`.
