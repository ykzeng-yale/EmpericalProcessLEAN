...
| `tendstoInDistribution_of_vdVWConvergesInOuterProbability`, `vdVW1102_ii_tendstoInDistribution_of_vdVWConvergesInOuterProbability`, `vdVW1102_ii_to_signedBoundedContinuousArbitraryMap_aemeasurable` | measurable common-domain version of Lemma 1.10.2(ii): outer-probability convergence implies convergence in distribution, plus the proof-carrying signed bounded-continuous arbitrary-map consequence |
| `VdVWWeakConvergenceProbabilityMeasures`, `vdVWWeakConvergenceProbabilityMeasures_iff_forall_integral_tendsto`, `vdVWWeakConvergenceProbabilityMeasures_iff_forall_bounded_lipschitz_integral_tendsto`, `vdVW1122_weakConvergenceProbabilityMeasures_of_forall_bounded_lipschitz_integral_tendsto`, `vdVW1122_forall_bounded_lipschitz_integral_tendsto_of_weakConvergenceProbabilityMeasures`, `vdVWWeakConvergenceProbabilityMeasures_iff_levyProkhorov_tendsto`, `vdVWWeakConvergenceProbabilityMeasures_iff_levyProkhorovDist_tendsto_zero`, `vdVW1124_weakConvergenceProbabilityMeasures_iff_levyProkhorovDist_tendsto_zero` | measure-level VdV&W weak-convergence wrapper and bounded-continuous, bounded-Lipschitz, and Levy-Prokhorov characterizations, proved by pinned mathlib |
...
| `VdVWWeakConvergenceProbabilityMeasures.map_continuous`, `vdVWTendstoInDistribution_continuous_comp`, `vdVW1111_tendstoInDistribution_continuous_comp`, `vdVW1111_signedBoundedContinuousArbitraryMap_comp_continuous`, `vdVW1111_signedBoundedContinuousVaryingDomains_comp_continuous` | mathlib-backed and local signed-outer continuous mapping wrappers for probability laws, convergence in distribution, arbitrary-map signed bounded-continuous weak convergence, and varying-domain endpoints |
...
| 1.9-1.12 | convergence in outer probability, extended continuous mapping, uniform integrability, bounded Lipschitz metric | common-domain outer-probability/TendstoInMeasure bridges, VdV&W-numbered Lemma 1.10.2(ii) common-domain distribution and signed bounded-continuous consequences, fixed continuous-map Theorem 1.11.1 wrappers for measurable, signed arbitrary-map, and varying-domain forms, bounded-Lipschitz weak-convergence criterion, and Levy-Prokhorov distance wrappers are formalized; full arbitrary-map/asymptotic-measurability, varying-map continuous mapping, uniform integrability, and broader nonmeasurable-cover variants remain pending |
