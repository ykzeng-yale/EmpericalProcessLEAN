...
`alpha = 0` branches, with interval-integrability assumptions exposed.  It
...
`PLGradientFlowLyapunovContinuousDataRouteToQGOn` hypothesis by proving or
cleanly supplying the gradient-flow convergence-to-minimizer input,
trajectory/gap continuity, positive-gap side condition, and nonzero
displacement on positive times; the derivative-to-antitone, PL scalar sign,
objective-gap derivative, nonzero norm-derivative, and Lyapunov-continuity
calculations are now compiled.
