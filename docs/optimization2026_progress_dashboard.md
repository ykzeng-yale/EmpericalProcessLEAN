...
- Current manual frontier after the Chapter 12 finite sampled rate packet,
  smooth integral-L2 sampled-model endpoint packet, and smooth
  Bochner-unbiased growth/star-upper packet for Chewi Theorem 12.1 SMPGD:
...
sampled, smooth integral-L2 sampled-model, and smooth Bochner-unbiased rate
packets: treat Chapters 3-8, Chapter 9/10
...
integral-L2 sampled-model final rate and the smooth Bochner-unbiased
growth/star-upper wrapper are instantiated; next prove variance domination
from (12.1), any exact source conditional-expectation/process packaging that
is still needed, and the non-smooth upgrade from pointwise bounded sampled
gradients to the source L2 assumption (12.2), then instantiate the non-smooth
integral-component final rate wrapper or a dedicated non-smooth L2 wrapper.
...
Gronwall/geometric weight infrastructure and leave only the remaining exact
sampled probability estimates as the active formal blocker.
