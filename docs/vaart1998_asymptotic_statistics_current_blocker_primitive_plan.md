...
1. Move one layer closer to a model-facing finite-dimensional statement:
   continue from the vector score-transform `L²` fixed-`theta0` endpoint toward a concrete
...
score-transform `L²` source endpoint for Vaart Theorem 5.41.
...
vector score-transform `L²` endpoint to a concrete
