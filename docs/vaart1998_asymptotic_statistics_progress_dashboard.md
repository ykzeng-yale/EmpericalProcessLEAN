...
Move next to the derivative-a.e. wrapper and remove its `hScaledEstimator`
hypothesis by calling the absorbing derivative-tendsto endpoint, then continue
upward through raw-score-CLT, raw-root, and estimator-substitution wrappers
only while each packet remains theorem-sized.  Otherwise prove the next
