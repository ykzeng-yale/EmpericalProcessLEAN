...
Route from `Live In-Thread Goal Prompt V50` in
...
measurable sets and feed the RN/ratio/top-set source endpoint.  The next target
is an actual source-production obligation: prove those generator-level
bounded-convergence identities for `Y` and `Z`, or prove `X = Y/Z` /
`{X = infinity}` singular separation.
...
to RN-derivative bridge and generator-extension bridge.  Aim at one actual
source-production obligation: prove the generator-level bounded-convergence
integral identities for `Y` and `Z`, or prove `X = Y/Z` / `{X = infinity}`
singular separation.  Search bounded convergence, lintegral/real-integral
convergence for bounded nonnegative functions, filtration restriction
identities, a.e. transfer, ENNReal limsup/top, and RN-derivative APIs.  Do not
