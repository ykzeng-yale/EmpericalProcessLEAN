...
Route from `Live In-Thread Goal Prompt V109` in
...
to move forward from Theorem 4.3.8: first try Lemma 4.3.9's
branching-process martingale if it is source-shaped, otherwise use mathlib's
Doob maximal inequality support for Section 4.4.
...
the closed no-top/no-ambient-dichotomy `tprod` textbook branch wrapper.  Aim at
one forward textbook packet: first test Lemma 4.3.9's branching-process
martingale formulation, and if that needs too much new infrastructure, switch
to Section 4.4 Doob maximal/Lp inequalities through mathlib
`MeasureTheory.maximal_ineq` and optional-stopping APIs.
