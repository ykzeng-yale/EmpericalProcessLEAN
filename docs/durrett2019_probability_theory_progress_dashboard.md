...
Route from `Live In-Thread Goal Prompt V80` in
...
and lower-bound-only overlap Cauchy handoff also now compile.  The next target
is the tail-overlap lower bound from finite Hellinger products/product tails,
the HasProd/prefix instantiation of the product-tail convergence hypotheses,
