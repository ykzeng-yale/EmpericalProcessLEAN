...
## Live In-Thread Goal Prompt V597
...
joint infinite-product, canonical product-coordinate, one-based, and pairwise
identically-distributed source shapes.  V507 adds the matching Theorem 2.1.16
...
and pairwise source shapes.  V509 adds the matching Theorem 2.1.16
supplied-density source wrappers: once a separate calculation identifies
...
empirical-function and range-sum display forms.  V513 adds the matching
...
shifted-joint-law, identical-distribution, pairwise-identically-distributed,
and canonical iid source shapes, in raw shifted empirical-function and
range-sum display forms.  V515 adds the matching arbitrary-tolerance
one-based global middle-partition-with-tails squeeze wrappers for the same
source shapes, in raw shifted empirical-function and range-sum display forms.
V516 adds the matching one-based route-named `middlePartitionWithTails`
...
expectation.
V523 adds law-side Chapter 2.1.13 zero-factor corollaries: ordinary Bochner,
...
V525 adds source-side Chapter 2.1.13 finite-product integrability wrappers:
...
expectation-exists wrappers: finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` `HasLaw` products now package source-space integrability of
the composed product together with the vanishing expectation conclusion when
one law-side factor integral is zero.
...
expectation-exists wrappers: for an independent measurable source family
...
V536 adds source-side iid/common-transform composed Chapter 2.1.13
zero-factor expectation-exists wrappers: under `IdentDistrib`, one integrable
zero-mean transformed base marginal `f(X_0)` gives product integrability and
zero expectation for finite/range/Ico and one-based products of `f(X_i)` that
contain a factor, without requiring `HasLaw` hypotheses.
V537 adds source-side iid/indexed-transform composed Chapter 2.1.13
expectation-exists-and-value wrappers: under `IdentDistrib`, index-dependent
measurable transforms `f_i(X_i)` have product integrability and
product-of-base-expectations formulas over `f_i(X_0)` for finite/range/Ico and
one-based products, without requiring `HasLaw` hypotheses.
V538 adds source-side iid/indexed-transform composed Chapter 2.1.13
zero-factor expectation-exists wrappers: under `IdentDistrib`,
index-dependent measurable transforms `f_i(X_i)` have product integrability
and zero product expectation whenever one selected base transformed factor
`f_i(X_0)` has expectation zero, for finite/range/Ico and one-based products,
without requiring `HasLaw` hypotheses.
V539 adds law-side iid/indexed-transform Chapter 2.1.13
expectation-exists-and-value wrappers: under common-law `HasLaw` hypotheses,
...
`ENNReal.ofReal` wrappers: under common-law `HasLaw` hypotheses,
index-dependent nonnegative measurable real transforms `f_i(X_i)` have
source-space `lintegral` formulas as products of law-side `lintegral`s over
`∫⁻ ENNReal.ofReal (f_i x) dμ`, for finite/range/Ico and one-based products.
V543 adds law-side iid/indexed-transform Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` zero-factor wrappers: under common-law `HasLaw` hypotheses,
the same index-dependent nonnegative measurable real transforms have
...
V545 adds source-side iid/indexed-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` product formulas: under `IdentDistrib`, index-dependent measurable
transforms `f_i(X_i) : ℝ≥0∞` factor as products of base-coordinate
`lintegral`s `∫⁻ f_i(X_0) dP`, for finite/range/Ico and one-based products,
without requiring `HasLaw` or ordinary integrability hypotheses.
V546 adds source-side iid/indexed-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` zero-factor formulas: under `IdentDistrib`, the same
index-dependent measurable transforms have source-space product `lintegral`
zero whenever one selected base-coordinate factor `lintegral`
`∫⁻ f_i(X_0) dP` is zero, for finite/range/Ico and one-based products,
without requiring `HasLaw` or ordinary integrability hypotheses.
V547 adds source-side iid/indexed-transform Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` product formulas: under `IdentDistrib`, index-dependent
nonnegative measurable real transforms `f_i(X_i)` have source-space
`lintegral` formulas as products of base-coordinate `lintegral`s over
`∫⁻ ENNReal.ofReal (f_i(X_0)) dP`, for finite/range/Ico and one-based
products, without requiring `HasLaw` or ordinary integrability hypotheses.
V548 adds source-side iid/indexed-transform Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` zero-factor formulas: under `IdentDistrib`, the same
index-dependent nonnegative measurable real transforms have source-space
product `lintegral` zero whenever one selected base-coordinate
`ENNReal.ofReal` factor `lintegral` is zero, for finite/range/Ico and
one-based products, without requiring `HasLaw` or ordinary integrability
hypotheses.
V549 adds source-side composed-function Chapter 2.1.13 nonnegative `ℝ≥0∞`
product formulas: for an independent measurable source family `X_i` and
measurable transforms `f_i`, the source-space product `lintegral` of
`f_i(X_i)` factors as the product of composed `lintegral`s for finite/range/
Ico and one-based products, without requiring `HasLaw` or ordinary
integrability hypotheses.
V550 adds source-side composed-function Chapter 2.1.13 nonnegative `ℝ≥0∞`
zero-factor formulas: for an independent measurable source family `X_i` and
measurable transforms `f_i`, the source-space product `lintegral` of
`f_i(X_i)` is zero whenever one composed factor `lintegral` is zero, for
...
`ENNReal.ofReal` product formulas: for an independent measurable source
family `X_i` and nonnegative measurable real transforms `f_i`, the
source-space product `lintegral` of `ENNReal.ofReal (∏ f_i(X_i))` factors as
the product of composed `ENNReal.ofReal` `lintegral`s for finite/range/Ico
and one-based products, without requiring `HasLaw` or ordinary integrability
hypotheses.
V552 adds source-side composed-function Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` zero-factor formulas: for an independent measurable source
family `X_i` and nonnegative measurable real transforms `f_i`, the
source-space product `lintegral` of `ENNReal.ofReal (∏ f_i(X_i))` is zero
...
V554 adds source-side composed-function Chapter 2.1.13 absolute-value/norm
zero-factor formulas: for an independent measurable source family `X_i` and
measurable transforms `f_i : S_i -> 𝕜`, the source-space `lintegral` of
`‖∏ f_i(X_i)‖` is zero whenever one composed norm factor `lintegral` is zero,
for finite/range/Ico and one-based products, without requiring `HasLaw` or
ordinary integrability assumptions.
V555 adds source-side iid/indexed-transform Chapter 2.1.13 absolute-value/norm
product formulas: under `IdentDistrib`, index-dependent measurable transforms
`f_i(X_i)` have source-space `lintegral` formulas for `‖∏ f_i(X_i)‖` as
...
V557 adds source-side iid/common-transform Chapter 2.1.13 absolute-value/norm
power formulas: under `IdentDistrib`, one common measurable transform
`f : S -> 𝕜` has source-space `lintegral` formulas for `‖∏ f(X_i)‖` as
powers of the base-coordinate norm `lintegral` over `X_0`, for finite/range/Ico
and one-based products, without requiring `HasLaw` or ordinary integrability
assumptions.
V558 adds source-side iid/common-transform Chapter 2.1.13 absolute-value/norm
zero-factor formulas: under `IdentDistrib`, one common measurable transform
`f : S -> 𝕜` has source-space `lintegral` zero for `‖∏ f(X_i)‖` whenever the
...
one-based products, without requiring `HasLaw` or ordinary integrability
assumptions.
V560 adds source-side iid/common-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` zero-factor formulas: under `IdentDistrib`, one common measurable
transform `f : S -> ℝ≥0∞` has source-space product `lintegral` zero whenever
the base-coordinate `lintegral` over `X_0` vanishes, for nonempty finite,
positive range/Ico, and one-based products, without requiring `HasLaw` or
ordinary integrability assumptions.
V561 adds source-side iid/common-transform Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` power formulas: under `IdentDistrib`, one common nonnegative
measurable real transform `f : S -> ℝ` has source-space `lintegral` formulas
...
products, without requiring `HasLaw` or ordinary integrability assumptions.
V563 adds law-side iid/common-law Chapter 2.1.13 nonnegative `ℝ≥0∞`
zero-factor formulas: under common `HasLaw` hypotheses, one common measurable
transform `f : S -> ℝ≥0∞` has source-space product `lintegral` zero whenever
...
one-based products.
V565 adds law-side Chapter 2.1.13 absolute-value/norm `lintegral`
factorization and iid/common-law power formulas: under `HasLaw` hypotheses,
...
product formulas: after measurable transforms `f_i`, the source-space
...
V570 adds Chapter 2.1.13 partial-sum-difference zero-mean and mixed-term-zero
wrappers under `IdentDistrib`: one integrable zero-mean base marginal now
supplies the future interval mean-zero hypotheses for zero-based and
one-based partial-sum differences, including the early-block statistic and
Kolmogorov-maximal indicator mixed-term surfaces.
V571 adds the matching common-law `HasLaw` Chapter 2.1.13 bridge: a law-side
integrable zero mean now supplies zero-based and one-based future
partial-sum-difference zero-mean hypotheses, early-block statistic mixed-term
zero wrappers, and Kolmogorov-maximal indicator mixed-term zero wrappers.
...
V576 adds the matching full infinite-product, shifted infinite-product
joint-law, and canonical iid product-coordinate Theorem 2.1.10 wrappers for
the first-variable/tail-product independence surface.
V577 adds the matching full infinite-product, shifted infinite-product
joint-law, and canonical iid product-coordinate Chapter 2.1.13
expectation-factorization and zero-mean wrappers for the first-variable/
tail-product surface.
V578 adds full infinite-product and shifted infinite-product joint-law
Chapter 2.1.13 law-side range/Ico product-expectation factorization and
zero-factor wrappers, in zero-based and one-based notation.
V579 adds the matching canonical iid product-coordinate Chapter 2.1.13
law-side range/Ico product-expectation factorization and zero-factor wrappers,
in zero-based and one-based notation.
V580 adds full infinite-product and shifted infinite-product joint-law
Chapter 2.1.13 law-side range/Ico product integrability plus expectation-value
and zero-factor wrappers, in zero-based and one-based notation.
V581 adds the matching canonical iid product-coordinate Chapter 2.1.13
law-side range/Ico product integrability plus expectation-value and
zero-factor wrappers, in zero-based and one-based notation.
...
V584 adds the matching canonical iid shifted-coordinate Chapter 2.1.13
law-side finite-subfamily and literal one-based `Icc` product integrability
plus expectation-value and zero-factor wrappers.
...
coordinate forms.
V590 adds the matching canonical iid common-law Chapter 2.1.13 value-only
ordinary zero-product displays over nonempty finite subfamilies, positive
range/Ico intervals, literal one-based `Icc`, and shifted one-based
coordinate forms.
V591 adds the matching source-side iid common-law Chapter 2.1.13 value-only
ordinary zero-product displays over nonempty finite subfamilies, positive
range/Ico intervals, literal one-based `Icc`, and shifted one-based
coordinate forms.
V592 adds the matching joint infinite-product common-law Chapter 2.1.13
value-only ordinary zero-product handoffs over nonempty finite subfamilies,
...
V593 adds the matching source-side iid common-law Chapter 2.1.13
...
positive range/Ico intervals, literal one-based `Icc`, full-law shifted
one-based coordinate forms, and shifted-joint-law one-based coordinate forms.
V595 adds the matching joint infinite-product common-law Chapter 2.1.13
...
V597 adds the matching joint infinite-product common-law Chapter 2.1.13
value-only power handoffs over finite subfamilies, range/Ico intervals,
literal one-based `Icc`, full-law shifted one-based coordinate forms, and
shifted-joint-law one-based coordinate forms.
...
wrappers unless an exact source-shape consumer is missing.  After V515, do
not rebuild the arbitrary-tolerance one-based global
...
exact later consumer needs a different source shape.
After V523, do not rebuild the law-side Theorem 2.1.13 zero-factor
...
source shape.
After V527, do not rebuild the source-side Theorem 2.1.13 paired
expectation-exists-and-value wrappers for finite/range/Ico or one-based
range/Ico/`Icc` products, including iid `IdentDistrib` power-value variants,
unless an exact later consumer needs a different source shape.
After V528, do not rebuild the law-side Theorem 2.1.13 paired
expectation-exists-and-value wrappers for finite/range/Ico or one-based
...
products unless an exact later consumer needs a different source shape.
After V530, do not rebuild the law-side Theorem 2.1.13 zero-factor
expectation-exists wrappers for finite/range/Ico or one-based range/Ico/`Icc`
products unless an exact later consumer needs a different source shape.
After V531, do not rebuild the source-side iid Theorem 2.1.13 zero-factor
...
After V536, do not rebuild the source-side iid/common-transform composed
Theorem 2.1.13 zero-factor expectation-exists wrappers for finite/range/Ico
or one-based range/Ico/`Icc` products unless an exact later consumer needs a
different source shape.
After V537, do not rebuild the source-side iid/indexed-transform composed
Theorem 2.1.13 expectation-exists-and-value wrappers for finite/range/Ico or
one-based range/Ico/`Icc` products unless an exact later consumer needs a
different source shape.
After V538, do not rebuild the source-side iid/indexed-transform composed
Theorem 2.1.13 zero-factor expectation-exists wrappers for finite/range/Ico
or one-based range/Ico/`Icc` products unless an exact later consumer needs a
different source shape.
...
range/Ico/`Icc` products unless an exact later consumer needs a different
source shape.
After V542, do not rebuild the law-side iid/indexed-transform Theorem 2.1.13
real nonnegative `ENNReal.ofReal` product formulas for finite/range/Ico or
one-based range/Ico/`Icc` products unless an exact later consumer needs a
different source shape.
After V543, do not rebuild the law-side iid/indexed-transform Theorem 2.1.13
...
range/Ico/`Icc` products unless an exact later consumer needs a different
source shape.
After V545, do not rebuild the source-side iid/indexed-transform Theorem
2.1.13 nonnegative `ℝ≥0∞` product formulas for finite/range/Ico or one-based
range/Ico/`Icc` products unless an exact later consumer needs a different
source shape.
After V546, do not rebuild the source-side iid/indexed-transform Theorem
2.1.13 nonnegative `ℝ≥0∞` zero-factor formulas for finite/range/Ico or
one-based range/Ico/`Icc` products unless an exact later consumer needs a
different source shape.
After V547, do not rebuild the source-side iid/indexed-transform Theorem
2.1.13 real nonnegative `ENNReal.ofReal` product formulas for finite/range/
Ico or one-based range/Ico/`Icc` products unless an exact later consumer
needs a different source shape.
After V548, do not rebuild the source-side iid/indexed-transform Theorem
2.1.13 real nonnegative `ENNReal.ofReal` zero-factor formulas for finite/
range/Ico or one-based range/Ico/`Icc` products unless an exact later
consumer needs a different source shape.
After V549, do not rebuild the source-side composed-function Theorem 2.1.13
nonnegative `ℝ≥0∞` product formulas for finite/range/Ico or one-based
range/Ico/`Icc` composed products unless an exact later consumer needs a
different source shape.
After V550, do not rebuild the source-side composed-function Theorem 2.1.13
nonnegative `ℝ≥0∞` zero-factor formulas for finite/range/Ico or one-based
range/Ico/`Icc` composed products unless an exact later consumer needs a
different source shape.
After V551, do not rebuild the source-side composed-function Theorem 2.1.13
real nonnegative `ENNReal.ofReal` product formulas for finite/range/Ico or
one-based range/Ico/`Icc` composed products unless an exact later consumer
needs a different source shape.
After V552, do not rebuild the source-side composed-function Theorem 2.1.13
real nonnegative `ENNReal.ofReal` zero-factor formulas for finite/range/Ico
...
range/Ico/`Icc` composed products unless an exact later consumer needs a
different source shape.
After V554, do not rebuild the source-side composed-function Theorem 2.1.13
absolute-value/norm zero-factor formulas for finite/range/Ico or one-based
range/Ico/`Icc` composed products unless an exact later consumer needs a
different source shape.
After V555, do not rebuild the source-side iid/indexed-transform Theorem
...
one-based range/Ico/`Icc` products unless an exact later consumer needs a
different source shape.
After V557, do not rebuild the source-side iid/common-transform Theorem 2.1.13
absolute-value/norm power formulas for finite/range/Ico or one-based
range/Ico/`Icc` products unless an exact later consumer needs a different
source shape.
After V558, do not rebuild the source-side iid/common-transform Theorem 2.1.13
...
range/Ico/`Icc` products unless an exact later consumer needs a different
source shape.
After V560, do not rebuild the source-side iid/common-transform Theorem 2.1.13
nonnegative `ℝ≥0∞` zero-factor formulas for nonempty finite, positive
range/Ico, or one-based range/Ico/`Icc` products unless an exact later consumer
needs a different source shape.
After V561, do not rebuild the source-side iid/common-transform Theorem 2.1.13
...
After V563, do not rebuild the law-side iid/common-law Theorem 2.1.13
nonnegative `ℝ≥0∞` zero-factor formulas for nonempty finite, positive
range/Ico, or one-based range/Ico/`Icc` products unless an exact later
consumer needs a different source shape.
...
positive range/Ico, or one-based range/Ico/`Icc` products unless an exact
later consumer needs a different source shape.
After V565, do not rebuild the law-side Theorem 2.1.13 absolute-value/norm
...
later consumer needs a different source shape.
After V567, do not rebuild the law-side composed-function Theorem 2.1.13
...
After V570, do not rebuild the Theorem 2.1.13 `IdentDistrib`
partial-sum-difference zero-mean or mixed-term-zero wrappers for zero-based or
one-based future intervals unless an exact later consumer needs a different
base-law source shape.
After V571, do not rebuild the common-law `HasLaw` Theorem 2.1.13
partial-sum-difference zero-mean or mixed-term-zero wrappers for zero-based or
one-based future intervals unless an exact later consumer needs a joint-law or
canonical source shape.
After V572, do not rebuild the full infinite-product or shifted
...
After V574, do not rebuild the canonical iid product-coordinate Theorem
...
After V576, do not rebuild the full infinite-product, shifted
infinite-product joint-law, or canonical iid product-coordinate Theorem
2.1.10 first-variable/tail-product independence wrappers in zero-based or
one-based notation unless an exact later consumer needs a different display
shape.
After V577, do not rebuild the full infinite-product, shifted
infinite-product joint-law, or canonical iid product-coordinate Theorem
2.1.13 first-variable/tail-product expectation-factorization or zero-mean
wrappers in zero-based or one-based notation unless an exact later consumer
needs a different display shape.
After V578, do not rebuild the full infinite-product or shifted
infinite-product joint-law Theorem 2.1.13 law-side range/Ico
product-expectation factorization or zero-factor wrappers in zero-based or
one-based notation unless an exact later consumer needs a different display
shape.
After V579, do not rebuild the canonical iid product-coordinate Theorem
2.1.13 law-side range/Ico product-expectation factorization or zero-factor
wrappers in zero-based or one-based notation unless an exact later consumer
needs a different display shape.
After V580, do not rebuild the full infinite-product or shifted
infinite-product joint-law Theorem 2.1.13 law-side range/Ico product
integrability plus expectation-value or zero-factor wrappers in zero-based or
one-based notation unless an exact later consumer needs a different display
shape.
After V581, do not rebuild the canonical iid product-coordinate Theorem
2.1.13 law-side range/Ico product integrability plus expectation-value or
zero-factor wrappers in zero-based or one-based notation unless an exact later
consumer needs a different display shape.
After V582, do not rebuild the full infinite-product joint-law or canonical iid
...
After V584, do not rebuild the canonical iid shifted-coordinate Theorem
2.1.13 law-side finite-subfamily or literal one-based `Icc` product
integrability plus expectation-value or zero-factor wrappers unless an exact
later consumer needs a different display shape.
After V585, do not rebuild the canonical iid shifted-coordinate Theorem
...
product-expectation factorization or zero-factor wrappers unless an exact
later consumer needs a different display shape.
After V587, do not rebuild the canonical iid common-law Theorem 2.1.13
...
shape.
After V590, do not rebuild the canonical iid common-law Theorem 2.1.13
value-only ordinary zero-product displays for nonempty finite, positive
...
After V591, do not rebuild the source-side iid common-law Theorem 2.1.13
value-only ordinary zero-product displays for nonempty finite, positive
range/Ico, literal one-based `Icc`, or shifted one-based coordinate products
unless an exact later consumer needs a different display shape.
After V592, do not rebuild the joint infinite-product common-law Theorem
2.1.13 value-only ordinary zero-product handoffs for finite, range/Ico,
...
shape.
After V593, do not rebuild the source-side iid common-law Theorem 2.1.13
...
one-based products unless an exact later consumer needs a different display
shape.
After V595, do not rebuild the joint infinite-product common-law Theorem
2.1.13 expectation-exists-and-power-value handoffs for finite, range/Ico,
literal one-based `Icc`, full-law shifted one-based, or shifted-joint-law
one-based products unless an exact later consumer needs a different display
shape.
...
After V597, do not rebuild the joint infinite-product common-law Theorem
2.1.13 value-only power handoffs for finite, range/Ico, literal one-based
`Icc`, full-law shifted one-based, or shifted-joint-law one-based products
unless an exact later consumer needs a different display shape.
Do not return to
...
solved Theorem 2.4.9 one-based bounded middle-partition squeeze wrappers in
...
pairwise-identically-distributed, and canonical iid source shapes,
solved Theorem 2.4.9 arbitrary-tolerance one-based global
middle-partition-with-tails squeeze wrappers in raw shifted empirical-function
and range-sum display forms for source, joint-law, shifted-joint-law,
identical-distribution, pairwise-identically-distributed, and canonical iid
source shapes,
solved Theorem 2.4.9 one-based route-named `middlePartitionWithTails`
outer-a.s. uniform-deviation range-sum endpoints for source, joint-law,
shifted-joint-law, identical-distribution, pairwise-identically-distributed,
and canonical iid source shapes,
...
under `IdentDistrib` for finite/range/Ico and one-based range/Ico/`Icc`
products,
solved source-side Theorem 2.1.13 paired expectation-exists-and-value wrappers
for finite/range/Ico and one-based range/Ico/`Icc` products, including iid
`IdentDistrib` power-value variants,
...
finite/range/Ico and one-based range/Ico/`Icc` products,
solved law-side Theorem 2.1.13 zero-factor expectation-exists wrappers for
finite/range/Ico and one-based range/Ico/`Icc` products,
...
solved source-side iid/common-transform composed Theorem 2.1.13 zero-factor
expectation-exists wrappers for finite/range/Ico and one-based range/Ico/
`Icc` products,
solved source-side iid/indexed-transform composed Theorem 2.1.13
expectation-exists-and-value wrappers for finite/range/Ico and one-based
range/Ico/`Icc` products,
solved source-side iid/indexed-transform composed Theorem 2.1.13 zero-factor
expectation-exists wrappers for finite/range/Ico and one-based range/Ico/
`Icc` products,
...
products,
solved source-side iid/indexed-transform Theorem 2.1.13 nonnegative `ℝ≥0∞`
product formulas for finite/range/Ico and one-based range/Ico/`Icc` products,
solved source-side iid/indexed-transform Theorem 2.1.13 nonnegative `ℝ≥0∞`
zero-factor formulas for finite/range/Ico and one-based range/Ico/`Icc`
products,
solved source-side iid/indexed-transform Theorem 2.1.13 real nonnegative
`ENNReal.ofReal` product formulas for finite/range/Ico and one-based
range/Ico/`Icc` products,
solved source-side iid/indexed-transform Theorem 2.1.13 real nonnegative
`ENNReal.ofReal` zero-factor formulas for finite/range/Ico and one-based
range/Ico/`Icc` products,
solved source-side composed-function Theorem 2.1.13 nonnegative `ℝ≥0∞`
...
solved law-side composed-function Theorem 2.1.13 absolute-value/norm product
...
Latest verified target V597 adds joint infinite-product common-law Chapter
2.1.13 value-only power handoffs over finite subfamilies, range/Ico intervals,
literal one-based `Icc`, full-law shifted one-based coordinate forms, and
shifted-joint-law one-based coordinate forms:
`durrett2019_theorem_2_1_13_integral_finset_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_oneBased_Icc_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_finset_law_prod_eq_pow_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_pow_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_pow_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_shift_integral_oneBased_Icc_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_finset_law_prod_eq_pow_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_pow_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_pow_integral_oneBased_of_shift_hasLaw_infinitePi`, and
`durrett2019_theorem_2_1_13_integral_oneBased_Icc_law_prod_eq_pow_integral_of_shift_hasLaw_infinitePi`.
These wrappers let joint infinite-product law consumers use iid common-law
power expectation values without carrying extra measurability hypotheses when
integrability packaging is not needed.
Next target:
...
Previous verified target V596 adds direct source-side iid shifted common-law
...
range/Ico intervals, literal one-based `Icc`, full-law shifted one-based
coordinate forms, and shifted-joint-law one-based coordinate forms:
`durrett2019_theorem_2_1_13_integrable_and_integral_finset_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_oneBased_Icc_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_finset_law_prod_eq_pow_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_pow_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_pow_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_shift_oneBased_Icc_law_prod_eq_pow_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_finset_law_prod_eq_pow_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_pow_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_pow_integral_oneBased_of_shift_hasLaw_infinitePi`, and
`durrett2019_theorem_2_1_13_integrable_and_integral_oneBased_Icc_law_prod_eq_pow_integral_of_shift_hasLaw_infinitePi`.
These wrappers let joint infinite-product law consumers carry both product
integrability and the iid common-law power expectation value directly from a
single integrable law-side factor.
Next target:
search for another genuinely missing 2.4.9 proof-step/final-display source
wrapper, or add the next Chapter 2.1 product-law/product-expectation handoff
that directly supports 2.4.9 or the adjacent Kolmogorov-maximal route.

Previous verified target V594 adds joint infinite-product common-law Chapter
...
positive range/Ico intervals, literal one-based `Icc`, and shifted one-based
coordinate forms:
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_finset_law_prod_eq_zero_of_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_law_prod_eq_zero_of_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_law_prod_eq_zero_of_lt_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_shift_integrable_and_integral_finset_law_prod_eq_zero_of_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_law_prod_eq_zero_oneBased_of_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_law_prod_eq_zero_oneBased_of_lt_and_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_iid_shift_integrable_and_integral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_integral_eq_zero`.
These wrappers let source iid consumers carry both product integrability and
the vanishing product expectation directly from a common law-side zero mean
and a nonempty/positive index condition.
Next target:
search for another genuinely missing 2.4.9 proof-step/final-display source
wrapper, or add the next Chapter 2.1 product-law/product-expectation handoff
that directly supports 2.4.9 or the adjacent Kolmogorov-maximal route.

Previous verified target V592 adds joint infinite-product common-law Chapter
2.1.13 value-only ordinary zero-product handoffs over nonempty finite
subfamilies, positive range/Ico intervals, literal one-based `Icc`, full-law
shifted one-based coordinate forms, and shifted-joint-law one-based
coordinate forms:
`durrett2019_theorem_2_1_13_integral_finset_law_prod_eq_zero_of_hasLaw_infinitePi_and_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_zero_of_hasLaw_infinitePi_and_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_zero_of_hasLaw_infinitePi_and_lt_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_oneBased_Icc_law_prod_eq_zero_of_hasLaw_infinitePi_and_one_le_and_integral_eq_zero`,
...
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_zero_oneBased_of_hasLaw_infinitePi_and_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_zero_oneBased_of_hasLaw_infinitePi_and_lt_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_shift_integral_oneBased_Icc_law_prod_eq_zero_of_hasLaw_infinitePi_and_one_le_and_integral_eq_zero`,
...
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_lt_and_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_integral_oneBased_Icc_law_prod_eq_zero_of_shift_hasLaw_infinitePi_and_one_le_and_integral_eq_zero`.
These wrappers let joint-law consumers use a common law-side zero mean and a
nonempty/positive index condition directly, without first extracting iid data
or naming a selected zero factor.
Previous verified target V591 adds source-side iid common-law Chapter 2.1.13
value-only ordinary zero-product displays over nonempty finite subfamilies,
positive range/Ico intervals, literal one-based `Icc`, and shifted one-based
coordinate forms:
`durrett2019_theorem_2_1_13_iid_integral_finset_law_prod_eq_zero_of_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integral_range_law_prod_eq_zero_of_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integral_Ico_law_prod_eq_zero_of_lt_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_shift_integral_finset_law_prod_eq_zero_of_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integral_range_law_prod_eq_zero_oneBased_of_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integral_Ico_law_prod_eq_zero_oneBased_of_lt_and_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_iid_shift_integral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_integral_eq_zero`.
These wrappers let source iid consumers get the vanishing expectation directly
from a common law-side zero mean and a nonempty/positive index condition,
without naming a selected zero factor or unpacking product-integrability
packages.
Previous verified target V590 adds canonical iid common-law Chapter 2.1.13
value-only ordinary zero-product displays over nonempty finite subfamilies,
positive range/Ico intervals, literal one-based `Icc`, and shifted one-based
coordinate forms:
`durrett2019_theorem_2_1_13_canonical_iid_integral_finset_law_prod_eq_zero_of_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_range_law_prod_eq_zero_of_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_zero_of_lt_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_shift_integral_finset_law_prod_eq_zero_of_nonempty_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_range_law_prod_eq_zero_oneBased_of_pos_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_zero_oneBased_of_lt_and_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_canonical_iid_shift_integral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_integral_eq_zero`.
These wrappers let canonical product-coordinate consumers get the vanishing
expectation directly from a common law-side zero mean and a nonempty/positive
index condition, without carrying or destructuring the V589 product
integrability conjunction.
Previous verified target V589 adds canonical iid common-law Chapter 2.1.13
...
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_pow_integral_oneBased`, and
`durrett2019_theorem_2_1_13_canonical_iid_shift_integral_oneBased_Icc_law_prod_eq_pow_integral`.
These wrappers give canonical product-coordinate consumers the iid/common-law
power form directly, including the shifted one-based displays used around
Kolmogorov-style partial-sum blocks, without requiring callers to rewrite a
constant product by hand.
Next target:
search for another genuinely missing 2.4.9 proof-step/final-display source
wrapper, or add the next Chapter 2.1 product-law/product-expectation handoff
that directly supports 2.4.9 or the adjacent Kolmogorov-maximal route.

Previous verified target V586 adds canonical iid product-coordinate Chapter
...
`durrett2019_theorem_2_1_13_canonical_iid_shift_integral_oneBased_Icc_law_prod_eq_prod_integral`,
`durrett2019_theorem_2_1_13_canonical_iid_shift_integral_finset_law_prod_eq_zero_of_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_canonical_iid_shift_integral_oneBased_Icc_law_prod_eq_zero_of_integral_eq_zero`.
These wrappers give canonical product-coordinate consumers the same shifted
finite-family expectation-value displays as the V584 integrability-packaged
forms, without requiring callers to carry the product-integrability
conjunction.
Next target:
search for another genuinely missing 2.4.9 proof-step/final-display source
wrapper, or add the next Chapter 2.1 product-law/product-expectation handoff
...
Previous verified target V584 adds canonical iid shifted-coordinate Chapter
...
`durrett2019_theorem_2_1_13_canonical_iid_shift_integrable_and_integral_finset_law_prod_eq_zero_of_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_canonical_iid_shift_integrable_and_integral_oneBased_Icc_law_prod_eq_zero_of_integral_eq_zero`.
These wrappers give canonical product-coordinate consumers the same shifted
finite-family expectation-exists packaging that V583 supplied for shifted
joint laws.

Previous verified target V583 adds shifted infinite-product joint-law Chapter
2.1.13 law-side finite-subfamily and literal one-based `Icc` product
integrability plus expectation-value and zero-factor wrappers:
...
`durrett2019_theorem_2_1_13_integrable_and_integral_oneBased_Icc_law_prod_eq_zero_of_hasLaw_infinitePi_and_integral_eq_zero`,
...
`durrett2019_theorem_2_1_13_canonical_iid_integrable_and_integral_range_law_prod_eq_zero_of_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integrable_and_integral_Ico_law_prod_eq_zero_of_integral_eq_zero`,
...
`durrett2019_theorem_2_1_13_canonical_iid_integrable_and_integral_range_law_prod_eq_zero_of_integral_eq_zero_oneBased`, and
`durrett2019_theorem_2_1_13_canonical_iid_integrable_and_integral_Ico_law_prod_eq_zero_of_integral_eq_zero_oneBased`.

Previous verified target V580 adds full infinite-product and shifted
infinite-product joint-law Chapter 2.1.13 law-side range/Ico product
integrability plus expectation-value and zero-factor wrappers:
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_prod_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_prod_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_zero_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_zero_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_prod_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_prod_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_prod_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_prod_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integrable_and_integral_range_law_prod_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_integrable_and_integral_Ico_law_prod_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_integral_eq_zero`.

Previous verified target V579 adds canonical iid product-coordinate Chapter
2.1.13 law-side range/Ico product-expectation factorization and zero-factor
wrappers:
`durrett2019_theorem_2_1_13_canonical_iid_integral_range_law_prod_eq_prod_integral`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_prod_integral`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_range_law_prod_eq_zero_of_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_zero_of_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_range_law_prod_eq_prod_integral_oneBased`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_prod_integral_oneBased`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_range_law_prod_eq_zero_of_integral_eq_zero_oneBased`, and
`durrett2019_theorem_2_1_13_canonical_iid_integral_Ico_law_prod_eq_zero_of_integral_eq_zero_oneBased`.

Previous verified target V578 adds full infinite-product and shifted
infinite-product joint-law Chapter 2.1.13 law-side range/Ico
product-expectation factorization and zero-factor wrappers:
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_prod_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_prod_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_zero_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_zero_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_prod_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_prod_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_prod_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_prod_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_range_law_prod_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_integral_eq_zero`, and
`durrett2019_theorem_2_1_13_integral_Ico_law_prod_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_integral_eq_zero`.

Previous verified target V577 adds full infinite-product, shifted
infinite-product joint-law, and canonical iid product-coordinate Chapter
2.1.13 expectation-factorization and zero-mean wrappers for the
first-variable/tail-product surface:
`durrett2019_theorem_2_1_13_integral_first_mul_tailProduct_eq_mul_integral_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_first_mul_tailProduct_eq_zero_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_first_mul_tailProduct_eq_mul_integral_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_first_mul_tailProduct_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_integral_first_mul_tailProduct_eq_mul_integral_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_13_integral_first_mul_tailProduct_eq_zero_oneBased_of_shift_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_first_mul_tailProduct_eq_mul_integral`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_first_mul_tailProduct_eq_zero_of_integral_eq_zero`,
`durrett2019_theorem_2_1_13_canonical_iid_integral_first_mul_tailProduct_eq_mul_integral_oneBased`, and
`durrett2019_theorem_2_1_13_canonical_iid_integral_first_mul_tailProduct_eq_zero_of_integral_eq_zero_oneBased`.

Previous verified target V576 adds full infinite-product, shifted
infinite-product joint-law, and canonical iid product-coordinate Theorem
2.1.10 first-variable/tail-product independence wrappers:
`durrett2019_theorem_2_1_10_indepFun_first_tailProduct_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_10_indepFun_first_tailProduct_oneBased_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_10_indepFun_first_tailProduct_oneBased_of_shift_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_10_canonical_iid_first_tailProduct`, and
`durrett2019_theorem_2_1_10_canonical_iid_first_tailProduct_oneBased`.

Previous verified target V575 adds full infinite-product and shifted
...
`durrett2019_theorem_2_1_10_canonical_iid_partialSumDiff_earlyBlockIndicator`,
`durrett2019_theorem_2_1_10_canonical_iid_lateIncrementSum_earlyBlockFunction_oneBased`,
`durrett2019_theorem_2_1_10_canonical_iid_partialSumDiff_earlyBlockFunction_oneBased`,
and
`durrett2019_theorem_2_1_10_canonical_iid_partialSumDiff_earlyBlockIndicator_oneBased`.
These wrappers let Kolmogorov-maximal and adjacent independent-sum consumers
...
`durrett2019_theorem_2_1_13_partialSumDiff_integral_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_partialSumDiff_mul_earlyBlockFunction_integral_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_partialSumDiff_mul_earlyBlockIndicatorSum_integral_eq_zero_oneBased_of_hasLaw_infinitePi_and_integral_eq_zero`,
...
These wrappers let Kolmogorov-maximal and adjacent independent-sum consumers
start from a common law with `∫ x, x dν = 0`, rather than restating per-index
source integrability and zero means over every future interval.

Previous verified target V570 adds Chapter 2.1.13 partial-sum-difference
source wrappers under `IdentDistrib`:
`durrett2019_theorem_2_1_13_partialSumDiff_integral_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_partialSumDiff_mul_earlyBlockFunction_integral_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_partialSumDiff_mul_earlyBlockIndicatorSum_integral_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_partialSumDiff_integral_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_partialSumDiff_mul_earlyBlockFunction_integral_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`,
and
`durrett2019_theorem_2_1_13_partialSumDiff_mul_earlyBlockIndicatorSum_integral_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`.
These wrappers let Kolmogorov-maximal and adjacent independent-sum consumers
start from one integrable zero-mean base marginal plus common distribution,
rather than restating per-index integrability and zero means over every future
interval.

Previous verified target V569 adds non-atomic Durrett Theorem 2.4.9
...
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_law_norm_comp_prod_eq_prod_lintegral_norm`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_law_norm_comp_prod_eq_prod_lintegral_norm`,
...
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_indexed_norm_comp_prod_eq_prod_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_indexed_norm_comp_prod_eq_prod_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_law_indexed_norm_comp_prod_eq_prod_lintegral_norm`,
...
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_norm_comp_prod_eq_pow_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_norm_comp_prod_eq_pow_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_law_norm_comp_prod_eq_pow_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_norm_comp_prod_eq_pow_lintegral_norm_oneBased`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_norm_comp_prod_eq_pow_lintegral_norm_oneBased`.
These transfer the source-side composed norm product factorization through
`HasLaw`: after measurable transforms `f_i : S_i -> 𝕜`, the source-space norm
`lintegral` factors into law-side norm `lintegral`s.  Common-law
...
Earlier verified target V565 adds law-side Chapter 2.1.13
...
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_law_norm_prod_eq_prod_lintegral_norm`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_law_norm_prod_eq_prod_lintegral_norm`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_law_norm_prod_eq_prod_lintegral_norm_oneBased`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_law_norm_prod_eq_prod_lintegral_norm_oneBased`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_law_norm_prod_eq_prod_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_finset_law_norm_prod_eq_pow_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_norm_prod_eq_pow_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_norm_prod_eq_pow_lintegral_norm`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_norm_prod_eq_pow_lintegral_norm_oneBased`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_norm_prod_eq_pow_lintegral_norm_oneBased`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_law_norm_prod_eq_pow_lintegral_norm`.
These transfer the already compiled source-side norm product factorization
through marginal `HasLaw` hypotheses, so `∫⁻ ENNReal.ofReal ‖∏ X_i‖` factors
...
Earlier verified target V563 adds law-side iid/common-law Chapter 2.1.13
...
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_prod_eq_zero_of_lt_and_lintegral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_law_prod_eq_zero_of_one_le_and_lintegral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_prod_eq_zero_oneBased_of_pos_and_lintegral_eq_zero`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_prod_eq_zero_oneBased_of_lt_and_lintegral_eq_zero`.
These specialize the existing law-side nonnegative zero-factor corollaries to
the common-law/common-transform iid display: for independent variables with
...
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_ofReal_comp_prod_eq_pow_lintegral_ofReal_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_ofReal_comp_prod_eq_pow_lintegral_ofReal_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_ofReal_comp_prod_eq_pow_lintegral_ofReal_oneBased_of_identDistrib`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_ofReal_comp_prod_eq_pow_lintegral_ofReal_oneBased_of_identDistrib`.
These specialize the V547 source-side iid/indexed-transform real nonnegative
product formulas to a common-transform power display: for an independent
source family `X_i` with `IdentDistrib` marginals and one nonnegative
...
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_norm_comp_prod_eq_zero_of_lt_identDistrib_and_lintegral_norm_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_norm_comp_prod_eq_zero_of_one_le_identDistrib_and_lintegral_norm_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_norm_comp_prod_eq_zero_oneBased_of_pos_identDistrib_and_lintegral_norm_base_eq_zero`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_norm_comp_prod_eq_zero_oneBased_of_lt_identDistrib_and_lintegral_norm_base_eq_zero`.
These specialize the V556 iid/indexed-transform norm zero-factor formulas to
a common-transform nonempty-product display: for an independent source family
`X_i` with `IdentDistrib` marginals and one measurable transform `f : S -> 𝕜`,
they package the source-space `lintegral` of `‖∏ f(X_i)‖` as zero whenever the
...
`durrett2019_theorem_2_1_13_iid_lintegral_finset_norm_comp_prod_eq_pow_lintegral_norm_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_norm_comp_prod_eq_pow_lintegral_norm_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_norm_comp_prod_eq_pow_lintegral_norm_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_norm_comp_prod_eq_pow_lintegral_norm_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_norm_comp_prod_eq_pow_lintegral_norm_oneBased_of_identDistrib`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_norm_comp_prod_eq_pow_lintegral_norm_oneBased_of_identDistrib`.
These specialize the V555 iid/indexed-transform norm product formulas to a
common-transform power display: for an independent source family `X_i` with
`IdentDistrib` marginals and one measurable transform `f : S -> 𝕜`, they
package the source-space `lintegral` of `‖∏ f(X_i)‖` as powers of the
base-coordinate norm `lintegral` over `X_0`, on finite/range/Ico and
one-based index sets without `HasLaw` hypotheses or ordinary integrability
assumptions.

Previous verified target V556 adds source-side iid/indexed-transform Chapter
2.1.13 absolute-value/norm zero-factor formulas:
...
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_norm_comp_prod_eq_prod_lintegral_norm_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_norm_comp_prod_eq_prod_lintegral_norm_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_norm_comp_prod_eq_prod_lintegral_norm_base_oneBased_of_identDistrib`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_norm_comp_prod_eq_prod_lintegral_norm_base_oneBased_of_identDistrib`.
These specialize the composed norm product formula through identical
distribution: for an independent source family `X_i` with `IdentDistrib`
marginals and index-dependent measurable transforms `f_i : S -> 𝕜`, they
package the source-space `lintegral` of `‖∏ f_i(X_i)‖` as products of
...
2.1.13 absolute-value/norm zero-factor formulas:
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_finset_norm_comp_prod_eq_zero_of_lintegral_norm_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_norm_comp_prod_eq_zero_of_lintegral_norm_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_norm_comp_prod_eq_zero_of_lintegral_norm_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_norm_comp_prod_eq_zero_of_lintegral_norm_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_norm_comp_prod_eq_zero_of_lintegral_norm_eq_zero_oneBased`,
and
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_norm_comp_prod_eq_zero_of_lintegral_norm_eq_zero_oneBased`.
These specialize the V553 composed norm product formulas to zero-factor
conclusions: for an independent measurable source family `X_i` and measurable
transforms `f_i : S_i -> 𝕜`, they package the source-space `lintegral` of
`‖∏ f_i(X_i)‖` as zero whenever one selected composed norm factor `lintegral`
vanishes, on finite/range/Ico and one-based index sets without `HasLaw`
hypotheses or ordinary integrability assumptions.

Previous verified target V553 adds source-side composed-function Chapter
2.1.13 absolute-value/norm product formulas:
...
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_ofReal_comp_prod_eq_zero_of_lintegral_ofReal_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_ofReal_comp_prod_eq_zero_of_lintegral_ofReal_eq_zero_oneBased`,
and
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_ofReal_comp_prod_eq_zero_of_lintegral_ofReal_eq_zero_oneBased`.
These specialize the V551 composed real nonnegative `ENNReal.ofReal` product
formulas to zero-factor conclusions: for an independent measurable source
family `X_i` and nonnegative measurable real transforms `f_i : S_i -> ℝ`,
they package `∫⁻ ENNReal.ofReal (∏ f_i(X_i)) dP = 0` whenever one selected
...
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_ofReal_comp_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_ofReal_comp_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_ofReal_comp_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_ofReal_comp_prod_eq_prod_lintegral_ofReal_oneBased`,
and
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_ofReal_comp_prod_eq_prod_lintegral_ofReal_oneBased`.
These lift the primitive real nonnegative `ENNReal.ofReal` product
factorization through measurable composition: for an independent measurable
source family `X_i` and nonnegative measurable real transforms
`f_i : S_i -> ℝ`, they package
`∫⁻ ENNReal.ofReal (∏ f_i(X_i)) dP =
∏ ∫⁻ ENNReal.ofReal (f_i(X_i)) dP` on finite/range/Ico and one-based index
sets without `HasLaw` hypotheses or ordinary integrability assumptions.

Previous verified target V550 adds source-side composed-function Chapter 2.1.13
nonnegative `ℝ≥0∞` zero-factor formulas:
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_finset_comp_prod_eq_zero_of_lintegral_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_comp_prod_eq_zero_of_lintegral_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_comp_prod_eq_zero_of_lintegral_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_comp_prod_eq_zero_of_lintegral_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_comp_prod_eq_zero_of_lintegral_eq_zero_oneBased`,
and
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_comp_prod_eq_zero_of_lintegral_eq_zero_oneBased`.
These specialize the V549 composed nonnegative `lintegral` product formulas to
zero-factor conclusions: for an independent measurable source family `X_i`
and measurable transforms `f_i : S_i -> ℝ≥0∞`, they package
`∫⁻ ∏ f_i(X_i) dP = 0` whenever one selected composed factor
`∫⁻ f_i(X_i) dP` vanishes, on finite/range/Ico and one-based index sets
without `HasLaw` hypotheses or ordinary integrability assumptions.

Previous verified target V549 adds source-side composed-function Chapter
2.1.13 nonnegative `ℝ≥0∞` product formulas:
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_finset_comp_prod_eq_prod_lintegral`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_comp_prod_eq_prod_lintegral`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_comp_prod_eq_prod_lintegral`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_oneBased_Icc_comp_prod_eq_prod_lintegral`,
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_range_comp_prod_eq_prod_lintegral_oneBased`,
and
`durrett2019_theorem_2_1_13_iIndepFun_lintegral_Ico_comp_prod_eq_prod_lintegral_oneBased`.
These lift the primitive nonnegative `lintegral` product factorization through
measurable composition: for an independent measurable source family `X_i` and
measurable transforms `f_i : S_i -> ℝ≥0∞`, they package
`∫⁻ ∏ f_i(X_i) dP = ∏ ∫⁻ f_i(X_i) dP` on finite/range/Ico and one-based index
sets without `HasLaw` hypotheses or ordinary integrability assumptions.

Previous verified target V548 adds source-side iid/indexed-transform Chapter
2.1.13 real nonnegative `ENNReal.ofReal` zero-factor formulas:
`durrett2019_theorem_2_1_13_iid_lintegral_finset_ofReal_comp_prod_eq_zero_of_identDistrib_and_lintegral_ofReal_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_ofReal_comp_prod_eq_zero_of_identDistrib_and_lintegral_ofReal_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_ofReal_comp_prod_eq_zero_of_identDistrib_and_lintegral_ofReal_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_ofReal_comp_prod_eq_zero_of_identDistrib_and_lintegral_ofReal_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_ofReal_comp_prod_eq_zero_oneBased_of_identDistrib_and_lintegral_ofReal_base_eq_zero`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_ofReal_comp_prod_eq_zero_oneBased_of_identDistrib_and_lintegral_ofReal_base_eq_zero`.
These specialize the V547 source-side `ENNReal.ofReal` product formulas to
zero-factor conclusions under `IdentDistrib`, with index-dependent
nonnegative measurable real transforms `f_i : S -> ℝ`.  They package
`∫⁻ ENNReal.ofReal (∏ f_i(X_i)) dP = 0` whenever one selected
base-coordinate `∫⁻ ENNReal.ofReal (f_i(X_0)) dP` vanishes, giving
source-only mixed real-valued nonnegative moment and indicator-product
cancellations without `HasLaw` hypotheses or ordinary integrability
assumptions.

Previous verified target V547 adds source-side iid/indexed-transform Chapter
2.1.13 real nonnegative `ENNReal.ofReal` product formulas:
`durrett2019_theorem_2_1_13_iid_lintegral_finset_ofReal_comp_prod_eq_prod_lintegral_ofReal_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_ofReal_comp_prod_eq_prod_lintegral_ofReal_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_ofReal_comp_prod_eq_prod_lintegral_ofReal_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_ofReal_comp_prod_eq_prod_lintegral_ofReal_base_of_identDistrib`,
...
These specialize the generic source-side `ENNReal.ofReal` nonnegative product
factorization to iid source families under `IdentDistrib`, with
index-dependent nonnegative measurable real transforms `f_i : S -> ℝ`.  They
package `∫⁻ ENNReal.ofReal (∏ f_i(X_i)) dP =
∏ ∫⁻ ENNReal.ofReal (f_i(X_0)) dP` directly on the source space, giving
mixed real-valued nonnegative moment and indicator-product support without
`HasLaw` hypotheses or ordinary integrability assumptions.

Previous verified target V546 adds source-side iid/indexed-transform Chapter
...
`durrett2019_theorem_2_1_13_iid_lintegral_range_comp_prod_eq_zero_of_identDistrib_and_lintegral_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_comp_prod_eq_zero_of_identDistrib_and_lintegral_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_comp_prod_eq_zero_of_identDistrib_and_lintegral_base_eq_zero`,
...
These specialize the V545 source-side nonnegative product formulas to
zero-factor conclusions under `IdentDistrib`, with index-dependent measurable
transforms `f_i : S -> ℝ≥0∞`.  They package
`∫⁻ ∏ f_i(X_i) dP = 0` whenever one selected base-coordinate
`∫⁻ f_i(X_0) dP` vanishes, giving source-only mixed nonnegative moment and
indicator-product cancellations without `HasLaw` hypotheses or ordinary
integrability assumptions.

Previous verified target V545 adds source-side iid/indexed-transform Chapter
2.1.13 nonnegative `ℝ≥0∞` product formulas:
`durrett2019_theorem_2_1_13_iid_lintegral_finset_comp_prod_eq_prod_lintegral_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_comp_prod_eq_prod_lintegral_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_comp_prod_eq_prod_lintegral_base_of_identDistrib`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_comp_prod_eq_prod_lintegral_base_of_identDistrib`,
...
These specialize the generic source-side nonnegative product factorization to
iid source families under `IdentDistrib`, with index-dependent measurable
transforms `f_i : S -> ℝ≥0∞`.  They package
`∫⁻ ∏ f_i(X_i) dP = ∏ ∫⁻ f_i(X_0) dP` directly on the source space, giving
mixed nonnegative moment and indicator-product support without `HasLaw`
hypotheses or ordinary integrability assumptions.

Previous verified target V544 adds law-side iid/indexed-transform Chapter 2.1.13
nonnegative `ℝ≥0∞` zero-factor formulas:
...
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_law_indexed_ofReal_prod_eq_zero_of_lintegral_ofReal_eq_zero`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_indexed_ofReal_prod_eq_zero_of_lintegral_ofReal_eq_zero_oneBased`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_indexed_ofReal_prod_eq_zero_of_lintegral_ofReal_eq_zero_oneBased`.
These specialize the generic law-side real nonnegative zero-factor formula to
...
2.1.13 real nonnegative `ENNReal.ofReal` product formulas:
`durrett2019_theorem_2_1_13_iid_lintegral_finset_law_indexed_ofReal_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_indexed_ofReal_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_indexed_ofReal_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iid_lintegral_oneBased_Icc_law_indexed_ofReal_prod_eq_prod_lintegral_ofReal`,
`durrett2019_theorem_2_1_13_iid_lintegral_range_law_indexed_ofReal_prod_eq_prod_lintegral_ofReal_oneBased`,
and
`durrett2019_theorem_2_1_13_iid_lintegral_Ico_law_indexed_ofReal_prod_eq_prod_lintegral_ofReal_oneBased`.
These specialize the generic law-side real nonnegative product formula to a
common law `μ` with index-dependent nonnegative measurable transforms
`f_i : S -> ℝ`, packaging
`∫⁻ ENNReal.ofReal (∏ f_i(X_i)) dP =
  ∏ ∫⁻ ENNReal.ofReal (f_i x) dμ` without ordinary integrability assumptions.

Previous verified target V541 adds law-side iid/indexed-transform Chapter
2.1.13 nonnegative `lintegral` product formulas:
...
mixed nonnegative identity `∫⁻ ∏ f_i(X_i) dP = ∏ ∫⁻ f_i dμ`.
...
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_law_indexed_prod_eq_prod_integral`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_law_indexed_prod_eq_prod_integral_oneBased`,
and
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_law_indexed_prod_eq_prod_integral_oneBased`.
These specialize the generic law-side product formula to a common law `μ` with
index-dependent transforms `f_i`, packaging source-space integrability and the
mixed product expectation
...
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_finset_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_base_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_comp_prod_eq_zero_oneBased_of_integrable_identDistrib_and_integral_base_eq_zero`,
and
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_comp_prod_eq_zero_oneBased_of_integrable_identDistrib_and_integral_base_eq_zero`.
These compose each indexed measurable transform `f_i` through
`IdentDistrib (X_i) (X_0)`, transfer integrability and the selected zero mean
from the base coordinate, and then reuse the source-side zero-factor product
package.  They are the no-`HasLaw` zero-factor companion to V537 and support
Durrett's centered mixed-power cancellations such as `E(X_i^3 X_j)` and
`E(X_i^2 X_j X_k)` when one transformed base factor has mean zero.

Previous verified target V537 adds source-side iid/indexed-transform composed
Chapter 2.1.13 expectation-exists-and-value wrappers:
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_finset_comp_prod_eq_prod_integral_base_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_comp_prod_eq_prod_integral_base_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_comp_prod_eq_prod_integral_base_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_comp_prod_eq_prod_integral_base_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_comp_prod_eq_prod_integral_base_oneBased_of_integrable_identDistrib`,
and
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_comp_prod_eq_prod_integral_base_oneBased_of_integrable_identDistrib`.
These compose each indexed measurable transform `f_i` through
`IdentDistrib (X_i) (X_0)`, transfer integrability from the base coordinate,
and then reuse the source-side composed product formula.  They are the
no-`HasLaw` source wrapper for Durrett's mixed-power factorization step, e.g.
`E(X_i^3 X_j)` and `E(X_i^2 X_j X_k)`.

Previous verified target V536 adds source-side iid/common-transform composed
Chapter 2.1.13 zero-factor expectation-exists wrappers:
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_finset_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_comp_prod_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_comp_prod_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`,
and
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_comp_prod_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`.
These compose `IdentDistrib` through a common measurable transform `f`, transfer
integrability and zero mean from `f(X_0)`, and then reuse the source-side
zero-factor product package for the transformed family.  They are the no-
`HasLaw` common-transform analogue of the V532 law-side iid zero-factor surface.

Previous verified target V535 adds source-side iid/common-transform composed
...
`durrett2019_theorem_2_1_13_iIndepFun_integrable_and_integral_oneBased_Icc_comp_prod_eq_zero_of_integrable_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iIndepFun_integrable_and_integral_range_comp_prod_eq_zero_oneBased_of_integrable_and_integral_eq_zero`,
and
`durrett2019_theorem_2_1_13_iIndepFun_integrable_and_integral_Ico_comp_prod_eq_zero_oneBased_of_integrable_and_integral_eq_zero`.
These apply the Chapter 2.1.10 composition theorem to `f_i ∘ X_i` and then
...
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_prod_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_prod_eq_zero_of_integrable_identDistrib_and_integral_eq_zero`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_prod_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`,
and
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_prod_eq_zero_oneBased_of_integrable_identDistrib_and_integral_eq_zero`.
...
These are the `HasLaw` analogues of V529, combining source-space
integrability of composed finite products with the zero-factor expectation
conclusion from a law-side zero integral.

Previous verified target V529 adds source-side Chapter 2.1.13 zero-factor
expectation-exists wrappers:
...
`durrett2019_theorem_2_1_13_iIndepFun_integrable_and_integral_range_prod_eq_prod_integral_of_integrable`,
`durrett2019_theorem_2_1_13_iIndepFun_integrable_and_integral_Ico_prod_eq_prod_integral_of_integrable`,
`durrett2019_theorem_2_1_13_iIndepFun_integrable_and_integral_oneBased_Icc_prod_eq_prod_integral_of_integrable`,
...
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_finset_prod_eq_pow_integral_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_prod_eq_pow_integral_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_prod_eq_pow_integral_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_range_prod_eq_pow_integral_oneBased_of_integrable_identDistrib`,
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_Ico_prod_eq_pow_integral_oneBased_of_integrable_identDistrib`,
and
`durrett2019_theorem_2_1_13_iid_integrable_and_integral_oneBased_Icc_prod_eq_pow_integral_of_integrable_identDistrib`.

Previous verified target V526 adds source-side iid Chapter 2.1.13
...
identical-distribution, pairwise-identically-distributed, and canonical iid
families.  These wrappers expose the final route-specific outer-a.s.
conclusion in the plain `(sum)/(n)` display, complementing the existing
one-based empirical-CDF and exact `n⁻¹ * sum` route endpoints without routing
through the later `empiricalDistributionFunction_outerAlmostSure_*` aliases.
Next target: search for another genuinely missing 2.4.9 proof-step/final-
display source wrapper, or add the next Chapter 2.1 product-law/product-
expectation handoff that directly supports 2.4.9 or the adjacent
Kolmogorov-maximal route.

Previous verified target V515 adds arbitrary-tolerance one-based source-shaped
Durrett Theorem 2.4.9 global middle-partition-with-tails squeeze wrappers:
`durrett2019_theorem_2_4_9_middlePartitionWithTails_oneBased_eventually_uniform_error_lt_of_iIndepFun`,
`durrett2019_theorem_2_4_9_middlePartitionWithTails_oneBased_range_sum_uniform_error_lt_of_iIndepFun`,
and the matching joint infinite-product, shifted joint infinite-product,
identical-distribution, pairwise-identically-distributed, and canonical iid
families.  These wrappers expose the normalized fixed-tolerance global tail
squeeze in the same one-based raw empirical-function and range-sum displays
as V514, avoiding a forced rewrite through exact `n⁻¹ * sum` notation for
later consumers.  Next target: search for another genuinely missing 2.4.9
proof-step/final-display source wrapper, or add the next Chapter 2.1
product-law/product-expectation handoff that directly supports 2.4.9 or the
adjacent Kolmogorov-maximal route.

Previous verified target V514 adds one-based source-shaped Durrett Theorem
...
and the matching joint infinite-product, shifted joint infinite-product,
identical-distribution, pairwise-identically-distributed, and canonical iid
families.  These wrappers expose the bounded middle-partition monotonicity
squeeze in the same one-based raw empirical-function and range-sum displays
available at the finite-cutpoint layer, before the global tail squeeze rewrites
to exact `n⁻¹ * sum` notation.  Next target: search for another genuinely
missing 2.4.9 proof-step/final-display source wrapper, or add the next
Chapter 2.1 product-law/product-expectation handoff that directly supports
2.4.9 or the adjacent Kolmogorov-maximal route.

Previous verified target V512 adds one-based source-shaped Durrett Theorem
...
one-based, and pairwise variants.  These wrappers expose the final theorem
hand-off "a supplied calculation gives the density of the sum" directly from
iid/source/joint/canonical/pairwise hypotheses, so future concrete examples
such as gamma or normal convolution can consume a source-shaped endpoint
without rebuilding selected-pair independence.  Next target: search for a
genuinely missing Chapter 2.1 handoff that supports 2.4.9 or the adjacent
Kolmogorov-maximal route; otherwise return to a proved-missing 2.4.9
proof-step/final-display source wrapper.

Previous verified target V508 adds source-shaped Durrett Theorem 2.1.16
...
law extraction.  V509 closed the supplied-density source layer.
...
and its identical-distribution, joint infinite-product, canonical,
one-based, and pairwise variants;
`durrett2019_theorem_2_1_16_iid_pair_sum_hasLaw_two_lintegral_density_of_iIndepFun`
and its matching source-shape variants; plus the corresponding
`left_real_lintegral_density` and `two_real_lintegral_density` families
...
V507 closed the next density-formula source layer over the existing
density/Fubini core.
...
- Separate law-level handoffs from density/Fubini handoffs.  V506 first
  ...
  shapes; V507 then keeps the density packet strictly as a wrapper layer over
  the existing density formulas, instead of mixing source extraction with
  measure calculation.
...
`durrett2019_theorem_2_1_11_iid_real_Iic_range_cylinder_prob_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_11_iid_real_Iio_range_cylinder_prob_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_11_iid_shift_real_Iic_range_cylinder_prob_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_11_iid_shift_real_Iio_range_cylinder_prob_of_hasLaw_infinitePi`,
`durrett2019_theorem_2_1_11_canonical_iid_real_Iic_range_cylinder_prob`,
`durrett2019_theorem_2_1_11_canonical_iid_real_Iio_range_cylinder_prob`,
`durrett2019_theorem_2_1_11_canonical_iid_shift_real_Iic_range_cylinder_prob`,
and `durrett2019_theorem_2_1_11_canonical_iid_shift_real_Iio_range_cylinder_prob`.
These wrappers expose finite products of `ν ((-∞, x_i])` and
`ν ((-∞, x_i))` directly over `Finset.range n`, in both zero-based and
Durrett one-based displays.
...
`durrett2019_theorem_2_1_11_canonical_iid_finite_restrict_hasLaw_pi`,
`durrett2019_theorem_2_1_11_canonical_iid_shift_finite_restrict_hasLaw_pi`,
`durrett2019_theorem_2_1_11_canonical_iid_finite_cylinder_prob`,
`durrett2019_theorem_2_1_11_canonical_iid_shift_finite_cylinder_prob`,
`durrett2019_theorem_2_1_11_canonical_iid_range_cylinder_prob`, and
`durrett2019_theorem_2_1_11_canonical_iid_oneBased_Icc_cylinder_prob`.
These wrappers package the actual coordinate projections on `ν^ℕ` over
arbitrary finite sets and literal one-based prefixes, complementing V489's
abstract joint-law shifted finite-set statements.

Latest verified target V489 adds joint-law shifted arbitrary finite-dimensional
...
`durrett2019_theorem_2_1_11_iid_hasLaw_infinitePi_shift_finite_restrict_hasLaw_pi`,
`durrett2019_theorem_2_1_11_iid_hasLaw_infinitePi_shift_finite_cylinder_prob`,
and `durrett2019_theorem_2_1_11_iid_hasLaw_infinitePi_shift_range_cylinder_prob`.
These wrappers turn a joint `Measure.infinitePi` law directly into Durrett's
one-based finite-dimensional displays over arbitrary finite sets and finite
prefixes, without reopening the independence extraction or cylinder-probability
plumbing.
...
`durrett2019_theorem_2_1_11_iid_finite_restrict_hasLaw_pi_of_identDistrib`,
and `durrett2019_theorem_2_1_11_iid_finite_cylinder_prob_of_iIndepFun`.
These wrappers handle an arbitrary finite set `s : Finset ℕ`, complementing
the V485/V486 finite-prefix vector statements and the V483 joint-law finite
restriction layer.
...
`durrett2019_theorem_2_1_13_iid_integral_range_law_prod_eq_pow_integral`, and
`durrett2019_theorem_2_1_13_iid_integral_Ico_law_prod_eq_pow_integral`.
These wrappers turn individual distribution and integrability hypotheses into
finite product expectation formulas, including the iid power displays used by
finite-dimensional product-law and empirical-CDF calculations.

Latest verified target V479 adds the law-side separated-product branch of
Durrett Theorem 2.1.12:
`durrett2019_theorem_2_1_12_product_integral_mul_of_integrable`,
`durrett2019_theorem_2_1_12_indepFun_lintegral_law_mul_eq_mul_lintegral`,
`durrett2019_theorem_2_1_12_indepFun_lintegral_law_ofReal_mul_eq_mul_lintegral_ofReal`,
and
`durrett2019_theorem_2_1_12_indepFun_integral_law_mul_eq_mul_integral`.
These wrappers consume `HasLaw` hypotheses directly, so later proofs can use
Durrett's distribution assumptions without reopening measurability plumbing.
...
mass-weight summability consumers.  New compiled anchors include:
...
The V459 shifted-tail plumbing remains solved; V461 later discharges the
monotone annulus partition identity, so future cycles should target the source
mean endpoint and truncated-square kernel estimate.
...
The source annulus-integral-to-mass bridge is discharged by V458; future cycles
should target identity(*) mass-tail summability and truncated-square kernel
majorization, not the finite cover or integral monotonicity.
...
`durrett2019_theorem_2_5_13_truncatedMean_normalized_sum_tendsto_zero_of_prefix_annulus_bound`, and
`durrett2019_theorem_2_5_13_ae_truncated_normalized_sum_tendsto_zero_of_scalar_kernel_bound_and_prefix_annulus_mean_bound`.
The source mean bridge above is discharged by V456; future cycles should
target the base annulus partition and weighted tail estimates, not the
`IdentDistrib` truncated-mean reduction.
...
The finite-annulus bridge into this mean squeeze is discharged by V455; future
cycles should target the measure/integral annulus decomposition, not a generic
prefix-plus-tail bound.
...
They turn the V445 all-real frequent-largeness statement into
`limsup (|S_n| / a_n : EReal) = ⊤` a.e. under the uniform scaled-tail
hypotheses.  The remaining blocker is the source-facing Feller dichotomy
wrapper.
...
formal infinite-limsup endpoint is discharged by V446; the live remaining
blocker is the source-facing Feller dichotomy wrapper.
...
`durrett2019_theorem_2_5_13_summable_of_antitone_subsequence_summable`, and
`durrett2019_theorem_2_5_13_not_summable_subsequence_of_not_summable`.  The
remaining blocker is now probability-tail instantiation plus the
Borel-Cantelli/limsup bridge for the divergent half.
...
and tail estimates on standard rpow Nat-indicator sums.
...
The remaining 2.5.12 blocker is now ordinary real p-series/integral comparison
for those two scalar kernels.
...
The remaining 2.5.12 blocker is now pure finite-`p` tail analysis: prove
weighted base truncated second-moment summability and base large-tail
first-moment scaled summability from the iid finite `p`-moment hypotheses.
...
The remaining 2.5.12 blocker is now purely analytic: derive the weighted base
truncated second-moment summability and normalized truncated-mean convergence
from iid finite `p`-moment source hypotheses.
...
The remaining 2.5.12 blocker after V423 was sharply analytic plus one
deterministic transfer: prove the weighted base truncated second-moment
summability from finite `p`-moment, prove the normalized truncated-mean
estimate, and transfer eventual equality across the `n^(1/p)` normalizer.
...
The proof uses mathlib Cauchy condensation and p-series results from V419,
plus the dyadic comparison
`log(2^k + 2) >= k * log 2`.  Theorem 2.5.11 is now closed at the source
wrapper level currently targeted by this lane.
...
Use `Live In-Thread Goal Prompt V597` at the top of this file.  Historical route
