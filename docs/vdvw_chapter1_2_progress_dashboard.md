...
NEXT       Do not repeat the finite-class geometry/entropy/consumer, finite-center integrability, centered-cover, centered-supremum/pair/sample/random-sign product integrability, canonical iid-sign instantiation, canonical sample-path instantiation, or untruncation bridges.  Use `EmpiricalL1Index`/mathlib internal-cover machinery to prove the remaining non-finite geometric selected fixed-radius entropy bound, or consume the finite-class canonical untruncated consumer in a final theorem-critical finite-class specialization.  The remaining full-route gap is now a structural internal-cover/maximal-separated-set cardinality estimate such as `Metric.coveringNumber <= base^n` for the chosen empirical pseudometric cover; the random-cover domination, selected tail/UI package handoff, analytic package handoff, finite-class cardinality/entropy/untruncated route, finite-cardinality padding, centered cover, centered-supremum/pair/sample/random-sign product integrability, canonical iid signs, canonical sample paths, and `edist` compatibility are compiled.
