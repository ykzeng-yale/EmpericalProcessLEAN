...
Route from `Live In-Thread Goal Prompt V71` in
...
likelihood mass supplies the positive-branch mass-one input.  The next target
is positive-product L1 convergence into that finite-cylinder integral-
convergence input, infinite-product criterion support, and tail-event zero-one
support for Kakutani's dichotomy.
