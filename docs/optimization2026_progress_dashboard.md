...
- Current manual frontier: Chapter 5 CG is past the basic Krylov/termination
  setup and `Theorem58.lean` now starts Theorem 5.8.  The active target is the
  AGF Lyapunov derivative formula from the ODE, then the exact source-facing
  Theorem 5.8 statement and Theorem 5.9.
...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean`, `StatInference/Optimization/Theorem54.lean`, `StatInference/Optimization/Theorem58.lean` | The Chapter 5 quadratic/CG substrate, Lemma 5.1 Krylov equality, Theorem 5.3 finite-dimensional termination-facing wrappers, and Theorem 5.4 descent/halving/log-rate layers now compile. The latest CG endpoint is the displayed restarted-CG specialization `chewi54_log_rate_of_displayed_cg_blocks_blockSize`. The Theorem 5.8 AGF lane now defines the AGF trajectory/friction interface, auxiliary point, Lyapunov function, Lyapunov-zero identity, Lyapunov-monotone-to-rate theorem, derivative-nonpositive-to-rate theorem, and first-order-convexity wrapper `chewi58_gap_le_of_lyapunov_derivative_formula_firstOrderConvex`. Search-first result: reused mathlib continuous linear maps, inner-product continuity, submodule/span APIs, finite-dimensional orthogonal-family APIs, scalar inverse/log APIs, local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers, Chapter 2 `antitoneOn_of_hasDerivWithinAt_nonpos`, Chapter 3 descent/telescoping helpers, and Chapter 4 scalar recurrence/log bridges. Next target: prove the source AGF Lyapunov derivative formula from `IsChewi58AcceleratedGradientFlowTrajectory`, `HasGradientAt`, and continuity/differentiability side conditions; then expose the exact Theorem 5.8 statement and move to Theorem 5.9. |
