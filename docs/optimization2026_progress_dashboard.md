...
- Current manual frontier after the 2026-05-05 Chapter 6 PSD packet:
  ...
  `chewi614_average_gap_bound`.  The active target is to bridge the source
  `L`-Lipschitz hypothesis to the supplied subgradient norm bound and add the
  displayed `h = R / sqrt N` corollary before moving to functional constraints
  in Theorem 6.16.
...
Latest proof target: Chapter 6 nonsmooth convex optimization.  The first
`ProjectedSubgradient.lean` packet now proves the supplied-interface Theorem
6.14 average-gap bound, including source-shaped projection characterization,
projection non-expansiveness, PSD trajectory, Jensen averaging, one-step
squared-distance recurrence, and finite telescoping.  The next packet should
bridge Chewi's `L`-Lipschitz assumption to the current supplied subgradient
norm bound, then add the displayed `h = R / sqrt N` rate.  The CG modules
