...
- Current manual frontier after rebasing local `main` onto `origin/main` at
  ...
  certificates.  The next target is no longer "finish 6.21/6.22 side
  conditions": only add an arbitrary `d > N` embedding/report wrapper if exact
  theorem reporting requires it, otherwise move aggressively to Theorem 6.23
  and Definition 6.24/Theorem 6.25 feasibility.  The exact Grünbaum/centroid
...
Current active target after the verified local 6.22 lower-bound packet on top
of the fetched `origin/main` at `c8916aa`: Chapter 6 lower-bound/feasibility is
...
source strong-convex/Lipschitz certificates.  The next theorem packet should
either add a literal arbitrary-`d > N` embedding/report wrapper only if needed
for exact theorem reporting, or move directly to Theorem 6.23 and Definition
6.24/Theorem 6.25 feasibility.
