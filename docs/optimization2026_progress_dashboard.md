...
- Current manual frontier after local sync to `origin/main` at `6c7c6cd`
  ...
  current `Σ⁻¹` identification, displayed center-update declarations, and
  `chewi620_matrix_rankOne_collapse` compile
  in `StatInference/Optimization/Ellipsoid.lean`.  A
  focused `lake env lean StatInference/Optimization/Ellipsoid.lean` check
  passed after the `6c7c6cd` sync and rank-one collapse update, while the exact
