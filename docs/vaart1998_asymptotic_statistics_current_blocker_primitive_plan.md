...
This endpoint replaces the vector-valued score `L²` hypothesis by
coordinatewise finite-dimensional `L²` assumptions and derives the vector
moment internally with `MemLp.of_eval`.
...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the coordinate score moment endpoint to a concrete
...
2. Derivative or score source: only add a model-specific coordinate/matrix or
   ...
   wrappers, or coordinate score moment wrappers.
