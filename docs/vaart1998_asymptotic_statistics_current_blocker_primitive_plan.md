...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the pushforward transform-law endpoint to a concrete
...
The latest theorem-sized packet specializes `scoreLaw` and `derivativeLaw` to
the pushforward laws of the measurable score and derivative transforms under
`observationLaw`, removing the direct transform-law assumptions.
...
pushforward transform-law endpoint to a concrete model-specialized Theorem
5.41 instantiation, or instantiate one remaining smoothness/root/envelope
field.
