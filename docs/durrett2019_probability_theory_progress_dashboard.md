...
Route from `Live In-Thread Goal Prompt V42` in
...
Theorem 4.3.2, Doob's decomposition for submartingales.
...
oscillation, and exact `EReal` liminf/limsup wrappers.
...
Next proof packet: start Durrett Theorem 4.3.2 by packaging mathlib's
Doob-decomposition `predictablePart`/`martingalePart` API into the source
statement for a submartingale: martingale part plus predictable increasing
part, with `A_0 = 0` and the finite-sum formula if immediate.
