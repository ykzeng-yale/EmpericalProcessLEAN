...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the observation-transform endpoint to a concrete model-specialized
   Theorem 5.41 instantiation, or package only a still-live source field that
   the current endpoint explicitly exposes.
...
(`Add Vaart observation-transform iid source endpoint`).

The latest theorem-sized packet removes the direct score-vector and
derivative-table measurability, marginal-law, and independence fields by
deriving them from a common i.i.d. observation process and measurable
score/derivative transforms.
...
observation-transform endpoint to a concrete model-specialized Theorem 5.41
instantiation, derive the common observation law/independence from a sequence
source, or instantiate one remaining smoothness/root/envelope field.
