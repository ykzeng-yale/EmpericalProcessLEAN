...
Route from `Live In-Thread Goal Prompt V48` in
...
top-set separation.  The next target is an actual Durrett proof obligation:
identify the bounded martingale limits `Y` and `Z` with RN derivatives, prove
`X = Y/Z`, or prove the `{X = infinity}` singular separation consumed by that
assembly.
...
density-ratio/top-set source assembly.  Aim at one actual source-identification
obligation: `Y = dmu/drho`, `Z = dnu/drho`, `X = Y/Z`, or the
`{X = infinity}` singular separation.  Search bounded convergence,
pi-lambda/generator extension, a.e. transfer, ENNReal limsup/top, and
