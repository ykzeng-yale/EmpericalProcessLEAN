...
The external SLT metric-entropy naming pattern has now been ported into the
local covering API as `vdVWLogEntropyOfNat`, `vdVWCoveringLogEntropy`,
`empiricalL1CoveringLogEntropy`, and `vdVWSemimetricLogEntropy`, with
finite-cardinality display and monotonicity lemmas over current mathlib
covering numbers.
...
The same source path now accepts mathlib empirical-wrapper packing-number
lower bounds directly via
`finiteEmpiricalL1CoveringNumberCard_ge_of_empiricalL1Index_packingNumber`,
`empiricalL1CoveringNumber_ge_of_empiricalL1Index_packingNumber`,
...
with terminal-sample source variants
...
The bridge uses mathlib's finite maximal separated set to turn
`Metric.packingNumber` on `EmpiricalL1Index.liftSet` into the proof-carrying
...
manually assembling selected-cardinality lower growth.  Its terminal-sample
companion
...
accepts the same covering-number lower bound on the realized `samplePath`
...
They now also feed the theorem-facing conclusion bundle through
...
2026-05-12 Chapter 1.7 separable Borel ball-sigma export:
`BallSigma.lean` now exports
`vdVWMeasurableSpace_eq_openBallMeasurableSpace_of_separable` and
`vdVWMeasurableSpace_eq_closedBallMeasurableSpace_of_separable`, making the
ambient Borel measurable-space equality reusable under the separable
pseudometric hypotheses used in the textbook distance-coordinate criterion.
...
| Lemma 1.7.1 | Open-ball and closed-ball sigma-field wrappers, open-ball topological basis, rational open/closed ball bridges, open-ball/closed-ball sigma equality, Borel equality, separable ambient measurable-space equality for open/closed ball sigma fields, generator measurability, separable dense-sequence distance-coordinate measurability iff, and bounded distance-coordinate measurability iff in `BallSigma.lean` | Full arbitrary-map/asymptotic-measurability clauses remain pending. |
...
| Definition 2.2.3 | Semimetric whole-space covering/packing wrappers `vdVWSemimetricCoveringNumber` and `vdVWSemimetricPackingNumber`, semimetric log-entropy wrapper `vdVWSemimetricLogEntropy`, finite-cover handoff, `N <= D <= N(epsilon/2)` comparison layer, proof-carrying empirical packing witnesses, empirical-wrapper `Metric.packingNumber` lower-bound handoff to selected covering cardinalities, empirical log-entropy wrapper `empiricalL1CoveringLogEntropy`, and the final set-countable Theorem 2.4.3 consumer for empirical-wrapper packing-number lower bounds | Square-root entropy, entropy integrals/Dudley chaining, and exact open-ball convention remain pending. |
