...
| Definition 2.2.3 | Semimetric whole-space covering/packing wrappers `vdVWSemimetricCoveringNumber` and `vdVWSemimetricPackingNumber`, finite-cover handoff, `N <= D <= N(epsilon/2)` comparison layer, proof-carrying empirical packing witnesses, empirical-wrapper `Metric.packingNumber` lower-bound handoff to selected covering cardinalities, and the final set-countable Theorem 2.4.3 consumer for empirical-wrapper packing-number lower bounds | Entropy/logarithm wrappers and exact open-ball convention remain pending. |
