...
Route from `Live In-Thread Goal Prompt V58` in
...
and the denominator top-set layer now proves
`nu {canonicalRatio = infinity} = 0` automatically from the generated
`withDensity` representation of the denominator density.  The next target is
the remaining source-production obligation: prove
`mu.singularPart nu {canonicalRatio = infinity}^c = 0`.
