...
support invariant for gradient-span trajectories.  The next aggressive step is
the actual strongly-convex gap/rate lower-bound assembly.
...
Next high-value task: prove a source-shaped strongly-convex gap/rate
lower-bound wrapper for Theorem 4.5, using either the abstract Lemma 4.2
transfer package or the concrete `Theorem45.lean` regularized chain setup.
Search mathlib/local APIs for logarithm/geometric decay/asymptotic
iteration-count wrappers before adding any local complexity primitive.  Keep
any exercise work opportunistic and centralized in
