...
Route from `Live In-Thread Goal Prompt V126` in
`docs/durrett2019_probability_theory_current_blocker_primitive_plan.md`.
The active theorem lane is now Chapter 4.4 martingale maximal inequalities.  Treat compiled Chapter
...
The next target is to construct the canonical infinite running absolute maximum
`S` and prove the supplied convergence/measurability hypotheses consumed by
this assembly.
...
supplied a.s. running-maximum limit.  Aim at one forward textbook packet:
construct the canonical infinite running maximum `S` and prove the supplied
