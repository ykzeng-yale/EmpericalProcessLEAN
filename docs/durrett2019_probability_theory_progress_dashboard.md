...
Route from `Live In-Thread Goal Prompt V28` in
...
truncation wrappers now compile.  The next frontier is Theorem 4.2.8
predictable transforms of supermartingales.
...
Next proof packet: package Theorem 4.2.8 predictable transforms for
supermartingales.  Search for existing predictable/stochastic-transform APIs
first; otherwise add a finite-sum source wrapper with explicit
predictability, bounded/integrable product, nonnegativity, and pull-out
conditional-expectation hypotheses, then derive the supermartingale
constructor and direct submartingale/martingale variants if cheap.
