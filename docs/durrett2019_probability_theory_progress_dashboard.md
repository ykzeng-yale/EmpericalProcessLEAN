...
Route from `Live In-Thread Goal Prompt V113` in
...
compiles.  The next target is the exact absolute-max/variance display for
Example 4.4.3 if cheap, otherwise Theorem 4.4.4 Lp maximal inequality.
...
Aim at one forward textbook packet: exact absolute-max/variance Kolmogorov
display if cheap, otherwise Theorem 4.4.4 Lp maximal inequality.
