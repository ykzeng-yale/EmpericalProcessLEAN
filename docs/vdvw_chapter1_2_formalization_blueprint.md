...
3. Promote Chapter 2 primitive infrastructure: covering/packing semimetric
   ...
   handoff, and random empirical-cover product random-sign handoff are now
   compiled.  The next frontier is proving the product-a.e. finite-center
   Hoeffding predicate for the selected random empirical covers, or replacing
   that handoff with the existing expectation-level maximal API, then
