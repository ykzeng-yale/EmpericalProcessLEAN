...
Route from `Live In-Thread Goal Prompt V55` in
...
`toReal` sequences converge to their filtration limit processes.  The next
target is an actual source-production obligation: convert those limit processes
into finite nonnegative `ENNReal` density candidates and feed the existing
`mu + nu` endpoint, or prove `X = Y/Z` / `{X = infinity}` singular separation
if that is the shorter current bridge.
