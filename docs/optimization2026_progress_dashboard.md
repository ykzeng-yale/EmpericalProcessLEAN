...
expected-lower-model handoff wrappers: treat Chapters 3-8, Chapter 9/10
...
Next theorem-sized packet: prove the analytic/probabilistic smooth variance
and non-smooth bounded-gradient expected lower estimates for `E psi_x(x+)`,
then instantiate the compiled expected-lower-model wrappers.  Keep the concrete
finite Sinkhorn KL identity layer as the next Chapter 11.8 blocker, but do not
let it stall Chapter 12 coverage.
...
SMPGD theorem existed; the new scalar/source-rate/model-bound/lower-model
handoff packets reuse local Gronwall/geometric-weight infrastructure and leave
the probabilistic expected lower estimates as the active formal blocker.
