...
`observationSecondDerivative`. The wrapper instantiates the old sample-indexed
first derivative as
...
and derives the fixed-`theta0` derivative-transform field, first Taylor fderiv
identity, derivative `ContDiffOn` field, and second-derivative Taylor identity
from observation-level assumptions. It replaces the second-derivative joint
measurability field and the abstract eventual envelope-domination field by
...
measurability/envelope route. It already removes the separate
`secondDerivative` kernel, but still keeps a sample-indexed `derivativeAt`
kernel and asks for its full joint measurability. The current endpoint removes
that first-derivative kernel by definitional instantiation from
