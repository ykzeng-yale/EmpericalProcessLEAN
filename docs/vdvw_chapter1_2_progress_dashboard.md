...
NEXT       Use the new terminal `base^n` log-cardinality arithmetic handoffs to prove the remaining geometric selected fixed-radius entropy bound.  Search found mathlib internal `Metric.coveringNumber`/`minimalCover`/`maximalSeparatedSet`, but no adapter yet from those internal metric covers to `FiniteEmpiricalL1CoverAtCard` for the empirical L1 pseudometric; build that adapter or record the exact theorem-level UI/tail side condition honestly.
