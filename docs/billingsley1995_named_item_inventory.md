...
| Sections 15-16 integration and limit theorems | `_122-242.md:3209,3321,3394,3565,3575` | local-wrapper | `StatInference/ProbabilityMeasure/Tail.lean` wraps mathlib layer-cake, tail-integral monotonicity, split-at-radius, Markov, and dominated-convergence upper-tail cutoff APIs for VdV&W Theorem 2.4.3 tail and truncation support; `Theorem243.lean` now consumes this for measurable-integrable lintegral and outer-expectation envelope-tail convergence. This is a content-based support layer, not a source-exact Billingsley Sections 15-16 theorem report; exact item selection, source screenshots, and report remain pending. |
| Section 18 product measure/Fubini and Section 20 product-law independence | `_243-363.md:61,67,73,111,115,119,173,185,1054,1102` | local-wrapper | `StatInference/ProbabilityMeasure/ProductMeasure.lean` wraps mathlib product probability measures, Tonelli/Fubini, product-coordinate marginal projection, separated product-expectation identities, product-copy mean-zero differences, binary self-copy laws/independence, mapped-coordinate joint-law independence, and finite-`Pi` mapped-coordinate laws/independence for symmetrization. This is a content-based support layer, not a source-exact Billingsley Section 18/20 theorem report. |
| Section 20 random variables/distributions | `_243-363.md` near Section 20 start | local-wrapper candidate | distribution and convergence-in-probability wrappers; fixed-endpoint empirical-CDF `TendstoInMeasure` support lives with the Theorem 20.6 source-crosswalk row |
| Theorem 20.6 empirical CDF Glivenko-Cantelli | `_243-363.md:1310,1312,1316,1320,1322,1330,1334,1342,1346` | local-wrapper/source-crosswalk | `StatInference/EmpiricalProcess/RealHalfLineGC.lean` contains pointwise CDF/empirical-average wrappers connecting Billingsley's empirical distribution function formula `(20.42)` for a fixed closed half-line to the local `realHalfLineIndicator` class, plus fixed-endpoint convergence-in-probability/`TendstoInMeasure` and outer-probability handoffs. This is fixed-endpoint support only, not a source-exact formalization of Billingsley Theorem 20.6; the uniform-in-`x` theorem remains pending. |
