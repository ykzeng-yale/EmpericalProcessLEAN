...
Route from `Live In-Thread Goal Prompt V75` in
...
Hellinger L1 bridge also now compile: pairwise L1 tail control plus pointwise
convergence supplies the L1 input, eventual Hellinger tail bounds tending to
zero feed the pairwise-liminf hypothesis, and square-root square-integral
estimates feed those tail bounds.  The next target is the concrete cylinder
