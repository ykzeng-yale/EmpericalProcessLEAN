...
Current frontier: Theorem 5.41 now has a compiled fixed true-parameter source
endpoint
...
`measurable_const`.  Its immediate predecessor is the derivative-transform
...
Its immediate predecessor is the textbook finite-sum root source endpoint
...
finite-sum estimating equation and derives the average root internally.  Its
immediate predecessor is the full-domain smoothness source endpoint
...
The next proof packet should continue from this endpoint to a concrete
model-specialized Theorem 5.41 instantiation, or package only a still-live
source field that the current endpoint explicitly exposes, not replay display
weak-convergence,
