...
Chapter 12 expected-model one-step bridge: treat Chapters 3-8, Chapter 9/10
...
Next theorem-sized packet: prove the smooth variance and non-smooth
bounded-gradient model lower bounds for `E psi_x(x+)`, then feed them into the
compiled expected-model bridge.  Keep the concrete finite Sinkhorn KL identity
layer as the next Chapter 11.8 blocker, but do not let it stall Chapter 12
coverage.
...
SMPGD theorem existed; the new scalar/source-rate/model-bound packet reuses
local Gronwall/geometric-weight infrastructure and leaves the smooth/non-smooth
model lower-bound estimates as the active formal blocker.
