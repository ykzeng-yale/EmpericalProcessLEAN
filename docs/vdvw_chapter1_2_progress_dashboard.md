...
NEXT       Prove the deterministic normalized log-cardinality bound needed by the selected fixed-radius route, likely from a finite value-grid cover for bounded truncated functions on each empirical sample, or state the exact additional theorem-level UI/tail condition honestly.
