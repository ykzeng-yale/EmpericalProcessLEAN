...
or compactness facts; the compact bridge reuses Mathlib
`IsCompact.bddAbove_image` and coordinate continuity from
`PiLp.continuous_apply`.  The remaining scalar comparison is against the
coordinate `sSup`.
