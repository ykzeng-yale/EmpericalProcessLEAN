...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after local sync to `origin/main` at `4e43e7d`
  (`Add Chewi ellipsoid PosDef cancellation bridge`) plus the current verified
  pullback-certificate packet in `StatInference/Optimization/Ellipsoid.lean`:
  ...
  invertibility/cancellation bridge, and the pullback-standard-cut certificate
  declarations compile in `StatInference/Optimization/Ellipsoid.lean`.  A
  focused `lake env lean StatInference/Optimization/Ellipsoid.lean` check
  passed after the `4e43e7d` sync, while the exact
...
| Chapter 6 nonsmooth convex optimization | local-layer | `StatInference/Optimization/ProjectedSubgradient.lean`, `StatInference/Optimization/CuttingPlane.lean`, `StatInference/Optimization/Ellipsoid.lean` | The Chapter 6 PSD layer now compiles through finite-valued Theorems 6.14 and 6.16 in source-facing supplied-interface form. It defines `IsSubgradientAt`, `ProjectionCharacterizationOn`, `ProjectionOracleOn`, `normalizedSubgradient`, `projectedSubgradientStep`, `projectedSubgradientScaledStep`, `IsProjectedSubgradientTrajectory`, `iterateAverage`, `FunctionalConstraintSuccess`, `IsFunctionalConstraintPSDStep`, and `IsFunctionalConstraintPSDTrajectory`; proves `IsSubgradientAt.gap_le_inner`, `ProjectionCharacterizationOn.dist_to_set_le`, `ProjectionCharacterizationOn.fixed`, `ProjectionCharacterizationOn.nonexpansive`, `ProjectionCharacterizationOn.toProjectionOracleOn`, `normalizedSubgradient_norm`, `normalizedSubgradient_inner_self`, `IsSubgradientAt.norm_le_of_lipschitzOnWith_feasible_ray`, Jensen wrappers for averaged iterates, the scaled-step recurrence `projectedSubgradient_scaled_sqdist_recurrence`, the PSD squared-distance recurrence `projectedSubgradient_sqdist_recurrence`, the finite telescope `projectedSubgradient_gap_average_bound_of_recurrence`, Theorem 6.14 wrappers, the Theorem 6.16 objective/constraint case decreases, `functionalConstraintPSD_step_sqdist_decrease_of_not_success`, `strict_decrease_contradiction_of_le_mul`, and `chewi616_exists_functionalConstraintSuccess`. `CuttingPlane.lean` proves the supplied-interface CoGM Theorem 6.19 spine through `chewi619_gap_le_display_rate_of_scaled_candidates`. `Ellipsoid.lean` starts Lemma 6.20 with `ellipsoidSet`, `ellipsoidCutHalfspace`, `ellipsoidCenterUpdate`, `ellipsoidVolumeRatio`, `ellipsoidVolumeRatio_nonneg`, `IsEllipsoidStepCertificate`, `ellipsoidSets`, `IsEllipsoidCuttingPlaneTrajectory`, `IsEllipsoidCuttingPlaneTrajectory.hasVolumeShrink`, `IsEllipsoidCuttingPlaneTrajectory.halfspace_subset`, `ellipsoidTrajectory_volume_ratio_le_pow`, `chewi620_volume_ratio_and_gap_bound_of_scaled_candidates`, `chewi620_standard_cut_scalar_containment_cleared`, `chewi620_standard_cut_scalar_containment`, `chewi620_ellipsoidVolumeRatio_source_nonneg`, `chewi620_standardCut_detRatio_eq_source`, `chewi620_ellipsoidVolumeRatio_sq_eq_standardCut_detRatio`, `chewi620StandardCutCenter`, `chewi620StandardCutInvShape`, `chewi620_norm_sq_eq_inner_sq_add_orthogonal_sq`, `chewi620_standardCutInvShape_quadratic`, `chewi620_standardCut_halfspace_subset`, `chewi620_affineTransport_halfspace_subset_of_quadratic`, `chewi620_affineTransport_stepCertificate_of_quadratic`, `matrixInvShape`, `matrixInvShape_quadratic_eq_dotProduct`, `matrixInvShape_quadratic_nonneg_of_posSemidef`, `matrixInvShape_quadratic_pos_of_posDef`, `chewi620_matrix_cut_sqrt_inv_pos_of_posDef`, `chewi620MatrixCutScale`, `chewi620MatrixNormalizedCutDirection`, `chewi620_matrixNormalizedCutDirection_norm_of_posDef`, `chewi620_matrixNormalizedCutDirection_inner_toStd`, `chewi620_rawAdjointIdentity_of_symmetric_inverse`, `chewi620_matrixSqrt_normalizedCutDirection_norm_of_posDef`, `chewi620_matrixSqrt_normalizedCutDirection_inner_toStd`, `chewi620PullbackIdentityInvShape`, `chewi620PullbackStandardCutInvShape`, `chewi620_pullbackIdentityInvShape_quadratic`, `chewi620_pullbackStandardCutInvShape_quadratic`, `chewi620_pullbackStandardCutInvShape_hnext`, and `chewi620_sqrtAffineTransport_stepCertificate_of_pullback`. Search-first result: reused mathlib `ConvexOn.map_sum_le`, `ConvexOn` segment fields, `LipschitzOnWith.le_add_mul`, `Real.rpow_nonneg`, local finite-recurrence patterns, `Real.sq_sqrt`, `pow_succ`, `Nat` subtraction, inner-product Pythagoras/orthogonal-decomposition APIs, `Matrix.toEuclideanLin`, `EuclideanSpace.inner_eq_star_dotProduct`, PosDef/PosSemidef dot-product APIs, `Real.sqrt` normalization, `LinearMap.IsSymmetric`, `LinearMap.IsSymmetric.toLinearMap_symm`, `LinearEquiv.isSymmetric_symm_iff`, and scalar `field_simp`/`nlinarith`; no direct mathlib/local Grünbaum-centroid or ellipsoid-method theorem was found. Lemma 6.20 matrix search found relevant mathlib matrix/PSD/det/inv and volume-scaling APIs including `Matrix.toEuclideanLin`, `EuclideanSpace.inner_eq_star_dotProduct`, `Matrix.posSemidef_iff_dotProduct_mulVec`, `Matrix.PosDef.dotProduct_mulVec_pos`, `Matrix.PosDef.isUnit`, `Matrix.PosDef.inv`, `Matrix.mul_nonsing_inv`, `Matrix.det_nonsing_inv`, `Matrix.det_add_mul`, rank-one determinant lemmas in `Matrix.SchurComplement`, and `Real.map_matrix_volume_pi_eq_smul_volume_pi` / `Real.map_linearMap_volume_pi_eq_smul_volume_pi`; the normalized central-cut containment, determinant-ratio, coordinate-free half-space, abstract affine-transport certificate, current matrix quadratic, positive-denominator, normalized-cut algebra, raw symmetric square-root cut bridge, PosDef cancellation bridge, and pullback-standard-cut certificate are now local. Current aggressive packet: prove equality with Chewi's displayed matrix update and the rank-one determinant/volume identities required by `chewi620_sqrtAffineTransport_stepCertificate_of_pullback`. |
...
matrix normalization, PosDef cancellation, and pullback-certificate packets:
Chapter 6 nonsmooth
...
invertibility/cancellation packet, and the pullback-standard-cut certificate
`chewi620_sqrtAffineTransport_stepCertificate_of_pullback`.  The next manual
goal packet should identify these pullback objects with Chewi's displayed
`Σ_n^{-1}` and `Σ_{n+1}^{-1}` matrix updates, then prove the rank-one
determinant/volume bridge using mathlib matrix/PSD/determinant/inverse/volume
APIs.  If the full matrix proof balloons, record the exact missing matrix API
and prove the smallest affine transport, matrix-coordinate, rank-one determinant,
or volume-scaling certificate that removes it.
