...
Next high-value task: build the final strongly-convex lower-bound rate
statement past the solved reduction obstruction.  Search mathlib/local APIs for
finite geometric sums, recurrence solutions, logarithm/order wrappers, and
asymptotic iteration-count wrappers before adding any local complexity
primitive.  The two viable routes are: convert the compiled obstruction
`beta / (16 * eps) - 1 <= N` plus condition-number bookkeeping into the source
`sqrt(kappa) * log(alpha R^2 / eps)` lower-bound shape, or close the direct
Exercise 4.2 geometric minimizer/tail lower bound for
`strongLowerBoundChainObjective`.  Keep any exercise work opportunistic and centralized in
