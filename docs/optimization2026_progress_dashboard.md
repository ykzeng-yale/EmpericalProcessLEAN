...
Next high-value task: prove or specialize the remaining comparison
`c * sqrt(kappa) * log(ratio) <= beta / (16 * eps) - 1` from concrete
condition-number/log hypotheses, or close the direct Exercise 4.2 geometric
minimizer/tail lower bound for `strongLowerBoundChainObjective`.  Search
