...
Route from `Live In-Thread Goal Prompt V22` in
...
The Example 4.2.2 quadratic martingale source bridge now compiles; the next
frontier is its natural-random-walk instantiation or Example 4.2.3 if the square
side conditions are not direct.
...
Example 4.2.1 linear random-walk and centered-display wrappers.
...
Next proof packet: instantiate Example 4.2.2 for the natural random walk if the
conditional-square and integrability side conditions are direct; otherwise move
to Example 4.2.3 exponential martingales.
