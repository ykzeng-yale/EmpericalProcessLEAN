...
This endpoint specializes the Theorem 5.41 route to the textbook case where the
true parameter `theta0` is fixed, removes the random-`theta0` measurability
source, and discharges it internally by `measurable_const`.
...
1. Move one layer closer to a model-facing finite-dimensional statement:
   continue from the fixed-`theta0` endpoint toward a concrete
