...
representative `Z`, assumes only coordinate covariance equality between
`Q.map Z` and `observationLaw.map scoreTransform`, then upgrades that equality
to the all-dual covariance-bilinear comparison using
...
covariance-bilinear assumptions, or full `Z` measurability, unless a source
