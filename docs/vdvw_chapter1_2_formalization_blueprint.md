...
support is the scalar-quantizer coordinate-cardinality natural-polynomial
route through
...
The same route now has a natural-polynomial selected-package and centered
untruncated convergence closure:
`VdVWTheorem243SelectedFixedRadiusTailSideConditions.of_coordinate_scalarQuantizer_decode_error_coordinateCard_bound_nat_poly`,
`VdVWTheorem243SelectedFixedRadiusTailSideConditions.forall_pos_of_coordinate_scalarQuantizer_decode_error_coordinateCard_bound_nat_poly`,
and
`VdVWTheorem243_centered_untruncated_convergesInOuterProbabilityConst_zero_of_forall_pos_coordinate_scalarQuantizer_decode_error_coordinateCard_bound_nat_poly`.
...
`WeakConvergence.lean` now proves the textbook construction that a pre-tight
probability measure concentrates on a countable union of measurable totally
bounded sets as
`VdVWProbabilityMeasurePreTight.exists_iUnion_totallyBounded_measure_compl_eq_zero`.
In countably generated uniform spaces this yields
`VdVWProbabilityMeasurePreTight.separable` and the numbered wrapper
`vdVW132_probabilityMeasure_separable_of_preTight`.  The separable-to-pre-tight
direction and the full complete-space equivalence remain future work.
...
the family-level construction that a tight probability measure is carried by a
countable union of compact sets.  The numbered wrapper
`vdVW132_probabilityMeasure_exists_iUnion_isCompact_measure_compl_eq_zero_of_tight`
records the singleton version, and `VdVWProbabilityMeasureSeparable.of_tight`
with `vdVW132_probabilityMeasure_separable_of_tight` turn tightness into the
local separability predicate in countably generated Hausdorff Borel uniform
spaces.

2026-05-11 Lemma 1.3.2 separable finite-cover follow-up:
`WeakConvergence.lean` now proves the finite high-mass ball-prefix cover step
from measure separability.  The general measure-continuity helper
`VdVWProbabilityMeasure_exists_finitePrefix_measure_compl_le_of_iUnion_measure_compl_eq_zero`
turns a full-mass countable cover into a finite-prefix cover up to any positive
tolerance.  Applying this to the dense sequence from
`VdVWProbabilityMeasureSeparable` gives
`VdVWProbabilityMeasureSeparable.exists_finitePrefix_ball_cover_measure_compl_le`
...
`WeakConvergence.lean` now proves the scale-intersection construction from the
finite ball-prefix covers.  The bridge
`VdVWProbabilityMeasureSeparable.exists_iInter_finitePrefix_ball_cover_measure_compl_le`
chooses geometric ENNReal tolerances and reciprocal-radius finite covers, then
intersects them to obtain a measurable totally bounded set whose complement is
bounded by the target tolerance.  This yields
`VdVWProbabilityMeasureSeparable.preTight` and the numbered wrapper
`vdVW132_probabilityMeasure_preTight_of_separable` for Borel pseudometric
spaces, plus `vdVW132_probabilityMeasure_preTight_iff_separable` under the
combined countably generated Borel pseudometric hypotheses.  The
pre-tight/separable measure-level equivalence is now compiled in both
directions under the current local hypotheses; the full complete-space
equivalence with tightness and Polish-measure clauses remain pending.
...
| 1.3.2 | Lemma | `..._1-100.md:582` | local-layer/mathlib-foundation: measure-level separability and pre-tightness predicates introduced as `VdVWProbabilityMeasureSeparable` and `VdVWProbabilityMeasurePreTight`; pre-tight countable-union support and pre-tight-to-separable direction compiled as `VdVWProbabilityMeasurePreTight.exists_iUnion_totallyBounded_measure_compl_eq_zero`, `VdVWProbabilityMeasurePreTight.separable`, and `vdVW132_probabilityMeasure_separable_of_preTight`; separable finite high-mass ball-prefix cover step compiled as `VdVWProbabilityMeasureSeparable.exists_finitePrefix_ball_cover_measure_compl_le` and `vdVW132_probabilityMeasure_exists_finitePrefix_ball_cover_measure_compl_le_of_separable`; separable-to-pre-tight scale-intersection bridge compiled as `VdVWProbabilityMeasureSeparable.exists_iInter_finitePrefix_ball_cover_measure_compl_le`, `VdVWProbabilityMeasureSeparable.preTight`, `vdVW132_probabilityMeasure_preTight_of_separable`, and `vdVW132_probabilityMeasure_preTight_iff_separable`; tight σ-compact support and tight-to-separable direction compiled as `VdVWProbabilityMeasuresTight.exists_iUnion_isCompact_measure_compl_eq_zero`, `vdVW132_probabilityMeasure_exists_iUnion_isCompact_measure_compl_eq_zero_of_tight`, `VdVWProbabilityMeasureSeparable.of_tight`, and `vdVW132_probabilityMeasure_separable_of_tight`; separable-space, tight-to-pre-tight, and complete separable metric-type components compiled as `VdVWProbabilityMeasureSeparable.of_separableSpace`, `VdVWProbabilityMeasurePreTight.of_tight`, `vdVW132_complete_separable_probabilityMeasure_tight`, `vdVW132_complete_separable_probabilityMeasure_separable_and_tight`, and `vdVW132_complete_separable_probabilityMeasure_preTight`; full complete-space equivalence and Polish-measure clauses still pending |
...
| 1.4.8 | Theorem | `..._1-100.md:910` | local-layer/mathlib-foundation: FDD forward direction now wrapped for weak convergence, including the VdV&W-named `vdVW148_finiteDimensional_weakConvergence_of_processLaw_weakConvergence`; finite-index `ell_infty(T)` weak-convergence/tightness criteria are packaged as `vdVW148_ellInfty_weakConvergence_iff_finiteProduct_weakConvergence_finite` and `vdVW148_ellInfty_asymptoticallyTight_iff_finiteProduct_asymptoticallyTight_finite`; process-law and `IdentDistrib` uniqueness-only FDD wrappers are compiled in `FiniteDimensional.lean`; raw bounded-process forward FDD, coordinate-law, and process-tightness consequences are compiled as `VdVWEllInftyProcessWeakConvergence.finiteDimensionalLaw`, `.finiteDimensionalLaw_congr_eventually_finite_coord_ae`, `.coordinateLaw`, `.rawCoordinateLaw`, `.rawCoordinateLaw_congr_eventually_ae`, `.asymptoticallyTight_atTop`, `.finiteDimensionalLaw_asymptoticallyTight_atTop`, `.rawCoordinateLaw_asymptoticallyTight_atTop`, `.finiteDimensionalLaw_congr_eventually_finite_coord_ae_asymptoticallyTight_atTop`, `.rawCoordinateLaw_congr_eventually_ae_asymptoticallyTight_atTop`, `VdVWEllInftyProcessAsymptoticallyTight.finiteDimensionalLaw`, `.finiteDimensionalLaw_congr_eventually_finite_coord_ae`, `.coordinateLaw`, `.rawCoordinateLaw`, and `.rawCoordinateLaw_congr_eventually_ae`; ordinary coordinate-law adapter `vdVWCoordinateProcessLaw`, `vdVWEllInftyProcessLaw_map_eval`, coordinatewise a.e.-replacement lemmas `aemeasurable_coordinate_congr_ae`/`vdVWCoordinateProcessLaw_congr_ae`, and fixed-FDD selected-coordinate replacement consumers are available; projective-limit/FDD law equality APIs exist, but no exact weak-convergence iff-over-FDD converse theorem found |
...
3. Promote Chapter 2 primitive infrastructure: covering/packing semimetric
   ...
   The selected-witness fiber now feeds the centered pair-difference bad event
   through
   ...
   with the latter discharging pair-difference boundedness from the usual
   ...
   The sign-vector support component is now measurable via
   `measurableSet_vdVWRademacherSignVector`, and
   `measurableSet_VdVWTheorem243PairDifferenceGhostRademacherSelectedNetEvent_of_components`
   reduces concrete-event measurability to the original finite-center side
   condition, the ghost finite-center side condition with negated signs, and
   the signed pair-difference bad event.  The missing bridge is now those three
   ...
   `epsilon < dist (4 * finiteNetHoeffdingUpper + eta) 0`.  The additive-error
   fixed-`M` convergence consumer is compiled through
   ...
   which turn nonnegative measurable ordinary mean convergence into VdV&W
...
selected-cardinality lintegral condition.

The selected first-level and untruncated lower-growth wrappers for this a.e.
surface are now compiled:
...
The next blueprint item is therefore not another wrapper: prove the actual
