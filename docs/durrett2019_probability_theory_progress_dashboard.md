...
Route from `Live In-Thread Goal Prompt V99` in
...
coordinate-sigma-field inclusion.  The next target is the exact trimmed-measure
finite-prefix RN-ratio identification over this canonical prefix filtration,
denominator-limit nonzeroness, and the remaining nonzero-lower-integral/product
inputs.
...
canonical-ratio, quotient-limit, Hellinger, tail-zero-set, and prefix-filtration
support.  Aim at one actual source-production obligation: prove the
trimmed-measure finite-prefix RN-ratio identity over
`durrett2019_theorem_4_3_8_prefixFiltration`, or discharge the denominator-limit
nonzero / nonzero-lower-integral inputs consumed by the compiled branch
assemblers.  Search `Measure.trim`, `withDensity`, `map`, `comap`, finite
restriction maps, RN derivative quotient APIs, and the existing
`infiniteProduct_map_restrict_withDensity_eq` cylinder handoff.  Do not
re-prove the compiled Chapter 4.3.5 or 4.3.8 assembly layers.
