...
4. Keep the supplied-interface Theorem 3.4 assembly module compiling:
   ...
   corollary, positive-`alpha` closed geometric denominator corollary, and a
   wrapper deriving monotonicity from Lemma 3.1.  The first-order supplied
   lower-model interface supplies recurrence (3.1) for GD trajectories.
...
finite-denominator bounds, and the positive-`alpha` closed form of the source's
function-value bound.  The remaining Chapter 3 choices are the `alpha = 0`
limiting case, source-audited report packaging, or the full
