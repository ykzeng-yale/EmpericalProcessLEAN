...
Route from `Live In-Thread Goal Prompt V125` in
...
The next target is to construct/package the infinite running absolute maximum
domination object from Theorem 4.4.4's finite martingale maximal wrappers.
...
when a single `MemLp` dominating variable is supplied.  Aim at one forward
textbook packet: construct that domination object from the finite maximal
wrappers.
