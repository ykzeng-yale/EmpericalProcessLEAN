...
Route from `Live In-Thread Goal Prompt V47` in
...
a supplied singular-part restriction.  The next target is the singular-set
identification `{X = infinity}` and `Y/Z` rnDeriv-div bridge from Durrett's
...
martingale/convergence bridge and compiled regular/singular decomposition
identity.  Aim at the singular-set identification `{X = infinity}` and the
`Y/Z` rnDeriv-div bridge from Durrett's proof, using mathlib
`rnDeriv_eq_div`, `rnDeriv_eq_div_rnDeriv_add`, `singularPart_eq_restrict`,
`singularPart_eq_restrict'`, `Measure.restrict`, and ENNReal/toReal
set-integral APIs.  Do not re-prove the decomposition identity.
