...
Route from `Live In-Thread Goal Prompt V107` in
...
Hellinger product.  The next target is only source-shaped wrappers that remove
the remaining ambient dichotomy or coordinate finiteness assumptions when a
...
the closed `tprod` textbook branch wrapper.  Aim at one actual source-production
obligation: remove only the remaining ambient dichotomy / coordinate-finiteness
assumptions when a direct mathlib or local route is visible.  Search
`withDensity`, `MutuallySingular`, `AbsolutelyContinuous`,
`LebesgueDecomposition`, RN derivative a.e. finiteness, and existing
