...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the observation-sequence-law endpoint to a concrete
...
(`Add Vaart observation sequence-law source endpoint`).

The latest theorem-sized packet removes the direct observation marginal-law
and independence fields by deriving them from one infinite-product observation
sequence law, then feeding the observation-transform endpoint.
...
observation-sequence-law endpoint to a concrete model-specialized Theorem 5.41
