...
Route from `Live In-Thread Goal Prompt V127` in
...
The next target is to prove the a.s. boundedness hypothesis for finite running
absolute maxima from the uniform finite-maximal `eLpNorm` bounds.
...
assembly now also compile.  Aim at one forward textbook packet: prove the a.s.
