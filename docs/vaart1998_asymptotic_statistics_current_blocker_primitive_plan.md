...
1. Move one layer closer to a model-facing finite-dimensional statement:
   continue from the vector score-transform measurability and `L²`
   fixed-`theta0` endpoint toward a concrete
...
Latest verified Vaart frontier before the next packet: the vector
score-transform measurability and `L²` source endpoint for Vaart Theorem 5.41.
...
vector score-transform measurability and `L²` endpoint to a concrete
