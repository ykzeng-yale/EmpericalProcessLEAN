...
boundedness/UI, and `coverRadius -> 0` hypotheses.  The inverse-radius variant
...
now package the selected inverse-radius cover, diagonal log-cardinality
convergence, and measurable cardinality, while keeping the finite-net
upper-bound/UI input explicit. The remaining blocker is deriving or supplying
that finite-net boundedness/UI or a variable-domain tail-expectation
replacement, then final assembly.
