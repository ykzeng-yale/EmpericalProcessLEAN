...
- Current manual frontier after focused Lean verification of the local Chapter
  ...
  `origin/main` by other agents' empirical-process commits.  Latest pushed
  Optimization frontier is `7899c07` (`Start Chewi Fenchel and Bregman
  layer`).  The Chapter 9/10 substrate now builds in `Fenchel.lean` and
  `Bregman.lean`:
  ...
  Next aggressive theorem lane: `MirrorDescent.lean` for Theorem 10.9 MPGD/OMD
  telescoping, not more Chapter 8 setup.
...
| Chapters 9-11 deterministic algorithms | local-layer-started | `StatInference/Optimization/Fenchel.lean`, `StatInference/Optimization/Bregman.lean` | The first finite-valued Chapter 9/10 substrate builds through `StatInference`. `Fenchel.lean` introduces `fenchelConjugateTerm`, `fenchelConjugate`, `IsFenchelUpperBound`, `IsFenchelConjugateValue`, `fenchelConjugate_isFenchelConjugateValue`, Fenchel-Young, equality iff primal subgradient, equality iff dual subgradient under supplied biconjugate equality, `fenchelDoubleConjugate_le`, `StrongSubgradientMonotoneOn`, and `fenchelGradient_lipschitz_of_strongSubgradientMonotone` for Lemma 9.12's core estimate. `Bregman.lean` introduces `bregmanDivergence`, `RelativelyStrongConvexOn`, `RelativelySmoothOn`, relative lower/upper model wrappers for Propositions 10.5/10.6, and `RelativelyStrongConvexOn.growth_of_stationary` for Lemma 10.7. Next lane is `MirrorDescent.lean` for MPGD/OMD telescoping. |
