...
| Lemma 1.2.2, measurable-cover algebra | `Textbooks/Vaart1996/Markdown/Vaart 1996 Weak Convergence and Emperical Process_1-100.md:389-437` | nonnegative sup/add/inf/product cover algebra proved; two-sided constant addition equality proved as `VdVWMeasurableCover.addConstLeft` and `VdVWMeasurableCover.addConstRight`; nonnegative threshold-indicator clause proved as `VdVWMeasurableCover.thresholdIndicatorCover` and `VdVWOuterExpectation_thresholdIndicator_eq_measure_cover_threshold`; nonnegative measurable-left infimum equality proved as `VdVWMeasurableCover.infOfMeasurableLeft`; full signed extended-real statement remains pending |
