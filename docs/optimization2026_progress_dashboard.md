...
Next high-value task: formalize the nontrivial-start side-condition route for
Proposition 2.7(2).  The already-minimizer branch now compiles separately, so
the remaining analytic work is only for starts with `¬ IsMinOn f C x`:
gradient-flow convergence to a minimizer, positive objective gap on positive
time, and nonzero displacement on positive times.  The derivative-to-antitone,
PL scalar sign, objective-gap derivative, nonzero norm-derivative,
trajectory/gap continuity, Lyapunov-continuity, and minimizer-start
bookkeeping calculations are now compiled.
