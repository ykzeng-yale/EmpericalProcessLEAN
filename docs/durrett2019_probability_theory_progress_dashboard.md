...
Route from `Live In-Thread Goal Prompt V36` in
...
now compile; the next step is the first-below stopping-time instantiation and
bounded-increment lower bound `0 ≤ X_{n ∧ N} + K + M`.
...
limit wrapper, plus the first Theorem 4.3.1 stopped-shifted convergence and
survival-transfer support wrappers.
...
Next proof packet: continue Durrett Theorem 4.3.1 by instantiating the
first-below stopping time `N = inf {n : X_n ≤ -K}` and proving
`0 ≤ X_{n ∧ N} + K + M` from the bounded-increment hypothesis, so it feeds the
compiled stopped-shifted bridge.
