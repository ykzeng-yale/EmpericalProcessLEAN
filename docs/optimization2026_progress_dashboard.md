...
- Current manual frontier after focused Lean verification of the Chapter 11.5
  RAM scalar recurrence layer on 2026-05-06, building on pushed frontier
  `0e411d9` (`Add Chewi theorem 11.4 log burn-in layer`): Chapter 9/10 now has a compiled finite-valued
  ...
  `IsChewi115RAMZeroGapCertificate.gap_le_eps_nonneg`.  The nonnegative weak
  wrappers remove the earlier strict-positive side condition by a zero-hit
  induction.  The active 11.5 lane is now the analytic
  conditional-expectation/Hopf-Lax bridge into those certificates.
