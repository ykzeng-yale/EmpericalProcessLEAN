...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after local sync to `origin/main` at `9c41b38`
  ...
  current `Σ⁻¹` identification, and displayed center-update declarations compile
  in `StatInference/Optimization/Ellipsoid.lean`.  A
  focused `lake env lean StatInference/Optimization/Ellipsoid.lean` check
  passed after the `9c41b38` sync, while the exact
