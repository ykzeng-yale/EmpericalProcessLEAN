...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the scaled-estimator display endpoint to a concrete
...
2. Derivative or score source: only add a model-specific coordinate/matrix or
   ...
   envelope-mean wrappers, sqrt-scale wrappers, or scaled-estimator display
   wrappers.
...
Latest verified Vaart frontier before the next packet: the scaled-estimator
display source endpoint for Vaart Theorem 5.41.

The latest theorem-sized packet specializes `delta`, `scaledEstimator`, and
`estimatingAtEstimator` to their textbook definitions on top of the compiled
textbook `√n` scaling route.
...
scaled-estimator display endpoint to a concrete model-specialized Theorem 5.41
instantiation, or instantiate one remaining smoothness/root/envelope field.
