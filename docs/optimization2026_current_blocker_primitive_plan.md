...
from the post-decrement layer itself.  The remaining assumption-reduction
target is to lift this pairwise form through the parameter-shift and finite
prefix budget induction, so the honest actual recurrence can eventually be
proved from local feasibility generated by the prefix budget rather than an
external `∀ k` range-feasibility hypothesis.
