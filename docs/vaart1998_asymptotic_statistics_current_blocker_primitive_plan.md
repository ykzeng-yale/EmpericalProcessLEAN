...
representative `Z`, assumes only observation-level coordinate covariance
equality between `Q.map Z` and the score-transform coordinates under
`observationLaw`, rewrites it to pushed-forward coordinate covariance via
...
`vaart1998_covarianceBilinDual_eq_of_law_coordinate_covariance` plus Gaussian
and score-transform `L²` moment facts. It then transports covariance to
`scoreLaw` through `hScoreTransform_law.map_eq`. It derives the abstract
score-law `MemLp` and mean-zero facts by pushing the score-transform facts through
`hScoreTransform_law.map_eq`, `vaart1998_memLp_id_map_of_memLp_transform`,
and `vaart1998_integral_transform_eq_integral_id_map`. It derives the
pushed-forward Gaussian-limit mean-zero statement from the sample-level `Z`
mean-zero source via `vaart1998_zLaw_map_mean_zero_of_sample_mean_zero`. It
rebuilds full finite-dimensional measurability of `Z` via
`measurable_pi_lambda`; do not replace this with
`HasGaussianLaw.aemeasurable`, which gives only `AEMeasurable Z Q`. It rebuilds
coordinatewise score and derivative measurability via `continuous_apply`,
rebuilds coordinatewise derivative integrability via
...
the centered-product/Gamma table or pushed-forward derivative-law
...
map-law covariance from observation-level score covariance.
