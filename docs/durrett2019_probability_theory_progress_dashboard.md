...
Route from `Live In-Thread Goal Prompt V156` in
...
nonnegative-increment Toeplitz/Kronecker route, and the normalized increment-sum
a.e. endpoint.  The next target is the zero-initial/shifted `X_n / b_n`
display plus the bounded-variance Exercise 4.4.11 corollary.
The V143-V156 packets added `durrett2019_exercise_4_4_6_varianceClock_succ`,
...
Next proof packet: continue Exercise 4.4.11 from the compiled normalized
increment-sum a.e. endpoint.  Exercise 4.4.10 now supplies the almost-sure
martingale convergence handoff, and Exercise 4.4.11 now feeds scaled-transform
almost-sure convergence through Kronecker pathwise.  Aim at one forward
textbook packet: package the zero-initial/shifted `X_n / b_n` display and then
prove the bounded-variance summability instantiation from `E ξ_n^2 ≤ K` and
`Summable fun n => (b (n+1))⁻²`.  Do not re-prove the compiled Chapter
4.3.5/4.3.8 assembly, Theorem 4.4.1 optional stopping, Exercises 4.4.5-4.4.10,
the Exercise 4.4.11 transform convergence bridge, or the deterministic
Abel/Toeplitz/Kronecker/normalized-increment route.
