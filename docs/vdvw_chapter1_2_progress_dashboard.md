...
| Definition 2.1.5 | `vdVWCoveringNumber` wrapper over mathlib `Metric.externalCoveringNumber`, explicit finite closed-ball cover witnesses, finite-number handoff, monotonicity, packing comparison wrappers, deterministic empirical `L1(P_n)` distance/finite-covering-number interface, random sample-path empirical covering-number wrapper, and outer-probability `o_P^*(n)` entropy condition for Theorem 2.4.3 | Truncated-class/envelope instantiation and deterministic fixed-sample net inequality `(2.4.4)` remain pending. |
...
READY      Definition 2.1.5 covering-number primitive plus fixed-sample and random empirical L1(P_n) covering/entropy interfaces for Theorem 2.4.3 setup.
