...
Route from `Live In-Thread Goal Prompt V443` in
...
one-based normalized partial sums at level `k / 2`.  The next
source work is all-`k` packaging: turn the V443 fixed-`k` frequently-large
statements into a formal `limsup_n |S_n| / a_n = ∞` endpoint for the divergent
half, then connect it to the source-facing Feller dichotomy wrapper.  Do not
route back into solved Theorem 2.5.12 plumbing, the solved Theorem 2.5.13
tail-series transfer, or the solved fixed-`k` Borel-Cantelli partial-sum
bridge.  New compiled anchors through V443:
