...
- Current manual frontier after rebasing local `main` onto `origin/main` at
  ...
  `L * R / (8 * sqrt (N + 1))`.  The next target is literal Theorem 6.21
  certification around this packet: arbitrary `d > N` embedding if needed,
  convexity/Lipschitz/radius and subgradient-oracle correctness, then the
  strongly convex Theorem 6.22 and feasibility Definition 6.24/Theorem 6.25,
  while the exact Grünbaum/centroid theorem remains a supplied blocker for
  exact CoGM reporting.
