...
conclusion from Chewi's gradient-flow limit inequality.  `Theorem28.lean`
proves Corollary 2.8's integrated descent identity, average bound, and
interval-minimum source form.
...
`PLGradientFlowLimitRouteToQGOn` hypothesis from the book's Lyapunov argument,
or instantiate Corollary 2.8's interval-minimum hypothesis from continuity on
compact intervals.  Do not spend proof runs on exercise derivations until the
main theorem lane is covered.
