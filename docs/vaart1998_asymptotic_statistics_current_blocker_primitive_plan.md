...
representative `Z`, assumes the all-dual covariance-bilinear comparison
between `Q.map Z` and `observationLaw.map scoreTransform`, then transports it
to `scoreLaw` through `hScoreTransform_law.map_eq`. It derives the abstract
