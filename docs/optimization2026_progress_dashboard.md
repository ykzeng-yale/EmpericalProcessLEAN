...
- Current manual frontier after rebasing local `main` onto `origin/main` at
  ...
  `eps`-ball obstruction/necessary-width wrappers.  The next target is the
  source scalar/log query-count wrapper and a supplied deterministic replay
  interface for the feasibility lower bound; Theorem 6.23 remains source
...
Current active target after the verified local 6.25 full-cycle width packet on
top of pushed frontier `24a859c`: Chapter 6 lower-bound/feasibility is
...
`NonsmoothLowerBounds.lean` now proves the hard max-coordinate objective,
first-max oracle subgradient correctness, prefix support, source-rate gap,
source-radius facts, centered Lipschitz-on-`B(x_*, R)` facts, first-order
...
source strong-convex/Lipschitz certificates.  It now also advances Theorem
6.25 with compiled box-halving/separation geometry, the short-side
no-`eps`-ball obstruction, the recursive cyclic box state with per-step
separation, query-exclusion, nesting, width-update facts, and the full-cycle
width closed form plus full-cycle `eps`-ball obstruction wrappers.  The next
theorem packet should close the scalar/log query-count wrapper and supplied
deterministic replay/separation-oracle theorem interface; add literal
arbitrary-`d > N`
