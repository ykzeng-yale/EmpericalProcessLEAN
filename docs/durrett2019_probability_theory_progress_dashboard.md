...
Route from `Live In-Thread Goal Prompt V44` in
...
`martingalePart_add_ae_eq` and `predictablePart_add_ae_eq`.  The next step is
Example 4.3.3 and Theorem 4.3.4, the conditional Borel-Cantelli route via
...
Doob-decomposition existence/formula and uniqueness wrappers.
...
Next proof packet: start Example 4.3.3 and Theorem 4.3.4 conditional
Borel-Cantelli by wrapping mathlib `BorelCantelli.process`, the
`martingalePart_process_ae_eq`/`predictablePart_process_ae_eq` formulas,
`process_difference_le`, and the `Set.limsup` plus sum-indicator divergence
equivalences.
