...
| Definition 2.1.5 | `vdVWCoveringNumber` wrapper over mathlib `Metric.externalCoveringNumber`, explicit finite closed-ball cover witnesses, finite-number handoff, monotonicity, packing comparison wrappers, and deterministic empirical `L1(P_n)` distance/finite-covering-number interface for Theorem 2.4.3 | Random sample-path covering-number sequence and outer-probability little-o wrapper remain pending. |
...
READY      Definition 2.1.5 covering-number primitive plus fixed-sample empirical L1(P_n) covering interface for Theorem 2.4.3 setup.
