...
- Current manual frontier after local sync to `origin/main` at `5f759ac`
  ...
  The next target is the actual CFC/spectral square-root quadratic identity and
  the displayed next-shape PosSemidef/PosDef facts for Lemma 6.20, while the
  exact Grünbaum/centroid theorem remains a supplied blocker for exact CoGM
  reporting.
