...
Route from `Live In-Thread Goal Prompt V54` in
...
both trimmed RN derivative sequences.  The next target is an actual
source-production obligation: prove those real-valued bounded martingale
convergence hypotheses, or prove `X = Y/Z` / `{X = infinity}` singular
separation if that is the shorter current bridge.
