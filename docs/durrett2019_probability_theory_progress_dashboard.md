...
Route from `Live In-Thread Goal Prompt V110` in
...
the closed no-top/no-ambient-dichotomy `tprod` textbook branch wrapper, plus
Lemma 4.3.9's normalized conditional-mean martingale bridge.  Aim at one
forward textbook packet: try the actual Galton-Watson conditional-mean source
instantiation only if local independence APIs make it cheap; otherwise switch
