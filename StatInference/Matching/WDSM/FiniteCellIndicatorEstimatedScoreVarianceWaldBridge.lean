...
/-!
# Finite score-cell estimated-score Wald inference from variance estimates

This module composes the finite score-cell estimated-score Wald bridge with the
variance-to-standard-error positivity bridge.  The final Wald inputs can now be
stated directly in terms of positive variance estimates.
-/
...
/-- PATE estimated-score Wald bridge with an absolute region and variance estimates. -/
structure PATEFiniteScoreCellEstimatedScoreAbsoluteVarianceWaldBridge
...
theorem
    ...
    (b :
      PATEFiniteScoreCellEstimatedScoreAbsoluteVarianceWaldBridge Cell Index
  ...
  let converted :
    ...
    { studentized_bridge := b.studentized_bridge
      coverage_input := absoluteWaldCoverageInput_of_variance b.coverage_input }
  simpa [converted] using
    pate_absoluteWaldCoverage_tendsto_of_finite_score_cell_estimated_score
      converted hdesign hbounded harray_lln hfinite henvelope hsimplex hlinear
      hmatrix hdecomp hdenominator hheterogeneity hresidual hfirst hlocal
      hgodambe

/-- PATE estimated-score Wald bridge with a two-sided region and variance estimates. -/
structure PATEFiniteScoreCellEstimatedScoreTwoSidedVarianceWaldBridge
    (Cell Index Sample LimitSample : Type*) [DecidableEq Cell]
    [MeasurableSpace Sample] [MeasurableSpace LimitSample]
    (sampleLaw : MeasureTheory.Measure Sample)
    (limitLaw : MeasureTheory.Measure LimitSample) (l : Filter Index)
...
theorem
    ...
    (b :
      PATEFiniteScoreCellEstimatedScoreTwoSidedVarianceWaldBridge Cell Index
        Sample LimitSample sampleLaw limitLaw l)
    (hdesign :
      b.studentized_bridge.finite_estimated_bridge.known_score_finite_cell_bridge.stochastic_bridge.lln_bridge.lln_bridge.survey_design_regularity)
    (hbounded :
  ...
  let converted :
      PATEFiniteScoreCellEstimatedScoreTwoSidedWaldBridge Cell Index Sample
        LimitSample sampleLaw limitLaw l :=
    { studentized_bridge := b.studentized_bridge
      coverage_input := twoSidedWaldCoverageInput_of_variance b.coverage_input }
  simpa [converted] using
...
/-- PATT estimated-score Wald bridge with an absolute region and variance estimates. -/
structure PATTFiniteScoreCellEstimatedScoreAbsoluteVarianceWaldBridge
    (Cell Index Sample LimitSample : Type*) [DecidableEq Cell]
    [MeasurableSpace Sample] [MeasurableSpace LimitSample]
    (sampleLaw : MeasureTheory.Measure Sample)
    (limitLaw : MeasureTheory.Measure LimitSample) (l : Filter Index)
...
theorem
    ...
    (b :
      PATTFiniteScoreCellEstimatedScoreAbsoluteVarianceWaldBridge Cell Index
        Sample LimitSample sampleLaw limitLaw l)
    (hdesign :
      b.studentized_bridge.finite_estimated_bridge.known_score_finite_cell_bridge.stochastic_bridge.lln_bridge.lln_bridge.survey_design_regularity)
    (hbounded :
...
/-- PATT estimated-score Wald bridge with a two-sided region and variance estimates. -/
structure PATTFiniteScoreCellEstimatedScoreTwoSidedVarianceWaldBridge
    (Cell Index Sample LimitSample : Type*) [DecidableEq Cell]
    [MeasurableSpace Sample] [MeasurableSpace LimitSample]
    (sampleLaw : MeasureTheory.Measure Sample)
    (limitLaw : MeasureTheory.Measure LimitSample) (l : Filter Index)
...
theorem
    ...
    (b :
      PATTFiniteScoreCellEstimatedScoreTwoSidedVarianceWaldBridge Cell Index
        Sample LimitSample sampleLaw limitLaw l)
    (hdesign :
      b.studentized_bridge.finite_estimated_bridge.known_score_finite_cell_bridge.stochastic_bridge.lln_bridge.lln_bridge.survey_design_regularity)
    (hbounded :
