...
Route from `Live In-Thread Goal Prompt V98` in
...
`(canonicalRatio).toReal`.  The next target is the exact finite-prefix
RN-ratio identification for the canonical prefix filtration, denominator-limit
nonzeroness, and the remaining nonzero-lower-integral/product inputs.
