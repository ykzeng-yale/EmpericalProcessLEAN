...
Route from `Live In-Thread Goal Prompt V51` in
...
endpoint.  The next target is an actual source-production obligation: prove
those eventual restricted-density identities for `Yseq` and `Zseq`, or prove
...
to RN-derivative bridge, generator-extension bridge, and bounded-convergence
generator-production bridge.  Aim at one actual source-production obligation:
prove the eventual restricted-density identities for `Yseq` and `Zseq`, or
prove `X = Y/Z` / `{X = infinity}` singular separation.  Search filtration
restriction identities, `Measure.trim`, `lintegral_trim`, a.e. transfer,
ENNReal limsup/top, and RN-derivative APIs.  Do not re-prove the compiled
assembly layers.
