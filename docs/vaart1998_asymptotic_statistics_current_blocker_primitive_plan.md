...
coordinatewise observation integrability plus a single vector-valued
observation derivative mean for the derivative transform; coordinatewise
...
derivative-transform integrability via `Integrable.of_eval`; do not replace
the `Z` step with `HasGaussianLaw.aemeasurable`, which gives only
...
`vaart1998_integral_transform_eq_integral_id_map`. The remaining derivative
source is the vector-valued observation derivative mean identity; do not route
back to direct observation-level covariance equality, vector-valued score-transform
moment assumptions, full score-transform measurability, pushed-forward
derivative-law vector-mean, coordinatewise observation basis-action,
