...
The latest covariance layer removes the explicit covariance table `Gamma` and
the two separate covariance-table assumptions. It accepts the textbook-shaped
coordinate covariance equality
...
coordinate covariance table assumptions,
...
endpoint. Prefer the highest-yield source-shaped wrapper among the empirical
estimating-equation root route, remaining score/observation covariance
identifications, or smoothness/Taylor source route; reuse local `StatInference`
...
This predecessor still asks for an explicit `Gamma` table and separate
