...
`PLGradientFlowLyapunovNonzeroDisplacementRouteToQGOn` hypothesis by proving
or cleanly supplying the gradient-flow convergence-to-minimizer input,
Lyapunov-expression continuity, positive-gap side condition, and nonzero
displacement on positive times; the derivative-to-antitone, PL scalar sign,
objective-gap derivative, and nonzero norm-derivative calculations are now
compiled.
