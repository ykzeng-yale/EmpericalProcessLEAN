...
Route from `Live In-Thread Goal Prompt V31` in
...
entrypoints, and nonnegative martingale transforms.  The next frontier is
Theorem 4.2.9 stopped-process wrappers now compile for submartingales,
...
initial-positive-part subtraction display.  The next frontier is Theorem
4.2.11 almost-sure martingale convergence.
...
4.2.4/4.2.5 all-times and strict-index wrappers, and the generic Theorem 4.2.6
convex-image and `|X_n|^p` wrappers.
...
Next proof packet: package Theorem 4.2.11 almost-sure martingale convergence.
Reuse mathlib `Probability/Martingale/Convergence.lean` first, especially
`Submartingale.exists_ae_tendsto_of_bdd` and
`Submartingale.ae_tendsto_limitProcess`; if the exact Durrett positive-part
boundedness assumption is not direct, add a source-shaped wrapper from the
mathlib eLpNorm/L1 boundedness hypothesis and record the positive-part bridge
as the next narrow target.
...
commit, and push.  Closed Chapter 2 through Theorem 4.1.15 material and
Examples 4.2.1-4.2.3 are support, not live prompt content, unless the active
Chapter 4.2 theorem requires them.
