...
Route from `Live In-Thread Goal Prompt V38` in
...
thresholds.  The next step is the symmetric bounded-above bridge via `-X`.
...
Next proof packet: continue Durrett Theorem 4.3.1 by applying the bounded-below
bridge to `-X`, yielding convergence on the bounded-above path event and,
if immediate, the union bridge for bounded below or bounded above paths.
