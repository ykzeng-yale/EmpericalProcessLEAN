...
| 1.4.8 | Theorem | `..._1-100.md:910` | local-layer/mathlib-foundation: FDD forward direction now wrapped for weak convergence, including the VdV&W-named `vdVW148_finiteDimensional_weakConvergence_of_processLaw_weakConvergence`; process-law and `IdentDistrib` uniqueness-only FDD wrappers are compiled in `FiniteDimensional.lean`; raw bounded-process forward FDD, coordinate-law, and process-tightness consequences are compiled as `VdVWEllInftyProcessWeakConvergence.finiteDimensionalLaw`, `.coordinateLaw`, `.rawCoordinateLaw`, `.rawCoordinateLaw_congr_eventually_ae`, `.asymptoticallyTight_atTop`, `.finiteDimensionalLaw_asymptoticallyTight_atTop`, `VdVWEllInftyProcessAsymptoticallyTight.coordinateLaw`, `.rawCoordinateLaw`, and `.rawCoordinateLaw_congr_eventually_ae`; ordinary coordinate-law adapter `vdVWCoordinateProcessLaw`, `vdVWEllInftyProcessLaw_map_eval`, and coordinatewise a.e.-replacement lemmas `aemeasurable_coordinate_congr_ae`/`vdVWCoordinateProcessLaw_congr_ae` are available; projective-limit/FDD law equality APIs exist, but no exact weak-convergence iff-over-FDD converse theorem found |
