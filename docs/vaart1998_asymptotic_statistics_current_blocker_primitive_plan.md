...
`vaart1998_theorem_5_41_zEstimator_scaledEstimator_handoff_of_empiricalAverage_scoreTransformCoordinateMomentSource_scoreLawSource_derivativeAtTheta0TransformSource_zCoordinateMeasurableSource_vDerivativeTransformObservationMeanSource_derivativeTransformCoordinateIntegrableSource_derivativeTransformCoordinateMeasurableSource_scoreTransformCoordinateMeasurableSource_zSampleObservationScoreCenteredProductCovarianceSource_zCoordinateMeanSource_observationSequenceEvalSource_jointMeasurableSource_nonnegativeEnvelopeSource_unscaledScoreSource_fixedTheta0Source_coordinateSumRootSource_univSmoothnessSource_observationEnvelopeMeanSource_observationTransformDisplaySource_observationRandomSequenceTransformMomentSource_observationSamplePathSource_observationEnvelopeAverageSource_observationScoreCovarianceSource_zSampleCoordinateMeanSource_derivativeBasisMatrixActionSource_zSampleMeanSource_scoreVectorMeanSource_scoreLawMeanSource_zGaussianMemLpSource_zLawCovarianceBilinSource_zLawMeanSource_derivativeLawVectorIntegrableSource_scoreLawVectorMomentSource_coordinateProjectionSource_derivativeTableVectorScoreDirectSource_scoreLawCovarianceMomentSource_scoreVectorDisplaySource_estimatingMapContDiffTaylorSource_pointwiseSmoothnessSource_populationBasisMatrixActionSource_pointwiseDerivativeMatrixActionSource_measurableSource_rawRootSource_estimatorDefinitionSource_vectorScoreCommonLawScoreCLT_absorbingSource_envelope`.

This endpoint is the preferred current route. It uses coordinatewise
...
measurability and coordinatewise sample mean-zero facts for the Gaussian limit
representative `Z`; and only the textbook
centered-product covariance display: a table `Gamma`, sample-level products
`∫ Z_i Z_j dQ = Gamma i j`, and observation-level score products
`∫ score_i score_j d observationLaw = Gamma i j`. It rebuilds full
...
derivative-transform measurability and full finite-dimensional measurability
of `Z` via `measurable_pi_lambda`, rebuilds sample vector mean-zero of `Z`
from coordinatewise sample means via
`vaart1998_z_sample_mean_zero_of_coordinate_mean_zero_of_gaussian`, and
rebuilds vector-valued derivative-transform integrability via
...
`vaart1998_derivativeTransform_vector_mean_of_observation_basis_action`; do not
replace the `Z` step with `HasGaussianLaw.aemeasurable`, which gives only
`AEMeasurable Z Q`. It derives
...
`vaart1998_covarianceBilinDual_eq_of_law_coordinate_covariance`. It transports
covariance to `scoreLaw` through `hScoreTransform_law.map_eq`, and derives the
abstract score-law `MemLp` and mean-zero facts by pushing the score-transform
facts through `hScoreTransform_law.map_eq`,
`vaart1998_memLp_id_map_of_memLp_transform`, and
`vaart1998_integral_transform_eq_integral_id_map`. Do not route back to direct
observation-level covariance equality, vector-valued score-transform
...
integrability, abstract score-law moment assumptions, abstract score-law
...
sample vector mean-zero of `Z`, or full `Z` measurability, unless a source
theorem specifically demands those displays.

Immediate predecessor endpoint:
`vaart1998_theorem_5_41_zEstimator_scaledEstimator_handoff_of_empiricalAverage_scoreTransformCoordinateMomentSource_scoreLawSource_derivativeAtTheta0TransformSource_zCoordinateMeasurableSource_vDerivativeTransformObservationMeanSource_derivativeTransformCoordinateIntegrableSource_derivativeTransformCoordinateMeasurableSource_scoreTransformCoordinateMeasurableSource_zSampleObservationScoreCenteredProductCovarianceSource_zSampleMeanSource_observationSequenceEvalSource_jointMeasurableSource_nonnegativeEnvelopeSource_unscaledScoreSource_fixedTheta0Source_coordinateSumRootSource_univSmoothnessSource_observationEnvelopeMeanSource_observationTransformDisplaySource_observationRandomSequenceTransformMomentSource_observationSamplePathSource_observationEnvelopeAverageSource_observationScoreCovarianceSource_zSampleCoordinateMeanSource_derivativeBasisMatrixActionSource_zSampleMeanSource_scoreVectorMeanSource_scoreLawMeanSource_zGaussianMemLpSource_zLawCovarianceBilinSource_zLawMeanSource_derivativeLawVectorIntegrableSource_scoreLawVectorMomentSource_coordinateProjectionSource_derivativeTableVectorScoreDirectSource_scoreLawCovarianceMomentSource_scoreVectorDisplaySource_estimatingMapContDiffTaylorSource_pointwiseSmoothnessSource_populationBasisMatrixActionSource_pointwiseDerivativeMatrixActionSource_measurableSource_rawRootSource_estimatorDefinitionSource_vectorScoreCommonLawScoreCLT_absorbingSource_envelope`.

This endpoint uses the same coordinate score-transform moment, observation
derivative basis-action, and centered-product covariance route, but keeps
sample vector mean-zero of the representative Gaussian `Z` as an assumption.
The current endpoint rebuilds that field from coordinatewise sample mean-zero
facts using Gaussian `L²` integrability.
