...
Route from `Live In-Thread Goal Prompt V138` in
`docs/durrett2019_probability_theory_current_blocker_primitive_plan.md`.
The active theorem lane is Chapter 4.4 martingale maximal inequalities.  The
current frontier is the bounded-increment overshoot estimate for Exercise
4.4.6's compiled finite absolute-exit stopping time, followed by the exact
small-ball source wrapper.
...
small-ball handoff, and the finite first-exit/small-ball assembly now also
compile.  The next target is the bounded-increment overshoot estimate for the
first-exit time.
...
wrappers, Exercise 4.4.6's stopped-variance small-ball handoff, and the finite
first-exit/small-ball assembly now compile.  Aim at one forward textbook
packet: prove the bounded-increment overshoot estimate for the first-exit time
and package the exact Exercise 4.4.6 source wrapper, or move to a nearby Chapter
4.4 theorem-sized packet if that route is blocked.
