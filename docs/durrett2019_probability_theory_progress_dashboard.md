...
Route from `Live In-Thread Goal Prompt V37` in
...
The next step is convergence on the bounded-below path event, matching
Durrett's `{liminf X_n > -∞}` passage.
...
and first-below survival convergence wrappers.
...
Next proof packet: continue Durrett Theorem 4.3.1 by upgrading first-below
survival convergence to convergence on the bounded-below path event, matching
Durrett's `{liminf X_n > -∞}` step.
