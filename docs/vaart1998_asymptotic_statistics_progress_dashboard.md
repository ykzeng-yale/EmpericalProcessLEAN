...
Move next to the law-tail wrapper and remove its `hScaledEstimator` hypothesis
by using the supplied law-tail criterion and calling the absorbing
estimator-measurability endpoint, then continue upward through derivative-norm
wrappers only while each packet remains theorem-sized.  Otherwise prove the next
