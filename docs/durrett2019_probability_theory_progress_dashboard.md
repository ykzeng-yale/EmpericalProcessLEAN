...
Route from `Live In-Thread Goal Prompt V102` in
...
external `P ≠ ∞` input.  The next target is the remaining canonical-ratio
nonzero-lower-integral/product inputs.
...
and positive-product finite-limit support.  Aim at one actual source-production
obligation: discharge the canonical-ratio nonzero-lower-integral input or the
remaining final product/nonzero hypotheses consumed by the compiled branch
assemblers.  Search `lintegral_ne_zero`, `lintegral_eq_one`, `HasProd`,
`Tendsto`, `withDensity`, and existing finite-prefix mass/cylinder likelihood
endpoints.  Do not re-prove the compiled Chapter 4.3.5 or 4.3.8 assembly layers.
