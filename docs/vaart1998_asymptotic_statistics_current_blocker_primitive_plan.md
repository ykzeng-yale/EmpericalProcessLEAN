...
between `Q.map Z` and `scoreLaw`, and derives the abstract score-law `MemLp`
and mean-zero facts by pushing the score-transform facts through
...
sources, abstract score-law moment assumptions, or full `Z` measurability,
unless a source theorem specifically demands those displays.
