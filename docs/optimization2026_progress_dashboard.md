...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean` | The Chapter 5 quadratic substrate now starts from the source display `f(x)=1/2 <x, A x> - <b, x>` with `quadraticObjective`, `quadraticGradient`, supplied symmetry and quadratic-form lower/upper bounds, exact expansion `quadraticObjective_eq_model_add_quadratic`, first-order strong-convexity and smoothness wrappers, and the minimizer bridge `quadraticObjective_isMinOn_of_apply_eq`. Search-first result: reused mathlib continuous linear maps, `continuous_id.inner`, `Continuous.inner`, `real_inner_comm`, and local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers; defer full spectral/self-adjoint API integration until a theorem needs it. Next target: `A`-inner product/norm, Krylov subspace definitions, supplied CG-state/optimality interface, Lemma 5.1, Theorem 5.3, and Theorem 5.4's descent/orthogonality/halving proof. |
...
from the quadratic display at markdown lines 954-960 and the minimizer/system
claim at line 960.  The next aggressive step is to build the `A`-inner
product, Krylov subspace, CG recurrence/optimality interfaces, then prove
Lemma 5.1 and use it for Theorem 5.3 and Theorem 5.4.  Do not replay
