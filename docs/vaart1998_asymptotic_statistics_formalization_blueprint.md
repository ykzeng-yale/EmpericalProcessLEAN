...
Current frontier: Theorem 5.41 now has a compiled derivative-transform vector
integrability source endpoint
...
integrability and derives each matrix-entry field with `Integrable.eval`.  Its
immediate predecessor is the coordinate score moment source endpoint
