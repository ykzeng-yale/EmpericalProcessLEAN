...
- Current manual frontier after rebasing local `main` onto `origin/main` at
  `be7af69` (`Add FDD asymptotic tightness bridges`) with the verified
  ...
  `chewi620_displayedMatrices_stepCertificate_of_cfcSqrt`.  The next target is
  to package the exact Lemma 6.20 one-step certificate into a trajectory/rate
  wrapper, while the exact Grünbaum/centroid theorem remains a supplied blocker
  for exact CoGM reporting.
