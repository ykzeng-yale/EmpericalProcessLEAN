...
measurability for the score and derivative transforms, observation-level
coordinate integrability plus a single vector-valued observation derivative
...
coordinatewise derivative basis-action means via
`vaart1998_derivativeTransform_observation_basis_action_of_vector_mean`, then
rebuilds the pushed-forward derivative-law mean display using
`MeasureTheory.eval_integral` and
`vaart1998_integral_transform_eq_integral_id_map`. Do not route back to the
centered-product/Gamma table or pushed-forward derivative-law
vector-integrability, law vector-mean, or coordinatewise observation
basis-action sources unless a source theorem specifically demands those
displays.
...
The current endpoint rebuilds those coordinatewise means from a single
vector-valued observation derivative mean. This predecessor still rebuilds
the pushed-forward derivative-law vector mean from observation-level
coordinate integrals and the pushforward integral identity, and still rebuilds
law integrability from observation-level coordinate integrability via
`vaart1998_derivativeLaw_coordinate_integrable_of_observation_derivativeTransform`
and `vaart1998_derivativeLaw_integrable_id_of_coordinate_integrable`.
