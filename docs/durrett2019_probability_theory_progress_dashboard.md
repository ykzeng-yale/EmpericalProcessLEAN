...
Route from `Live In-Thread Goal Prompt V220` in
...
the two auto interval/monotonicity endpoint wrappers.  The next target is to
package the remaining lower-bound/no-zero and normalizer-divergence hypotheses
from cleaner source assumptions, or move to the next martingale theorem if
those are already the exact textbook hypotheses.
