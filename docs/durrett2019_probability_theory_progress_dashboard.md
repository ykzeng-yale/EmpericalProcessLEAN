...
Route from `Live In-Thread Goal Prompt V53` in
...
final source endpoint is specialized to this bound automatically.  The next
target is an actual source-production obligation: prove convergence of those
sequences, or prove `X = Y/Z` / `{X = infinity}` singular separation.
