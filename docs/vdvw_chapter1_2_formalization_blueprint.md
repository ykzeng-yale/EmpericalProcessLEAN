...
| 1.2.5 | Lemma | `..._1-100.md:467` | local-layer: measurable-target first/second coordinate product-cover transfers compiled as `VdVWMeasurableCover.fstProductOfMeasurable` and `.sndProductOfMeasurable`, plus probability-product outer-expectation invariance for measurable and a.e.-measurable targets as `VdVWOuterExpectation_prod_fst_eq_of_measurable`, `.prod_snd_eq_of_measurable`, `.prod_fst_eq_of_aemeasurable`, and `.prod_snd_eq_of_aemeasurable`; full arbitrary-map perfect-projection theorem still pending |
