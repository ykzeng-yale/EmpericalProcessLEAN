...
`scoreTransform = observationEstimatingMap · theta0` tie assumptions,
...
source or a direct observation-estimating-map theta0 moment/covariance source;
the sample-indexed estimating-map plumbing and separate score-transform map are
solved and should not be replayed. Reuse local `StatInference` and mathlib APIs
...
This predecessor still asks for a separate `scoreTransform` parameter, vector
