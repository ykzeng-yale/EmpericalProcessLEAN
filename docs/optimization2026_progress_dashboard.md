...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after the 2026-05-05 cutting-plane source packet:
  ...
  `HasScaledOutsideCandidateWithinDiameter`, finite shrink and ratio bounds,
  scaled-candidate Lipschitz/diameter bound, the limiting scalar helper, and
  source-rate wrappers.  The active target is now the remaining CoGM
  supplied-geometry/candidate bridge, then Lemma 6.20.
...
forms, and `CuttingPlane.lean` now compiles the supplied-interface algebraic
spine for Lemma 6.18/Theorem 6.19.  The next manual goal packet should derive
the eventual scaled-candidate bound from the volume comparison/localization
certificate when feasible, package the source-shaped CoGM theorem around
`chewi619_gap_le_display_rate_of_eventual_scaled_bound`, and otherwise keep
the centroid/Grunbaum geometry as a precise supplied blocker before moving to
ellipsoid Lemma 6.20.
