...
Route from `Live In-Thread Goal Prompt V40` in
...
one-sided-bounded union bridge now compile.  The next step is the
explicit liminf/limsup display for the oscillation side; the range-form
event-classification layer already compiles.
...
wrappers, plus the range-form convergence-or-unbounded dichotomy wrapper.
...
Next proof packet: continue Durrett Theorem 4.3.1 by upgrading the unbounded
range side of the compiled dichotomy to the textbook
