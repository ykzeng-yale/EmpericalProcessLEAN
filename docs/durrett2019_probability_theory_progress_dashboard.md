...
Route from `Live In-Thread Goal Prompt V115` in
...
The next target is the first positive-part Lp maximal inequality for
submartingales, or a compiled integration/Hölder bridge feeding it.
...
from a supplied positive-part Lp maximal bound.  Aim at one forward textbook
packet: the first positive-part Lp maximal inequality / integration-Hölder
layer.
