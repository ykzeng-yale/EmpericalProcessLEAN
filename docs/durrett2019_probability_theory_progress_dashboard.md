...
Route from `Live In-Thread Goal Prompt V29` in
...
Theorem 4.2.9 stopped supermartingales.
...
Next proof packet: package Theorem 4.2.9 stopped supermartingales.  Search
`IsStoppingTime`, `stoppedProcess`, `stoppedValue`, and optional-sampling APIs
first; otherwise use the compiled Theorem 4.2.8 transform with
`H_m = 1_{N ≥ m}` plus the finite telescoping identity
`(H · X)_n = X_{N ∧ n} - X_0`, then add the constant process back.  Add
submartingale and martingale stopped-process variants if direct.
