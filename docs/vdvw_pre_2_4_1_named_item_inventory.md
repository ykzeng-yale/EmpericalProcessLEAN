...
| `VdVWMeasurableCover.addConstLeft`, `VdVWOuterExpectation_eq_lintegral_const_add_cover` | proved nonnegative Lemma 1.2.2(i) equality layer when the left summand is a constant: `(c + T)* = c + T*` |
...
| 1.2.2 Lemma | `M1:389` | a.s. rules for arbitrary maps | local layer exists: nonnegative sup/add/inf/product, constant-left addition equality, threshold-indicator, and measurable-left inf-equality cover algebra; exact extended-real signed clauses, subtraction, absolute value, and remaining equality cases remain pending |
