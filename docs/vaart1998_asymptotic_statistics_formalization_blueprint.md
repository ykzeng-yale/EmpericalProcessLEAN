...
Current frontier: Theorem 5.41 now has a compiled joint-measurability source
endpoint
...
It derives the two sampled derivative summand measurability fields from joint
measurability of the derivative kernels on `Ω × Observation` and the
measurable observation sequence. Its immediate predecessor is the
absolute-envelope source endpoint
...
measurable observation sequence. The next proof packet should continue from
this endpoint to a concrete model-specialized
