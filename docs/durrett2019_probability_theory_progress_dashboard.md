...
Route from `Live In-Thread Goal Prompt V57` in
...
existing `mu + nu` endpoint.  The next target is an actual source-production
obligation: prove the remaining top-set singular separation for the canonical
ratio, namely the singular-part complement vanishing and `nu`-null top-set
facts.
