...
Route from `Live In-Thread Goal Prompt V96` in
...
`nu {canonicalRatio = infinity} = 0` automatically.  The next target is proving
standard full-prefix likelihood convergence to this canonical ratio and
discharging its integrability or nonzero-lower-integral/product inputs for the
