...
Route from `Live In-Thread Goal Prompt V221` in
...
The next target is either the final deterministic normalizer-divergence source
lemma from Durrett's exact `f` hypotheses, or the next martingale theorem if
`Tendsto f atTop atTop` is accepted as the textbook source assumption.
