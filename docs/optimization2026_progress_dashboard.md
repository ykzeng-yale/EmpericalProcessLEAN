...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean`, `StatInference/Optimization/Theorem54.lean` | The Chapter 5 quadratic substrate now starts from the source display `f(x)=1/2 <x, A x> - <b, x>` with `quadraticObjective`, `quadraticGradient`, supplied symmetry and quadratic-form lower/upper bounds, exact expansion `quadraticObjective_eq_model_add_quadratic`, first-order strong-convexity and smoothness wrappers, and the minimizer bridge `quadraticObjective_isMinOn_of_apply_eq`. The Krylov layer adds `aInner`/`aNormSq`, Krylov vectors/subspaces, direction spans, monotonicity and `A`-image lemmas, the supplied-interface Lemma 5.1 theorem `IsCGKrylovRecurrence.cgDirectionSubmodule_eq_krylovSubmodule`, and the Theorem 5.3-facing residual exactness wrapper `quadraticObjective_isMinOn_of_cgResidualExactnessState`. The source-recurrence layer adds `IsCGThreeTermRecurrence`, proves residuals/directions stay in Krylov spaces, proves residuals stay in direction spans, proves `A p_n ∈ span {p_0,...,p_{n+1}}` from nonzero line-search coefficient, and derives Lemma 5.1 from the three-term update formulas. The displayed-coefficient layer adds `cgLineSearchCoeff`, `cgDirectionUpdateCoeff`, their nonzero denominator/coefficient lemmas, and `IsCGDisplayedIteration`, deriving `IsCGThreeTermRecurrence`, `IsCGKrylovRecurrence`, and Lemma 5.1's Krylov equality from the literal textbook residual/direction formulas. The termination layer proves the `p_{n+1}=0` residual-zero/minimizer branch, the finite-dimensional mutually-orthogonal residual counting lemma, and `exists_quadraticObjective_isMinOn_of_pairwise_orthogonal_residuals`: orthogonal residuals that identify with quadratic gradients force a global minimizer by time `finrank ℝ E`. The point-update bridge proves `quadraticGradient_succ_of_point_step`, propagates `r_n = quadraticGradient A b (x_n)` from matching point/residual updates, and gives `IsCGDisplayedIteration.exists_quadraticObjective_isMinOn_of_pairwise_orthogonal` from the displayed point update plus residual orthogonality. The orthogonality propagation layer proves `IsCGThreeTermRecurrence.pairwise_residual_orthogonal`, `IsCGDisplayedIteration.pairwise_residual_orthogonal`, and `IsCGDisplayedIteration.exists_quadraticObjective_isMinOn_of_orthogonalToPrevious`: new-residual orthogonality to the previous direction span implies pairwise residual orthogonality and therefore the finite-dimensional minimizer wrapper. The scalar-orthogonality layer proves `isOrthogonalToSubmodule_cgDirectionSubmodule_of_inner_direction_eq_zero`, `orthogonalToPrevious_of_inner_directions_eq_zero`, and `IsCGDisplayedIteration.exists_quadraticObjective_isMinOn_of_inner_directions_eq_zero`, reducing Theorem 5.3 to scalar equations `inner ℝ (r (n+1)) (p k)=0` for `k≤n`. The line-search/A-conjugacy layer proves `inner_residual_succ_direction_eq_zero_of_inner_residual_direction_eq_norm_sq`, `IsCGDisplayedIteration.inner_residual_direction_eq_norm_sq`, `IsCGDisplayedIteration.inner_residual_succ_direction_self_eq_zero`, `IsCGDisplayedIteration.inner_residual_succ_directions_eq_zero_of_aOrthogonal`, `IsCGDisplayedIteration.exists_quadraticObjective_isMinOn_of_aOrthogonal`, and the zero-direction branch `IsCGDisplayedIteration.quadraticObjective_isMinOn_of_direction_succ_eq_zero_and_aOrthogonal`. The simultaneous induction proves `IsCGDisplayedIteration.aOrthogonal_and_inner_residual_succ_directions`, `IsCGDisplayedIteration.aOrthogonal_directions`, `IsCGDisplayedIteration.inner_residual_succ_directions_eq_zero`, `IsCGDisplayedIteration.quadraticObjective_isMinOn_of_direction_succ_eq_zero`, and the finite-dimensional Theorem 5.3-facing wrapper `IsCGDisplayedIteration.exists_quadraticObjective_isMinOn`, removing the external A-conjugacy and scalar residual-orthogonality hypotheses. The Theorem 5.4 layer now has the descent-sum bound, its `sum <= 2 beta gap_0` algebraic form, the summed first-order/orthogonality inequality, Cauchy-Schwarz wrapper, finite Pythagoras identity for pairwise orthogonal gradients, square-root product bound, competitive-step instantiation, strong-convexity radius estimate, first-order-strong-convexity pre-halving wrapper, scalar simplification to `2 * sqrt(beta / alpha) * gap_0`, and the halving algebra `gap_N >= gap_0 / 2 -> N <= 4 * sqrt(beta / alpha)`. Search-first result: reused mathlib continuous linear maps, `continuous_id.inner`, `Continuous.inner`, `real_inner_comm`, `real_inner_smul_left`, `Function.iterate_succ_apply'`, `Submodule.span_induction`, `Submodule.span_mono`, `smul_smul`/`inv_mul_cancel₀`, coefficient APIs `div_ne_zero`, `neg_ne_zero`, `pow_ne_zero`, `norm_ne_zero_iff`, `field_simp`, `ring`, finite-dimensional bilinear-form APIs `LinearMap.BilinForm.linearIndependent_of_iIsOrtho`, `LinearIndependent.fintype_card_le_finrank`, `innerₗ`, local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers, Chapter 3 `gradient_sq_step_le_drop_of_smoothWithGradientOn` plus `sum_range_sub_succ`, `sum_inner`/`inner_sum`, `real_inner_le_norm`, `Real.le_sqrt_of_sq_le`, `Real.sq_sqrt`, `sq_le_sq₀`, and scalar `field_simp`/`ring_nf`. Next target: assemble a source-facing CG Theorem 5.4 wrapper by deriving the competitive-step, monotone-gap, and first-order/orthogonality hypotheses from the displayed affine-minimizer/search-span interfaces. |
| Chapters 6-11 deterministic algorithms | pending-local | none | Nonsmooth optimization, Frank-Wolfe, proximal methods, Fenchel duality, mirror methods, and alternating minimization should wait until Chapter 5's acceleration/CG layer is stable. |
