...
- Current manual frontier after promoted Lean verification of the Chapter 10
  MPGD one-step packet in `StatInference/Optimization/MirrorDescent.lean`,
  rebased over current `origin/main` at `5b35a16`: Chapter 9/10 now has a
  compiled finite-valued Fenchel/Bregman/mirror substrate.  The latest
  verified Optimization proof frontier is `9b618de` (`Add Chewi theorem 10.9
  MPGD one-step layer`).  `MirrorDescent.lean` compiles
  ...
  `mirrorProximalGradient_descent`, and trajectory accessors.  Reuse facts:
  no direct pinned mathlib Bregman/mirror-descent/MPGD theorem was found;
  local reuse is `Bregman.lean`'s relative lower/upper models, the
  supplied relative-growth certificate, `Proximal.lean`'s
  `compositeObjective`, and Chapter 3/5 recurrence/telescope algebra for the
  next rate layer.  Next aggressive theorem lane: finish Chewi Theorem 10.9's
  full MPGD rate from the one-step inequality, then package Theorem 10.11
  nonsmooth MPGD and Theorem 10.13 OMD regret if they stay bounded; otherwise
  open Chapter 11 alternating Bregman projections/minimization.
