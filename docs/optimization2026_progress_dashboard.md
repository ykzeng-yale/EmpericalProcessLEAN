...
sampled, smooth integral-L2 sampled-model, smooth Bochner-unbiased, and
non-smooth source-L2 sampled rate packets: treat Chapters 3-8, Chapter 9/10
...
are instantiated.  Next prove variance domination from (12.1), any exact
source conditional-expectation/process packaging that is still needed, and a
final source-shaped Chewi Theorem 12.1 wrapper combining those fields with the
compiled sampled endpoints.
