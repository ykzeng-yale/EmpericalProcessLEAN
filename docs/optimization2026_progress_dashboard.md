...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean` | The Chapter 5 quadratic substrate now starts from the source display `f(x)=1/2 <x, A x> - <b, x>` with `quadraticObjective`, `quadraticGradient`, supplied symmetry and quadratic-form lower/upper bounds, exact expansion `quadraticObjective_eq_model_add_quadratic`, first-order strong-convexity and smoothness wrappers, and the minimizer bridge `quadraticObjective_isMinOn_of_apply_eq`. The Krylov layer adds `aInner`/`aNormSq`, Krylov vectors/subspaces, direction spans, monotonicity and `A`-image lemmas, the supplied-interface Lemma 5.1 theorem `IsCGKrylovRecurrence.cgDirectionSubmodule_eq_krylovSubmodule`, and the Theorem 5.3-facing residual exactness wrapper `quadraticObjective_isMinOn_of_cgResidualExactnessState`. The newest source-recurrence layer adds `IsCGThreeTermRecurrence`, proves residuals/directions stay in Krylov spaces, proves residuals stay in direction spans, proves `A p_n ∈ span {p_0,...,p_{n+1}}` from nonzero line-search coefficient, and derives Lemma 5.1 from the three-term update formulas. Search-first result: reused mathlib continuous linear maps, `continuous_id.inner`, `Continuous.inner`, `real_inner_comm`, `Function.iterate_succ_apply'`, `Submodule.span_induction`, `Submodule.span_mono`, `smul_smul`/`inv_mul_cancel₀`, and local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers; defer full spectral/self-adjoint API integration until a theorem needs it. Next target: define the scalar line-search/Gram-Schmidt coefficients and denominator side conditions, then package finite-dimensional termination and Theorem 5.4's descent/orthogonality/halving proof. |
