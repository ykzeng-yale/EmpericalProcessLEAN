...
measurability, coordinatewise root equations, finite-vector sum root equations,
or explicit nonnegative-envelope hypotheses, unless a source theorem
...
absolute-envelope route, but still consumes the finite-vector sum root equation.
The current endpoint rebuilds that sum root from the textbook empirical-average
estimating equation.
