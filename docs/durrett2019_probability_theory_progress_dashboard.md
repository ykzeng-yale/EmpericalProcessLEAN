...
Route from `Live In-Thread Goal Prompt V140` in
...
deterministic variance clock, square-martingale source from increments,
initialization, clock nonnegativity, and `var(S_n)` endpoint feeding the
compiled bounded-increment square-martingale wrapper.
...
with automatic stopped integrability now also compile.  The next target is the
remaining exact Exercise 4.4.6 source instantiation.
...
compile.  Aim at one forward textbook packet: instantiate the deterministic
variance clock, square-martingale source, initialization, clock nonnegativity,
and `var(S_n)` endpoint for the exact Exercise 4.4.6 source wrapper, or move to
a nearby Chapter 4.4 theorem-sized packet if that route is blocked.
