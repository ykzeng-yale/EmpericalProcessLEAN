...
Route from `Live In-Thread Goal Prompt V70` in
...
zero-set input, collapses to `mu << nu`.  The next target is positive-product
mass support also now compiles: nonzero `lintegral` of the limiting likelihood,
or `lintegral X = 1`, feeds the same absolute-continuity conclusion.  The next
target is positive-product L1 convergence into that mass-one/nonzero-lintegral
input, infinite-product criterion support, and tail-event zero-one support for
