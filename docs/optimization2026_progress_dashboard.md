...
- Current manual frontier: Chapter 5 CG is past the basic Krylov/termination
  setup.  `Theorem54.lean` now contains the displayed restarted-CG/log-rate
  endpoint for Theorem 5.4; the active target is final source packaging if
  cheap, then `Theorem58.lean` for AGF Lyapunov-to-rate and the rest of
  Chapter 5.
...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean`, `StatInference/Optimization/Theorem54.lean` | The Chapter 5 quadratic/CG substrate, Lemma 5.1 Krylov equality, Theorem 5.3 finite-dimensional termination-facing wrappers, and Theorem 5.4 descent/halving/log-rate layers now compile. The latest endpoint is the displayed restarted-CG specialization `chewi54_log_rate_of_displayed_cg_blocks` and `chewi54_log_rate_of_displayed_cg_blocks_blockSize`, which discharges the per-block span, residual-gradient, orthogonality, and affine-minimizer assumptions from `IsCGDisplayedIteration`-style block records. Search-first result: reused mathlib continuous linear maps, inner-product continuity, submodule/span APIs, finite-dimensional orthogonal-family APIs, scalar inverse/log APIs, local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers, Chapter 3 descent/telescoping helpers, and Chapter 4 scalar recurrence/log bridges. Next target: source-audit the exact Theorem 5.4 wrapper/report path only if cheap and source-useful; otherwise open `StatInference/Optimization/Theorem58.lean` for Theorem 5.8's AGF Lyapunov-to-rate proof, then continue through Theorem 5.9 and the rest of Chapter 5. |
