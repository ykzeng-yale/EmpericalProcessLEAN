...
Route from `Live In-Thread Goal Prompt V118` in
...
The next target is the weighted/Fubini identification of the remaining
threshold integral with `(p - 1)⁻¹ * ∫ X_n^+ (bar X_n)^(p-1)`, followed by
finite-truncation/monotone-convergence/division algebra feeding the compiled
`lintegral_rpow` hypothesis.
...
threshold bound now compile as well.  Aim at one forward textbook packet:
weighted/Fubini identification of the remaining threshold integral, then
