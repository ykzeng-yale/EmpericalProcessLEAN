...
mean for the derivative transform, abstract score-law `MemLp id 2 scoreLaw`
and mean-zero sources, coordinatewise measurability for the Gaussian limit
representative `Z`, assumes the all-dual covariance-bilinear comparison
between `Q.map Z` and `scoreLaw`, and derives the pushed-forward
Gaussian-limit mean-zero statement from the sample-level `Z` mean-zero source
via `vaart1998_zLaw_map_mean_zero_of_sample_mean_zero`. It rebuilds full
finite-dimensional measurability of `Z` via `measurable_pi_lambda`; do not
replace this with `HasGaussianLaw.aemeasurable`, which gives only
`AEMeasurable Z Q`. It rebuilds coordinatewise score and derivative
...
sources, or full `Z` measurability, unless a source theorem specifically
demands those displays.
