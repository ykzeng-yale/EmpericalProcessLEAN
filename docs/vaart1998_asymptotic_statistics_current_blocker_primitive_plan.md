...
coordinate integrability and coordinatewise observation-level basis-action
means for the derivative transform, abstract score-law `MemLp id 2 scoreLaw`
...
via `vaart1998_zLaw_map_mean_zero_of_sample_mean_zero`. It rebuilds the
pushed-forward derivative-law mean display using `MeasureTheory.eval_integral`
...
vector-integrability or vector-mean sources unless a source theorem
...
the pushed-forward derivative-law vector mean as an assumption. The current
endpoint rebuilds that vector mean from observation-level coordinate
integrals and the pushforward integral identity, while this predecessor still
rebuilds law integrability from observation-level coordinate integrability via
