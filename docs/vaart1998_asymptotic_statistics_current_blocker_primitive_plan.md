...
This endpoint replaces the empirical-average raw-root hypothesis by the
textbook finite-sum estimating equation and derives the average root internally
with `empiricalAverageVector_eq_inv_smul_sum` and `samplePath`.
...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the finite-sum root endpoint to a concrete
...
2. Derivative or score source: only add a model-specific coordinate/matrix or
   ...
   wrappers, full-domain smoothness/source-set wrappers, or finite-sum root
   wrappers.
