...
feeds the same final route from eventual `samplePath`-level empirical-wrapper
covering-number lower bounds.
...
The selector burden is now lowered further by
...
2026-05-09 signed pair-difference component measurability:
`Theorem243.lean` now proves the generic varying-weight measurability bridges
`measurable_vdVWWeightedSampleSum_varying_weights` and
`measurable_vdVWWeightedClassSupremum_of_countable_varying_weights`, then uses
them in
`measurableSet_VdVWTheorem243PairDifferenceGhostRademacher_pairBad_of_countable`.
The corresponding event constructor
...
The inverse-square replacement is now compiled through
...
The probability-facing lower-growth bridge is now also compiled as
...
Thus the selected-cardinality growth hypothesis may be supplied either almost
...
These lower-growth routes now also package directly into the
`VdVWTheorem243TextbookAlignedConclusion` bundle via
...
them at both the selected-net source layer and the fixed-`M` convergence layer:
...
`VdVWTheorem243ProductPairRademacherSelectedNetEvent_outerProbability_bound_of_chebyshev_countable_signSample_ae_finiteCenter_halfScale_of_selected_truncated_quarterRadius_firstLevel_unneg`, and
...
The same branch now reaches the large-`M` untruncated centered convergence
target through
...
The unnegated first-level product-pair endpoint now also feeds the final
textbook-facing bundle through
...
`VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_displayedBeta_countable_firstLevel_of_signSwapBad_finiteCenter_failure_tails_selectedCenterAt_of_invSq_lintegral`,
`VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_displayedBeta_countable_firstLevel_of_pairSubBad_signSwap_finiteCenter_failure_tails_selectedCenterAt_of_invSq_lintegral`,
`VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_displayedBeta_countable_firstLevel_of_signSwapBad_finiteCenter_failure_tails_selectedCenterAt_of_eventually_ae_cardinality_ge`, and
`VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_forall_pos_radius_logCardinality_of_displayedBeta_countable_firstLevel_of_pairSubBad_signSwap_finiteCenter_failure_tails_selectedCenterAt_of_eventually_ae_cardinality_ge`.
The additive sign-swap route no longer exposes raw integrated failure-tail
convergence when a full-law inverse-square selected-cardinality integral, or
equivalently a deterministic lower bound tending to infinity a.e. for the
canonical half-radius selected cover, is supplied.  The active source frontier
