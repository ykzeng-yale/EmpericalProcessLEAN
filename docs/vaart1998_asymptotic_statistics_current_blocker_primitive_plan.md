...
measurability for the score transform, abstract score-law `MemLp id 2
scoreLaw` and mean-zero sources, assumes the all-dual covariance-bilinear
comparison between `Q.map Z` and `scoreLaw`, and derives the pushed-forward
Gaussian-limit mean-zero statement from the sample-level `Z` mean-zero source
via `vaart1998_zLaw_map_mean_zero_of_sample_mean_zero`. Do not route back to
the centered-product/Gamma table unless a source theorem specifically demands
that display.
