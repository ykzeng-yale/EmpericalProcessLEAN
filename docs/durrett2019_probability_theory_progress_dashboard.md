...
Route from `Live In-Thread Goal Prompt V35` in
...
4.3.1, beginning with the stopped-below convergence-on-survival bridge used in
the bounded-increments dichotomy proof.
...
wrappers, and Theorem 4.2.12 nonnegative-supermartingale convergence,
...
Next proof packet: start Durrett Theorem 4.3.1 by adding the stopped-below
convergence-on-survival bridge that applies Theorem 4.2.12 to
`X_{n ∧ N} + K + M`.
