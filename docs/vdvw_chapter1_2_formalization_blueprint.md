...
3. Promote Chapter 2 primitive infrastructure: covering/packing semimetric
   ...
   expectation bound, split-at-radius tail-to-expectation bound, and ordinary
   measurable truncation-tail integral bridge are now compiled.  The next
   frontier is proving or primitive-registering the Mills/logarithmic
   Gaussian-tail estimate that turns the split bound into the textbook
   maximal-expectation scale, packaging it as the VdV&W
