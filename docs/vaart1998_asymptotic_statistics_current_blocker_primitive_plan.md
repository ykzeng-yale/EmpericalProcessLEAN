...
This endpoint replaces the two sampled summand measurability assumptions by
joint measurability of `(omega, observation) ↦ derivativeAt n omega observation
theta0` and `(omega, observation) ↦ secondDerivative n omega observation`,
deriving the sampled fields from the measurable observation sequence.
...
1. Move one layer closer to a model-facing finite-dimensional statement:
   continue from the joint-measurability absolute-envelope fixed-`theta0` endpoint toward a concrete
...
Latest verified Vaart frontier before the next packet: the joint-measurability
absolute-envelope source endpoint for Vaart Theorem 5.41.
