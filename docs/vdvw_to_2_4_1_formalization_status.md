...
| `vdVWTheorem243_productCopy_fst_hasLaw`, `vdVWTheorem243_productCopy_snd_hasLaw`, `vdVWTheorem243_productCopy_fst_snd_indep`, `vdVWTheorem243_productCopy_fst_snd_identDistrib`, `integrable_vdVWTruncatedClassFun_pairDifference`, `vdVWTheorem243_productCopy_truncatedClassFun_laws_indep`, `integral_vdVWTruncatedClassFun_productCopy_pairDifference_eq_zero`, `ae_vdVWWeightedClassSupremum_le_finiteNetHoeffdingUpper_add_of_rademacherSigns` | 2026-05-03 update: Theorem 2.4.3 now has VdVW-facing product-copy wrappers for the two `P.prod P` coordinates, reusing the Billingsley/ProbabilityMeasure `probability_prod_independent_self_copies` and mapped-coordinate product-law layers.  Fixed truncated pair differences are integrable on `P.prod P` whenever the fixed truncated coordinate is integrable under `P`, their product-copy mean is zero, and the deterministic Rademacher finite-net handoff has an a.e. random-sign wrapper.  The next step is to assemble these local precursors into the `Phi(x)=x` product/Fubini symmetrization inequality. |
