...
Route from `Live In-Thread Goal Prompt V120` in
...
Coefficient extraction and the assembled Doob/Fubini/Hölder endpoint now also
compile:
...
The next target is scalar cancellation/division algebra that turns this
assembled bound into the compiled `lintegral_rpow` hypothesis with the
textbook constant `p/(p-1)`.
...
coefficient extraction, and assembled Doob/Fubini/Hölder endpoint now also
compile.  Aim at one forward textbook packet: scalar cancellation/division
algebra for the actual positive-part p-th-power estimate.
