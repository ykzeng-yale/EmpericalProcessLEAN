...
| --- | --- | --- | --- |
| Chapter 1 convexity/smoothness foundations | local-layer/mathlib-foundation | `StatInference/Optimization/Basic.lean`; mathlib `StrongConvexOn`, `ConvexOn`, `gradient` | Initial interfaces for strong convexity, Chewi convexity, smooth upper models, gradient-descent steps, first-order lower models, strong gradient monotonicity, step co-coercivity, mathlib-gradient Lipschitzness, and GD trajectories compile as the intended surface for Definition 1.5, Definition 1.12, Proposition 1.6, Exercise 3.1, and Chapter 3. Prefer mathlib's `StrongConvexOn`, `ConvexOn`, and `gradient` APIs for exact derivative-heavy theorem routes. |
...
| Chapter 3 smooth gradient descent | local-layer | `gradientDescentStep`, `IsGradientDescentTrajectory`, `DiscreteGronwall.lean`, `GradientDescent.lean`, `Theorem33.lean`, `Theorem34.lean` | First deterministic GD trajectory interface is available. Chewi Lemma 3.5 now has zero-based and source-shaped one-based compiled wrappers, Chewi Lemma 3.1 is compiled from the smooth upper-model interface, GD function values are antitone under the descent step-size condition, Theorem 3.3 supplied-interface contraction compiles in squared and norm forms, and Theorem 3.4 positive-`alpha` plus `alpha = 0` denominator bounds compile from the first-order strong-convexity supplied interface. Next target is deriving the Theorem 3.3 supplied inputs from Exercise 3.1/Proposition 1.6 or source-audited report packaging. |
...
4. Keep the compiled Theorem 3.3 supplied-interface contraction layer
   ...
   `GradientStepCocoerciveOn`, while treating the Exercise 3.1 co-coercivity
   derivation and Proposition 1.6 gradient-monotonicity bridge as the next
...
supplied-interface squared and norm contraction declarations, and Theorem 3.4
now has a compiled supplied weighted-sum assembly layer, a first-order
...
remaining Chapter 3 choices are source-audited report packaging, the Exercise
3.1 co-coercivity derivation, or the full
