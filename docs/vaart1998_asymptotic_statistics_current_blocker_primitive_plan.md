...
It still consumes the second-derivative joint measurability field separately.
...
measurability hypotheses, or explicit nonnegative-envelope hypotheses, unless a
source theorem specifically demands those displays.

Immediate predecessor endpoint:
`vaart1998_theorem_5_41_zEstimator_scaledEstimator_handoff_of_canonicalProductObservationSequence_estimatorVectorConsistencySource_absEnvelopeIntegrableSource_scoreTransformMapLawSource_scoreTransformCoordinateMomentSource_derivativeAtTheta0TransformSource_zGaussianMeasurableModificationSource_vDerivativeTransformObservationMeanSource_derivativeTransformCoordinateIntegrableSource_derivativeTransformCoordinateMeasurableSource_scoreTransformCoordinateMeasurableSource_zSampleCoordinateCovarianceObservationScoreCoordinateCovarianceSource_zCoordinateMeanSource_jointMeasurableSource_absEnvelopeSource_unscaledScoreSource_fixedTheta0Source_empiricalAverageRootSource_univSmoothnessSource_observationEnvelopeMeanSource_observationTransformDisplaySource_observationRandomSequenceTransformMomentSource_observationSamplePathSource_observationEnvelopeAverageSource_observationScoreCovarianceSource_zSampleCoordinateMeanSource_derivativeBasisMatrixActionSource_zSampleMeanSource_scoreVectorMeanSource_scoreLawMeanSource_zGaussianMemLpSource_zLawCovarianceBilinSource_zLawMeanSource_derivativeLawVectorIntegrableSource_scoreLawVectorMomentSource_coordinateProjectionSource_derivativeTableVectorScoreDirectSource_scoreLawCovarianceMomentSource_scoreVectorDisplaySource_estimatingMapContDiffTaylorSource_pointwiseSmoothnessSource_populationBasisMatrixActionSource_pointwiseDerivativeMatrixActionSource_measurableSource_rawRootSource_estimatorDefinitionSource_vectorScoreCommonLawScoreCLT_absorbingSource_envelope`.
...
probability, and absolute-envelope integrability route, but still consumes the
fixed-`theta0` derivative joint measurability field directly. The current
endpoint derives that fixed-`theta0` field from full joint measurability of
`derivativeAt` in the parameter argument, while keeping the second-derivative
joint measurability source as a separate live model obligation.
...
full derivative-at joint measurability source endpoint for Vaart Theorem 5.41.

The latest theorem-sized packet derives the fixed-`theta0` derivative joint
measurability field from full joint measurability of
`fun ((sample, observation), theta) => derivativeAt n sample observation theta`
by composing with the constant `theta0` map, on top of the canonical iid
product observation sequence, estimator-vector consistency, absolute-envelope
integrability, coordinate score/derivative, and covariance-source route.
...
second-derivative joint measurability, derivative smoothness/Taylor, root, or
envelope fields to a concrete model-specialized Theorem 5.41 instantiation.
Do not route back to fixed-`theta0` derivative joint measurability.
