...
Route from `Live In-Thread Goal Prompt V249` in
...
the outer second-RHS assembly.  The next target is finite-horizon RHS
aggregation plus the monotone finite-horizon-to-supremum limit; do not redo the Doob/Kolmogorov probability
...
calculus, or V248 fixed-`b` inner integral, or V249 second-RHS assembly.
...
Next proof packet: combine the V241 finite running-maximum bound, V242 first
RHS bridge, and V249 second-RHS assembly into the finite-horizon
`3 * E sqrt(A_infty)` bound, then connect the result to
`E(sup_n |X_n|) <= 3 E(A_infty^(1/2))`.  Search mathlib/local APIs first,
especially `lintegral_add`, `ENNReal` arithmetic for `1 + 2 = 3`,
`lintegral_const_mul'`, and monotone convergence APIs for the finite running
maxima.
