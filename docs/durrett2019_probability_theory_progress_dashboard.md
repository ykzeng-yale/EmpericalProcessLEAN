...
Route from `Live In-Thread Goal Prompt V41` in
...
next step is the exact liminf/limsup display for the oscillation side.
...
Next proof packet: continue Durrett Theorem 4.3.1 by deriving the textbook
`liminf X_n = -∞` and `limsup X_n = +∞` display from the compiled
threshold-form oscillation wrapper.
