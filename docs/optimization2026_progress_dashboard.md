...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after local sync to `origin/main` at `b4690bc`
  (`Add Chewi ellipsoid forward-shape transport theorem`) with the verified
  ...
  support, concrete displayed-to-normalized forward-shape transport, and
  real Haar/matrix image-volume scaling packets in
  ...
  `matrixInvShape_image_volume_real`.  The next target is exact ellipsoid
  affine-image/hvolume packaging for Lemma 6.20, while the exact
  Grünbaum/centroid theorem remains a supplied blocker for exact CoGM reporting.
