...
Route from `Live In-Thread Goal Prompt V30` in
...
supermartingales, and martingales.  The next frontier is Theorem 4.2.10
...
Next proof packet: package Theorem 4.2.10 upcrossing inequality.  Reuse
mathlib `Probability/Martingale/Upcrossing.lean` first, especially
`upcrossingStrat`, `upcrossingsBefore`,
`Submartingale.sum_upcrossingStrat_mul`, and
`Submartingale.mul_integral_upcrossingsBefore_le_integral_pos_part`; add the
exact Durrett positive-part-minus-initial display if direct, otherwise record
that display as the next narrow gap after the compiled estimate.
