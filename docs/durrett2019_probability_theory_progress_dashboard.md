...
Route from `Live In-Thread Goal Prompt V241` in
...
next target is deterministic/Fubini evaluation of the two RHS integrals plus
the monotone finite-horizon-to-supremum limit; do not redo the
Doob/Kolmogorov probability conversion, stopped-source packaging,
raw/stopped survival packaging, or V241 layer-cake handoff.
...
Next proof packet: evaluate the two V241 right-hand-side integrals by
deterministic/Fubini calculus and connect the result to
`E(sup_n |X_n|) <= 3 E(A_infty^(1/2))`.  Search mathlib/local APIs first,
especially `lintegral_lintegral_swap`, `lintegral_add`, `lintegral_const_mul`,
`ENNReal.ofReal`, square-root/rpow layer-cake, interval integral calculus for
`∫_{sqrt b}^∞ a^{-2} da`, and monotone convergence APIs for the finite
running maxima.
...
packaging, the V240 raw/stopped survival split, or the V241 layer-cake handoff
while this frontier is active.
