...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after the Chapter 12 source-rate packet and focused
  Lean/module verification of the Chewi Theorem 12.1 expected-model one-step
  bridge:
...
Current active target after focused Lean verification of the Chapter 12 RMS
analytic expected-lower-model wrappers: treat Chapters 3-8, Chapter 9/10
...
RMS wrappers: smooth `hcore`, variance domination from (12.1), and the
non-smooth bounded-gradient `hcore` from (12.2), then instantiate the compiled
RMS model-bound wrappers.  Keep the concrete finite Sinkhorn KL identity layer
as the next Chapter 11.8 blocker, but do not let it stall Chapter 12 coverage.
...
packets reuse local Gronwall/geometric-weight infrastructure and leave only
the Bochner/probability expected lower-estimate fields as the active formal
blocker.
