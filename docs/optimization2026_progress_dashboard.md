...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after the 2026-05-05 Theorem 5.10 source-rate packet:
  `StatInference/Optimization/Theorem510.lean` now proves the source-shaped
  discrete AGD rate
  `chewi510_gap_le_two_beta_dist_sq_over_nat_sq` after the weighted two-point
  inequality, finite telescope, and `lambda_N >= N / 2` denominator step, and
  the module is imported by `StatInference.lean`.  Chapter 5 CG is past the
  Krylov/termination/log-rate substrate, `Theorem58.lean` proves the AGF
  Lyapunov derivative formula and Theorem 5.8 wrapper, and `Theorem59.lean`
  proves a source-shaped Theorem 5.9 exponential AGF convergence wrapper.  The
  active target is now Chapter 6 nonsmooth convex optimization, starting with a
  projection/subgradient/PSD packet for Lemmas 6.12-6.13 and Theorem 6.14.
...
| Chapter 6 nonsmooth convex optimization | active-next | planned `StatInference/Optimization/ProjectedSubgradient.lean` | Active next theorem-sized packet: source-shaped finite-valued subgradient interface for Definition 6.8, projection interface for Definition 6.11, projection characterization/non-expansiveness wrappers for Lemmas 6.12-6.13, PSD trajectory for the display at markdown lines 1512-1517, and Theorem 6.14's average-gap bound. Search-first result: mathlib `Analysis/InnerProductSpace/Projection/Minimal.lean` has `exists_norm_eq_iInf_of_complete_convex` and `norm_eq_iInf_iff_real_inner_le_zero`; `Topology/MetricSpace/Lipschitz.lean` has `LipschitzOnWith`, `LipschitzOnWith.dist_le_mul`, and `LipschitzOnWith.le_add_mul`; local Chapter 3/5 finite-sum telescope and convex average-lower-bound patterns should be reused. Avoid starting with a heavy extended-real regular-convex foundation unless needed for a named theorem; first close Theorem 6.14 in a supplied-interface style. |
...
Latest proof target: Chapter 6 nonsmooth convex optimization, specifically the
projection/subgradient packet leading to Theorem 6.14 (convergence of projected
subgradient descent) around markdown lines 1458-1554.  Theorem 5.10 is now
closed in `StatInference/Optimization/Theorem510.lean` with the final
source-rate wrapper `chewi510_gap_le_two_beta_dist_sq_over_nat_sq`, so do not
spend the main lane on additional AGD algebra unless it is report packaging.
The next packet should create `ProjectedSubgradient.lean`, reuse mathlib's
projection minimizer/characterization APIs and `LipschitzOnWith`, then prove
the PSD one-step squared-distance inequality, finite average-gap bound, and
`h = R / sqrt N` corollary if the scalar algebra stays bounded.  The CG modules
