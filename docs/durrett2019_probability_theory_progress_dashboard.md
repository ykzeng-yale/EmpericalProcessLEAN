...
Route from `Live In-Thread Goal Prompt V106` in
...
measurability or full-prefix convergence.  The next target is final textbook
...
canonical ratio measurability, and the ENNReal full-prefix convergence upgrade.
Aim at one actual source-production obligation: promote the closed branch
criterion into the final textbook Kakutani statement, or remove only the
remaining ambient dichotomy / coordinate-finiteness assumptions when a direct
mathlib or local route is visible.  Search `withDensity`, `MutuallySingular`,
`AbsolutelyContinuous`, `LebesgueDecomposition`, `HasProd`, `Multipliable`,
`tprod`, and existing product-measure dichotomy endpoints.
