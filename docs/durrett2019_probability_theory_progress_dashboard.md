...
Route from `Live In-Thread Goal Prompt V223` in
...
The next target is the next martingale theorem/source wrapper unless a narrow
terminal-clock event bridge is needed to feed the already compiled
`hA_atTop_on` hypothesis.
