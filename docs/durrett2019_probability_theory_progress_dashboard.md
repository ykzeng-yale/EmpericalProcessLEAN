...
Route from `Live In-Thread Goal Prompt V124` in
...
The next target is the remaining `L^p` convergence endpoint
`eLpNorm (X n - ℱ.limitProcess X P) (ofReal p) P -> 0`.
...
`eLpNorm` wrapper also compile.  The first Theorem 4.4.6 bridge from a uniform
`L^p` martingale bound to the 4.2.11 almost-sure limit and limit-process
`MemLp` now also compiles.  Aim at one forward textbook packet: prove the final
Theorem 4.4.6 `L^p` convergence endpoint.
