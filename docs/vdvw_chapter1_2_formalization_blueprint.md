...
| 1.2.5 | Lemma | `..._1-100.md:467` | local-layer: measurable-target first/second coordinate product-cover transfers compiled as `VdVWMeasurableCover.fstProductOfMeasurable` and `.sndProductOfMeasurable`, plus probability-product outer-expectation invariance as `VdVWOuterExpectation_prod_fst_eq_of_measurable` and `VdVWOuterExpectation_prod_snd_eq_of_measurable`; full arbitrary-map perfect-projection theorem still pending |
