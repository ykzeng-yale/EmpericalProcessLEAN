...
- Current manual frontier: Chapter 5 CG is past the basic Krylov/termination
  ...
  Lyapunov continuity, and the source-facing Theorem 5.8 rate wrapper.  The
  active target is Theorem 5.9's strong-convex AGF Lyapunov proof, which the
  notes leave to Exercise 5.3.
...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean`, `StatInference/Optimization/Theorem54.lean`, `StatInference/Optimization/Theorem58.lean` | The Chapter 5 quadratic/CG substrate, Lemma 5.1 Krylov equality, Theorem 5.3 finite-dimensional termination-facing wrappers, and Theorem 5.4 descent/halving/log-rate layers now compile. The latest CG endpoint is the displayed restarted-CG specialization `chewi54_log_rate_of_displayed_cg_blocks_blockSize`. The Theorem 5.8 AGF lane now defines the AGF trajectory/friction interface, auxiliary point, Lyapunov function, Lyapunov-zero identity, gap derivative, auxiliary-point derivative, full Lyapunov derivative formula, auxiliary/Lyapunov continuity, Lyapunov-monotone-to-rate theorem, derivative-nonpositive-to-rate theorem, and the source wrapper `chewi58_gap_le_of_agf_firstOrderConvex` with continuity discharged internally. Search-first result: reused mathlib continuous linear maps, inner-product continuity, submodule/span APIs, finite-dimensional orthogonal-family APIs, scalar inverse/log APIs, local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers, Chapter 2 `antitoneOn_of_hasDerivWithinAt_nonpos`, Chapter 3 descent/telescoping helpers, Chapter 4 scalar recurrence/log bridges, and mathlib `HasDerivAt.smul`/`mul`/`norm_sq`, `ContinuousOn.smul`, and `ContinuousOn.pow` APIs. Next target: Theorem 5.9 strong-convex AGF. Since Chewi leaves the proof to Exercise 5.3, first formalize the standard Lyapunov proof route and then expose the theorem statement. |
