...
## Live In-Thread Goal Prompt V149
...
tail-bound-to-`L^2` / summability-to-shifted-tail plumbing.

Next aggressive step: finish the Exercise 4.4.10 handoff from finite tail
identity to convergence.  Best target: prove the shifted infinite tail tends
to zero under the square-increment `Summable` hypothesis and package the
result as an explicit `L^2` Cauchy statement, reusing
`durrett2019_exercise_4_4_10_martingale_eLpNorm_increment_le_tsum_tail_of_summable`
and existing convergence wrappers where possible.  If that stalls, move to
Exercise 4.4.11 by combining predictable-transform theorem 4.2.8 support with
the square-summability hypothesis for scaled increments.  Do not loop back to
...
tail-bound-to-`L^2`/summability-to-shifted-tail layer.  Theorem 4.1.16 remains
