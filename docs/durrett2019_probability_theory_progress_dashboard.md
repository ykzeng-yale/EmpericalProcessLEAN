...
Route from `Live In-Thread Goal Prompt V43` in
...
finite convergence or `liminf = ⊥` and `limsup = ⊤`.  The next step is
the uniqueness side of Theorem 4.3.2.  The existence/formula side of Doob's
decomposition now compiles over mathlib `predictablePart` and
`martingalePart`: a submartingale is the sum of a martingale and a predictable
increasing process starting at zero, with Durrett's finite-sum formula.
...
oscillation, exact `EReal` liminf/limsup wrappers, and Theorem 4.3.2
Doob-decomposition existence/formula wrapper.
...
Next proof packet: finish Durrett Theorem 4.3.2 by packaging the uniqueness
side of Doob's decomposition with `martingalePart_add_ae_eq` and
`predictablePart_add_ae_eq`, then move to Example 4.3.3 if the uniqueness
wrapper is immediate.
