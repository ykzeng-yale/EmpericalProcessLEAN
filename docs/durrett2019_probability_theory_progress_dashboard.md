...
Route from `Live In-Thread Goal Prompt V210` in
...
`ΔA/f(A_{n+1})^2 ≤ ∫_[A_n,A_{n+1}] f(t)^{-2} dt` estimate.  The next target is
the pathwise finite-sum lift and variance-ratio summability package for
`b_n = f(A_n)`, using V204-V210 as closed support.
