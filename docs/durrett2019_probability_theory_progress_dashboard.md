...
Route from `Live In-Thread Goal Prompt V165` in
...
predictable-part identity `E X_n^2 = E A_n`, and the terminal
monotone-limit handoff to `E A_n ≤ E A∞`.  The next target is the exact
conditional-variance finite-sum display for Durrett's increasing process, or
Theorem 4.5.2 if that display is not immediately cheap.
The V143-V165 packets added `durrett2019_exercise_4_4_6_varianceClock_succ`,
