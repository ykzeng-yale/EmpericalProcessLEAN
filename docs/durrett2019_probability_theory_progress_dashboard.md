...
Route from `Live In-Thread Goal Prompt V52` in
...
derivative sequences.  The next target is an actual source-production
obligation: prove boundedness/convergence of those sequences, or prove
`X = Y/Z` / `{X = infinity}` singular separation.
...
generator-production bridge, and trimmed-RN eventual restricted-density bridge.
Aim at one actual source-production obligation: prove boundedness/convergence
of the trimmed RN derivative sequences, or prove `X = Y/Z` /
`{X = infinity}` singular separation.  Search RN derivative bounds from
measure domination, measure addition/scaling APIs, nonnegative/bounded
martingale convergence wrappers, a.e. transfer, ENNReal limsup/top, and
