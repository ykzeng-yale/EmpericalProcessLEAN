...
| Chapter 5 acceleration and conjugate gradient | local-layer | `StatInference/Optimization/ConjugateGradient.lean` | The Chapter 5 quadratic substrate now starts from the source display `f(x)=1/2 <x, A x> - <b, x>` with `quadraticObjective`, `quadraticGradient`, supplied symmetry and quadratic-form lower/upper bounds, exact expansion `quadraticObjective_eq_model_add_quadratic`, first-order strong-convexity and smoothness wrappers, and the minimizer bridge `quadraticObjective_isMinOn_of_apply_eq`. The newest layer adds `aInner`/`aNormSq`, Krylov vectors/subspaces, direction spans, monotonicity and `A`-image lemmas, the supplied-interface Lemma 5.1 theorem `IsCGKrylovRecurrence.cgDirectionSubmodule_eq_krylovSubmodule`, and the Theorem 5.3-facing residual exactness wrapper `quadraticObjective_isMinOn_of_cgResidualExactnessState`. Search-first result: reused mathlib continuous linear maps, `continuous_id.inner`, `Continuous.inner`, `real_inner_comm`, `Function.iterate_succ_apply'`, `Submodule.span_induction`, `Submodule.span_mono`, and local `FirstOrderStrongConvexOn`/`SmoothWithGradientOn`/minimizer wrappers; defer full spectral/self-adjoint API integration until a theorem needs it. Next target: prove the concrete CG recurrence fields from line-search/Gram-Schmidt formulas, then package finite-dimensional termination and Theorem 5.4's descent/orthogonality/halving proof. |
...
layer has the Krylov equality and residual-exactness supplied interfaces.  The
next aggressive step is to derive those interface fields from the concrete CG
line-search/Gram-Schmidt formulas and then package finite-dimensional
termination.  Do not replay Chapter 3 GD convergence or Chapter 4 hard-instance
