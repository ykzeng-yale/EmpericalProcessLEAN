...
Route from `Live In-Thread Goal Prompt V103` in
...
strict-positive product variants.  The next target is the remaining
canonical-ratio nonzero-lower-integral input, the positive/zero infinite
Hellinger product source criterion, or the final zero/positive product case
split.
...
positive-product finite-limit support, and canonical product-tail/`tprod`
positive-product support, including strict-positive product variants.  Aim at
one actual source-production obligation: discharge the canonical-ratio
nonzero-lower-integral input, prove the positive/zero infinite Hellinger product
source criterion, or perform the final zero/positive product case split consumed
by the compiled branch assemblers.  Search `lintegral_ne_zero`,
`lintegral_eq_one`, `HasProd`, `Multipliable`, `tprod`, `Tendsto`,
