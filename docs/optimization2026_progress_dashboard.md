...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier: Chapter 5 CG is past the basic Krylov/termination
  setup and `Theorem58.lean` now proves the AGF Lyapunov derivative formula,
  Lyapunov continuity, and the source-facing Theorem 5.8 rate wrapper.
  `Theorem59.lean` now proves a source-shaped Theorem 5.9 exponential AGF
  convergence wrapper from first-order strong convexity, the strong-convex AGF
  ODE, minimizer stationarity, and `p 0 = 0`.  The active target is now
  Theorem 5.10 discrete AGD after a source/API audit of the one-step inequality
  `(3.3)`, lambda recurrence, and telescoping algebra.
...
Latest proof target: Chapter 5 acceleration.  The AGF modules
