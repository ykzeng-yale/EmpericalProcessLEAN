...
Route from `Live In-Thread Goal Prompt V597` in
...
nonnegative `lintegral` product-expectation branch of Theorem 2.1.13; and
V478 adds the nonnegative separated product-measure and independent-pair
product-expectation branch of Theorem 2.1.12; V479 adds the law-side
nonnegative and integrable separated-product branch of Theorem 2.1.12; V480
...
probability wrappers for Theorem 2.1.11; V484 adds one-based finite-prefix
...
canonical infinite-product coordinates; V486 adds the general/non-iid
finite-prefix product-law and joint infinite-product extraction wrappers; V487
...
arbitrary finite-index-set product-law and cylinder probability wrappers from
joint infinite-product law hypotheses; V490 adds canonical iid arbitrary
finite-index-set product-law and cylinder probability wrappers for the
infinite product coordinate process; V491 adds one-based finite-prefix
cylinder displays from joint infinite-product laws and canonical coordinates
with prefix-indexed cylinder sets; V492 adds reusable event-independence
extraction wrappers from `iIndepFun`, joint `Measure.infinitePi` laws, and
canonical product coordinates; V493 specializes those event-independence
wrappers to closed/open real half-line events for empirical-CDF and left-limit
consumers; V494 specializes finite-prefix cylinder probability wrappers to the
same closed/open half-line events over `Finset.range n`, for source,
joint-law, and canonical iid displays; V495 adds the matching literal
one-based `{1, ..., n}` half-line cylinder displays over `Finset.Icc 1 n`,
including identical-distribution source wrappers; V496 promotes the closed
and open half-line displays to real-valued CDF and CDF-left-limit finite
...
matching Theorem 2.1.13 expectation-factorization and zero-mean corollaries
in zero-based and one-based notation.  V505 adds source-shaped Durrett Theorem
...
expectation-exists-and-power-value wrappers for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing common transformed-product
power formulas to `IdentDistrib`, source independence, measurability, and one
integrable transformed base marginal, without `HasLaw` hypotheses.
V536 adds source-side iid/common-transform composed Chapter 2.1.13
zero-factor expectation-exists wrappers for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing common transformed-product
zero formulas to `IdentDistrib`, source independence, measurability, one
integrable transformed base marginal, and one zero transformed base mean,
without `HasLaw` hypotheses.
V537 adds source-side iid/indexed-transform composed Chapter 2.1.13
expectation-exists-and-value wrappers for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing mixed transformed-product
factorizations to `IdentDistrib`, source independence, measurability, and
base-coordinate integrability for each indexed transform, without `HasLaw`
hypotheses.
V538 adds source-side iid/indexed-transform composed Chapter 2.1.13
zero-factor expectation-exists wrappers for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing mixed transformed-product
vanishing to `IdentDistrib`, source independence, measurability,
base-coordinate integrability for each indexed transform, and one selected
zero base transformed expectation, without `HasLaw` hypotheses.
V539 adds law-side iid/indexed-transform Chapter 2.1.13
expectation-exists-and-value wrappers for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing mixed transformed-product
factorizations under common-law `HasLaw` hypotheses to source independence,
measurability, law-side integrability for each indexed transform, and
products of law-side expectations.
V540 adds law-side iid/indexed-transform Chapter 2.1.13 zero-factor
expectation-exists wrappers for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing mixed transformed-product
cancellations under common-law `HasLaw` hypotheses to source independence,
measurability, law-side integrability for each indexed transform, and one
zero law-side factor expectation.
V541 adds law-side iid/indexed-transform Chapter 2.1.13 nonnegative
`lintegral` product formulas for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing mixed nonnegative
...
range/Ico/`Finset.Icc 1 n` products, reducing mixed real-valued nonnegative
product cancellations under common-law `HasLaw` hypotheses to source
independence, measurability, law-side nonnegativity and measurability for each
indexed transform, and one zero law-side `ENNReal.ofReal` factor `lintegral`.
V544 adds law-side iid/indexed-transform Chapter 2.1.13 nonnegative `ℝ≥0∞`
zero-factor formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing mixed nonnegative product cancellations
under common-law `HasLaw` hypotheses to source independence, measurability,
law-side measurability for each indexed transform, and one zero law-side
factor `lintegral`.
V545 adds source-side iid/indexed-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` product formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing mixed nonnegative transformed-product
factorizations under `IdentDistrib` to source independence, measurability for
each indexed transform, and products of base-coordinate `lintegral`s, without
...
`ENNReal.ofReal` zero-factor formulas for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing mixed real-valued nonnegative
transformed-product cancellations under `IdentDistrib` to source
independence, measurability and nonnegativity for each indexed transform, and
one zero base-coordinate `ENNReal.ofReal` `lintegral`, without `HasLaw` or
...
factorizations to source independence plus measurability of `X_i` and `f_i`,
...
`ENNReal.ofReal` zero-factor formulas for finite/range/Ico and one-based
range/Ico/`Finset.Icc 1 n` products, reducing composed real-valued
nonnegative transformed-product cancellations to source independence,
...
zero-factor formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing composed norm-product cancellations to
source independence, measurability of `X_i` and `f_i`, and one zero composed
norm factor `lintegral`, without `HasLaw` or ordinary integrability
assumptions.
V555 adds source-side iid/indexed-transform Chapter 2.1.13 absolute-value/norm
product formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing indexed norm-product factorizations under
`IdentDistrib` to source independence, measurability of `X_i` and `f_i`, and
base-coordinate norm `lintegral`s over `X_0`, without `HasLaw` or ordinary
integrability assumptions.
V556 adds source-side iid/indexed-transform Chapter 2.1.13 absolute-value/norm
zero-factor formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing indexed norm-product cancellations under
`IdentDistrib` to source independence, measurability of `X_i` and `f_i`, and
one zero base-coordinate norm `lintegral` over `X_0`, without `HasLaw` or
ordinary integrability assumptions.
V557 adds source-side iid/common-transform Chapter 2.1.13 absolute-value/norm
power formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing common transformed norm-product
factorizations under `IdentDistrib` to source independence, measurability of
`X_i` and `f`, and powers of the base-coordinate norm `lintegral` over `X_0`,
without `HasLaw` or ordinary integrability assumptions.
V558 adds source-side iid/common-transform Chapter 2.1.13 absolute-value/norm
zero-factor formulas for nonempty finite products and positive range/Ico and
one-based range/Ico/`Finset.Icc 1 n` products, reducing common transformed
...
`ℝ≥0∞` zero-factor formulas for nonempty finite products and positive
range/Ico and one-based range/Ico/`Finset.Icc 1 n` products, reducing common
transformed nonnegative product cancellations under `IdentDistrib` to source
independence, measurability of `X_i` and `f`, and one zero base-coordinate
`lintegral` over `X_0`, without `HasLaw` or ordinary integrability
assumptions.
V561 adds source-side iid/common-transform Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` power formulas for finite/range/Ico and one-based range/Ico/
`Finset.Icc 1 n` products, reducing common transformed `ENNReal.ofReal`
product factorizations under `IdentDistrib` to source independence,
measurability and nonnegativity of `f`, and powers of the base-coordinate
`ENNReal.ofReal` `lintegral` over `X_0`, without `HasLaw` or ordinary
integrability assumptions.
V562 adds source-side iid/common-transform Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` zero-factor formulas for nonempty finite products and
positive range/Ico and one-based range/Ico/`Finset.Icc 1 n` products,
reducing common transformed `ENNReal.ofReal` product cancellations under
`IdentDistrib` to source independence, measurability and nonnegativity of
`f`, and one zero base-coordinate `ENNReal.ofReal` `lintegral` over `X_0`,
without `HasLaw` or ordinary integrability assumptions.
V563 adds law-side iid/common-law Chapter 2.1.13 nonnegative `ℝ≥0∞`
zero-factor formulas for nonempty finite products and positive range/Ico and
one-based range/Ico/`Finset.Icc 1 n` products, reducing common-law
nonnegative product cancellations under `HasLaw` hypotheses to source
independence, measurability of `X_i` and `f`, and one zero common law-side
`lintegral` over `μ`.
V564 adds law-side iid/common-law Chapter 2.1.13 real nonnegative
`ENNReal.ofReal` zero-factor formulas for nonempty finite products and
positive range/Ico and one-based range/Ico/`Finset.Icc 1 n` products,
reducing common-law real nonnegative product cancellations under `HasLaw`
hypotheses to source independence, measurability and nonnegativity of `f`,
and one zero common law-side `ENNReal.ofReal` `lintegral` over `μ`.
V565 adds law-side Chapter 2.1.13 absolute-value/norm `lintegral`
factorization and iid/common-law power formulas for finite products and
range/Ico and one-based range/Ico/`Finset.Icc 1 n` products, transferring
...
V569 adds non-atomic Durrett Theorem 2.4.9 final-display wrappers for the
...
V571 adds the matching common-law `HasLaw` Chapter 2.1.13 wrappers: a
law-side integrable zero mean now supplies zero-based and one-based future
...
V512 adds the one-based source-shaped Theorem 2.4.9
...
V513 adds the one-based source-shaped Theorem 2.4.9
...
identical-distribution, pairwise-identically-distributed, and canonical iid
source shapes.
V515 adds the arbitrary-tolerance one-based source-shaped Theorem 2.4.9
`middlePartitionWithTails_oneBased_*_uniform_error_lt` raw/range
global-squeeze families for source, joint-law, shifted-joint-law,
identical-distribution, pairwise-identically-distributed, and canonical iid
source shapes.
V516 adds the one-based route-named Theorem 2.4.9
...
pairwise-identically-distributed, and canonical iid source shapes.
V517 adds the zero-based route-named Theorem 2.4.9
...
V518 adds the one-based Chapter 2.1.10/2.1.13 Kolmogorov-support
late-increment/early-block independence and mixed-term-zero wrappers.
V519 adds the one-based Chapter 2.1.13 range/Ico product-expectation wrappers,
...
V520 adds the one-based law-side Chapter 2.1.13 range/Ico product-expectation
...
V524 adds source-side Chapter 2.1.13 absolute-value/norm product displays and
...
products.
V545 adds source-side iid/indexed-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` product formulas for finite/range/Ico and one-based range/Ico/`Icc`
products.
V546 adds source-side iid/indexed-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` zero-factor formulas for finite/range/Ico and one-based range/Ico/
`Icc` products.
...
products.
V560 adds source-side iid/common-transform Chapter 2.1.13 nonnegative
`ℝ≥0∞` zero-factor formulas for nonempty finite, positive range/Ico, and
...
plumbing or the closed V500-V597 2.4.9/2.1 source-wrapper surfaces unless an
