...
Current frontier: Theorem 5.41 now has a compiled unscaled score source
endpoint
...
It defines the auxiliary `scoreAtTheta0` internally as
`sqrt n • scoreTransform` and derives both scaled score-display fields from
the unscaled textbook score identity.  Its immediate predecessor is the fixed
true-parameter source endpoint
...
It specializes the route to the textbook fixed `theta0`, removes the
random-`theta0` measurability source, and discharges it internally by
`measurable_const`. Earlier source endpoints remain recorded below; the live
continuation should not replay them.
...
current route. The next proof packet should continue from this endpoint to a
concrete model-specialized Theorem 5.41 instantiation, or package only a
still-live source field that the current endpoint explicitly exposes, not
replay display weak-convergence,
