...
Route from `Live In-Thread Goal Prompt V122` in
...
The next target is proving the monotone-convergence/iSup handoff from bounded
truncations to the original running maximum, then feeding the final
positive-part and martingale `p/(p-1)` source wrappers.
...
truncated `lintegral` estimate also compiles.  Aim at one forward textbook
packet: prove the monotone-convergence/iSup handoff from bounded truncations to
the original running maximum and package the final positive-part p-th-power
estimate.
