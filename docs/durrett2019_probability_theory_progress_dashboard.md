...
Route from `Live In-Thread Goal Prompt V73` in
...
absolute-continuity branch.  The pairwise-liminf Cauchy-to-L1 handoff also now
compiles: pairwise L1 tail control plus pointwise convergence supplies that L1
input and hence the same absolute-continuity branch.  The next target is the
actual Hellinger tail-product bound proving the pairwise-liminf hypothesis,
