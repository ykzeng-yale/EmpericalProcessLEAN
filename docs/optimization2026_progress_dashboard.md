...
iterate.  The next aggressive step is the geometric minimizer/tail lower bound
and the actual strongly-convex gap/rate assembly.
...
Next high-value task: construct or supply the geometric zero-gradient
minimizer for `strongLowerBoundChainObjective`, prove a lower bound on its
`coordinateTailSq` beyond `V_N`, then feed that into
`strongLowerBoundChainObjective_gap_ge_tailSq_of_gradientSpanTrajectory`.
Search mathlib/local APIs for finite geometric sums, recurrences, logarithm,
and asymptotic iteration-count wrappers before adding any local complexity
primitive.  Keep any exercise work opportunistic and centralized in
