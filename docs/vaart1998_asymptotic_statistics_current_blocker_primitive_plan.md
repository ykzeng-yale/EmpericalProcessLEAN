...
vector-valued observation integrability plus a single vector-valued
...
`vaart1998_vector_integral_zero_of_coordinate_mean_zero`, and rebuilds full
derivative-transform measurability plus full finite-dimensional measurability
of `Z` via `measurable_pi_lambda`; do not replace the `Z` step with
`HasGaussianLaw.aemeasurable`, which gives only `AEMeasurable Z Q`. It derives
...
`vaart1998_integral_transform_eq_integral_id_map`. It rebuilds coordinatewise
score and derivative measurability via `continuous_apply`, rebuilds
coordinatewise derivative integrability via
`vaart1998_derivativeTransform_coordinate_integrable_of_vector_integrable`,
then rebuilds coordinatewise derivative basis-action means via
`vaart1998_derivativeTransform_observation_basis_action_of_vector_mean` and
the pushed-forward derivative-law mean display using `MeasureTheory.eval_integral`
and `vaart1998_integral_transform_eq_integral_id_map`. Do not route back to
direct observation-level covariance equality, vector-valued score-transform
...
full derivative-transform measurability, derivative-integrability sources,
abstract score-law moment assumptions, abstract score-law covariance-bilinear
assumptions, pushed-forward score-transform all-dual covariance-bilinear
