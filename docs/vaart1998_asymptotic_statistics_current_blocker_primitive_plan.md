...
This endpoint replaces the auxiliary `scoreAtTheta0` object and both scaled
score-display hypotheses by the textbook unscaled score identity
`estimatingMap ... theta0 = scoreTransform ...`.
...
1. Move one layer closer to a model-facing finite-dimensional statement:
   continue from the unscaled score fixed-`theta0` endpoint toward a concrete
