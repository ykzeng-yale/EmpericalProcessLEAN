...
Route from `Live In-Thread Goal Prompt V215` in
...
lower bound `1 <= f(A_n)` from the interval source lower bound.  The next
target is to discharge the source clock-integral, ratio integrability, and
reciprocal-transform `MemLp` side conditions.
