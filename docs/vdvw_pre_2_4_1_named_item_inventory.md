...
| 1.3.2 Lemma | `M1:582` | pre-tightness, separability, tightness | local-layer/mathlib-foundation: measure-level separability and pre-tightness predicates introduced as `VdVWProbabilityMeasureSeparable` and `VdVWProbabilityMeasurePreTight`; pre-tight measures concentrate on a countable union of totally bounded sets via `VdVWProbabilityMeasurePreTight.exists_iUnion_totallyBounded_measure_compl_eq_zero` and are separable via `VdVWProbabilityMeasurePreTight.separable`/`vdVW132_probabilityMeasure_separable_of_preTight`; separable measures have finite high-mass ball-prefix covers via `VdVWProbabilityMeasureSeparable.exists_finitePrefix_ball_cover_measure_compl_le`/`vdVW132_probabilityMeasure_exists_finitePrefix_ball_cover_measure_compl_le_of_separable` and pre-tightness via `VdVWProbabilityMeasureSeparable.exists_iInter_finitePrefix_ball_cover_measure_compl_le`, `VdVWProbabilityMeasureSeparable.preTight`, `vdVW132_probabilityMeasure_preTight_of_separable`, and `vdVW132_probabilityMeasure_preTight_iff_separable`; tight families give σ-compact full-measure support via `VdVWProbabilityMeasuresTight.exists_iUnion_isCompact_measure_compl_eq_zero` and tight probability measures are separable via `VdVWProbabilityMeasureSeparable.of_tight`/`vdVW132_probabilityMeasure_separable_of_tight`; separable ambient spaces give `VdVWProbabilityMeasureSeparable.of_separableSpace`, tight probability measures give `VdVWProbabilityMeasurePreTight.of_tight`, and complete separable metric-type probability measures are packaged by `vdVW132_complete_separable_probabilityMeasure_tight`, `vdVW132_complete_separable_probabilityMeasure_separable_and_tight`, and `vdVW132_complete_separable_probabilityMeasure_preTight`; full complete-space equivalence and Polish-measure clauses still pending |
