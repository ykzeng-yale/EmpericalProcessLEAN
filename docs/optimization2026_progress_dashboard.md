...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after the 2026-05-05 Theorem 6.14 source packet:
  ...
  displayed `h = R / sqrt N` corollary, and scaled-step recurrence for the
  functional-constraint method.  The active target is now Theorem 6.16:
  formalize the functional-constraint PSD interface, prove the two case
  decrease lemmas, and assemble the finite contradiction proof of (6.5).
...
| Chapter 6 nonsmooth convex optimization | local-layer | `StatInference/Optimization/ProjectedSubgradient.lean` | The Chapter 6 PSD layer now compiles through a source-complete finite-valued Theorem 6.14 packet. It defines `IsSubgradientAt`, `ProjectionCharacterizationOn`, `ProjectionOracleOn`, `normalizedSubgradient`, `projectedSubgradientStep`, `projectedSubgradientScaledStep`, `IsProjectedSubgradientTrajectory`, and `iterateAverage`; proves `IsSubgradientAt.gap_le_inner`, `ProjectionCharacterizationOn.dist_to_set_le`, `ProjectionCharacterizationOn.fixed`, `ProjectionCharacterizationOn.nonexpansive`, `ProjectionCharacterizationOn.toProjectionOracleOn`, `normalizedSubgradient_norm`, `normalizedSubgradient_inner_self`, `IsSubgradientAt.norm_le_of_lipschitzOnWith_feasible_ray`, Jensen wrappers for the averaged iterate, the scaled-step recurrence `projectedSubgradient_scaled_sqdist_recurrence`, the PSD squared-distance recurrence `projectedSubgradient_sqdist_recurrence`, the finite telescope `projectedSubgradient_gap_average_bound_of_recurrence`, Theorem 6.14 supplied-interface wrapper `chewi614_average_gap_bound`, Lipschitz/ray wrapper `chewi614_average_gap_bound_of_lipschitzOnWith_feasible_rays`, displayed-step wrapper `chewi614_average_gap_bound_stepsize`, and combined source-rate wrapper `chewi614_average_gap_bound_of_lipschitzOnWith_feasible_rays_stepsize`. Search-first result: reused mathlib `ConvexOn.map_sum_le`, `LipschitzOnWith.le_add_mul`, `Real.coe_toNNReal`, `Real.sqrt_pos`, `Real.sq_sqrt`, local `sum_range_sub_succ`, `norm_add_sq_real`/`norm_sub_sq_real`, `real_inner_le_norm`, and scalar `field_simp`/`nlinarith`. Current aggressive packet: Theorem 6.16 functional constraints, using the compiled scaled-step recurrence to prove the objective-case and constraint-case distance decreases, then the finite contradiction argument for (6.5). |
