...
truncated pair-difference measurability, real-valued envelope-tail
outer-expectation/probability handoffs, generic ordinary tail cutoff
convergence, and proof-carrying product-coordinate/map independence wrappers
are closed.  Next build the product/Fubini-compatible symmetrization interface
targeted to the `Phi(x)=x` case, reusing
`StatInference/ProbabilityMeasure/ProductMeasure.lean`,
`probability_prod_independent_mapped_copies_with_joint_law`, the existing
