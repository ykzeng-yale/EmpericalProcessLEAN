...
This endpoint removes explicit envelope nonnegativity by accepting the
domination bound with `|envelope x|` and feeding the previous endpoint the
nonnegative absolute envelope internally.
...
1. Move one layer closer to a model-facing finite-dimensional statement:
   continue from the absolute-envelope unscaled-score fixed-`theta0` endpoint toward a concrete
...
Latest verified Vaart frontier before the next packet: the full-domain
smoothness source endpoint for Vaart Theorem 5.41.
