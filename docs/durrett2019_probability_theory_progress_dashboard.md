...
Route from `Live In-Thread Goal Prompt V101` in
...
wrapper with that convergence supplied now also compile.  The next target is the
remaining canonical-ratio nonzero-lower-integral/product inputs.
...
trimmed-prefix RN-ratio, denominator-limit nonzero, and canonical prefix
convergence support.  Aim at one actual source-production obligation: discharge
the canonical-ratio nonzero-lower-integral input or the final product/nonzero
hypotheses consumed by the compiled branch assemblers.  Search `lintegral_ne_zero`,
`lintegral_eq_one`, `HasProd`, `Tendsto`, `withDensity`, and existing
finite-prefix mass/cylinder likelihood endpoints.  Do not re-prove the compiled
Chapter 4.3.5 or 4.3.8 assembly layers.
