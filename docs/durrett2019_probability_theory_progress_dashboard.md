...
Route from `Live In-Thread Goal Prompt V27` in
...
frontier is Theorem 4.2.7 increasing-convex transforms.
...
Next proof packet: package Theorem 4.2.7 increasing-convex transforms for real
submartingales using conditional Jensen plus the compiled one-step
submartingale inequality.  If direct, add the `(X_n - a)^+` and `X_n ∧ a`
consequences.
