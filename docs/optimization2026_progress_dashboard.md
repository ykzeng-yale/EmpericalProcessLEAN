...
Latest proof target: Chapter 4 Theorem 4.5 from Lemma 4.2 plus Theorem 4.4.  The
...
lower-bound foundation work; the next aggressive step is to connect the
compiled Theorem 4.4 lower-bound lane to Theorem 4.5.
...
Next high-value task: derive a source-shaped strongly-convex lower-bound
wrapper for Theorem 4.5 by combining `Reductions.lean`'s Lemma 4.2 package
with `LowerBounds.lean`'s compiled Theorem 4.4 finite-dimensional gap.  Search
mathlib/local APIs for logarithm/asymptotic/iteration-count wrappers before
adding any local complexity primitive.  Keep any exercise work opportunistic
and centralized in `StatInference/Optimization/Exercises.lean`; the main
theorem lane remains the priority.
