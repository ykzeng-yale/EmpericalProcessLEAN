...
3. Promote Chapter 2 primitive infrastructure: covering/packing semimetric
   ...
   outer-expectation projection for the expectation-level route.  The next
   frontier is the outer-probability entropy-to-Hoeffding-scale convergence
   handoff and final assembly; deterministic Hoeffding-scale algebra and fixed
   entropy projection helpers are now compiled.
