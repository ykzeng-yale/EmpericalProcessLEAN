...
Route from `Live In-Thread Goal Prompt V141` in
...
the square-martingale source from increments and the `var(S_n) = s_n^2`
endpoint feeding the compiled deterministic variance-clock wrapper.
...
wrapper now also compile.  The next target is the remaining exact Exercise
4.4.6 source instantiation.
...
compile, along with the deterministic variance-clock wrapper.  Aim at one
forward textbook packet: instantiate the square-martingale source from
increments and the `var(S_n) = s_n^2` endpoint for the exact Exercise 4.4.6
source wrapper, or move to a nearby Chapter 4.4 theorem-sized packet if that
route is blocked.
