...
| `VdVWTheorem243_fixedM_centered_truncated_convergesInOuterProbabilityConst_zero_of_logCardinality_div_bounded_invRadius` | 2026-05-03 update: the fixed-`M` bounded entropy/measurable-cardinality route now has a canonical shrinking-radius consumer for covers at radius `1 / (n + 1)`, discharging nonnegativity and `coverRadius -> 0` internally.  The remaining side conditions are diagonal variable-domain entropy for that selected radius, measurable cardinality, and deterministic boundedness or an explicit variable-domain UI/tail-expectation replacement. |
| `measurable_terminal_minimalRandomEmpiricalL1CoveringNumberCard_of_countable_of_measurable`, `measurable_selected_randomEmpiricalL1CoveringNumberCard_at_sampleSize_of_countable_of_measurable`, `measurable_selected_truncatedRandomEmpiricalL1CoveringNumberCard_at_sampleSize_of_countable` | 2026-05-03 update: selected terminal minimal-cardinality measurability is now packaged for countable class-coordinate-measurable classes, including the theorem-facing truncated class.  The remaining blocker is feeding the theorem finite-cover/entropy hypotheses into these wrappers, plus the log-ratio bound or UI replacement and diagonal radius selection. |
