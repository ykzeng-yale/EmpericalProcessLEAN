...
1. Tightness source: propagate the absorbing coordinate path Taylor endpoint
   into the actual estimating-map and finite-derivative source wrappers,
...
(`Add Vaart theorem 5.41 absorbing coordinate path endpoint`).
...
scaled-estimator tightness first.  The newest coordinate path wrapper derives
the finite-coordinate raw Taylor identities from scalar path Taylor assumptions,
common scaling relations, and an envelope bound.
...
coordinate path Taylor endpoint, or to a genuinely model-specific Taylor
...
absorbing-coordinate-raw-endpoint, or absorbing-coordinate-path-endpoint
