...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the textbook sqrt-scale endpoint to a concrete
...
2. Derivative or score source: only add a model-specific coordinate/matrix or
   ...
   envelope-mean wrappers, or sqrt-scale wrappers.
...
(`Add Vaart observation score covariance endpoint`).

The latest theorem-sized packet derives score-law covariance under
`observationLaw.map scoreTransform` from the observation-level score-transform
covariance identity, after deriving the observation-level population `V`
basis-action source and pushforward score and derivative moment fields.
...
observation score covariance endpoint to a concrete model-specialized
Theorem 5.41 instantiation, or instantiate one remaining
smoothness/root/envelope field.
