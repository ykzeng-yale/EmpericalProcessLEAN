...
/--
Durrett 2019, Theorem 4.5.1 monotone-convergence support: on a path where the
finite running absolute maxima are bounded above, the nonnegative `ENNReal`
`iSup` of their squares is exactly the square of the canonical real running
absolute supremum.
-/
theorem durrett2019_iSup_ofReal_runningAbsMax_sq_eq_ofReal_runningAbsSup_sq_of_bddAbove
    {Ω : Type*} {X : ℕ -> Ω -> ℝ} {ω : Ω}
    (hBdd :
      BddAbove (Set.range fun n => durrett2019_runningAbsMax X n ω)) :
    (⨆ n : ℕ, ENNReal.ofReal (durrett2019_runningAbsMax X n ω ^ 2)) =
      ENNReal.ofReal (durrett2019_runningAbsSup X ω ^ 2) := by
  let a : ℕ -> ℝ := fun n => durrett2019_runningAbsMax X n ω
  have hmono_a : Monotone a :=
    durrett2019_runningAbsMax_mono (X := X) ω
  have hmono_sq : Monotone fun n => ENNReal.ofReal (a n ^ 2) := by
    intro n m hnm
    refine ENNReal.ofReal_le_ofReal ?_
    exact
      (sq_le_sq₀
  ...
  have htend_a : Tendsto a atTop (𝓝 (durrett2019_runningAbsSup X ω)) := by
    simpa [a] using
      durrett2019_runningAbsMax_tendsto_runningAbsSup_of_bddAbove
        (X := X) (ω := ω) hBdd
  have htend_sq :
      Tendsto (fun n => a n ^ 2) atTop
        (𝓝 (durrett2019_runningAbsSup X ω ^ 2)) := by
    simpa [pow_two] using htend_a.mul htend_a
  have htend_ofReal :
      Tendsto (fun n => ENNReal.ofReal (a n ^ 2)) atTop
        (𝓝 (ENNReal.ofReal (durrett2019_runningAbsSup X ω ^ 2))) :=
    ENNReal.continuous_ofReal.continuousAt.tendsto.comp htend_sq
  have htend_iSup :
      Tendsto (fun n => ENNReal.ofReal (a n ^ 2)) atTop
        (𝓝 (⨆ n : ℕ, ENNReal.ofReal (a n ^ 2))) :=
    tendsto_atTop_iSup hmono_sq
  have heq := tendsto_nhds_unique htend_iSup htend_ofReal
  simpa [a] using heq

/--
Durrett 2019, Theorem 4.5.1 monotone-convergence support: the previous
pointwise `iSup` identification can be used inside a `lintegral`.
-/
theorem durrett2019_lintegral_iSup_runningAbsMax_sq_eq_lintegral_runningAbsSup_sq_of_ae_bddAbove
    {Ω : Type*} [MeasurableSpace Ω] {P : Measure Ω}
    {X : ℕ -> Ω -> ℝ}
    (hBdd :
      ∀ᵐ ω ∂P,
    ...
    (∫⁻ ω,
        ⨆ n : ℕ,
          ENNReal.ofReal (durrett2019_runningAbsMax X n ω ^ 2) ∂P) =
      ∫⁻ ω, ENNReal.ofReal (durrett2019_runningAbsSup X ω ^ 2) ∂P := by
...
/--
Durrett 2019, Theorem 4.5.1 monotone-convergence support: after the pointwise
`iSup` is identified with the canonical running supremum square, the uniform
terminal second-moment bound controls the `lintegral` of that square.
-/
theorem durrett2019_theorem_4_5_1_lintegral_runningAbsSup_sq_le_of_terminal_integral_sq_le
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] {ℱ : Filtration ℕ mΩ}
    {X : ℕ -> Ω -> ℝ} (hX : Martingale X ℱ P)
    {C : ℝ}
...
/--
Durrett 2019, Theorem 4.5.2 automatic bounded-running-maximum bridge.

The finite stopped terminal square estimates already imply the a.s. boundedness
of the stopped finite running absolute maxima.  This removes the former
manual `hBdd` side condition from the threshold convergence route.
-/
theorem durrett2019_theorem_4_5_2_stopped_runningAbsMax_ae_bddAbove_of_terminal_integral_sq_le
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    ...
    ∀ᵐ ω ∂P,
      BddAbove
        (Set.range fun m => durrett2019_runningAbsMax (stoppedProcess X N) m ω) := by
  have hStopped : Martingale (stoppedProcess X N) ℱ P :=
    durrett2019_theorem_4_2_9_martingale_stoppedProcess hX hN
  ...
  have hbound :
      ∀ n,
        eLpNorm (durrett2019_runningAbsMax (stoppedProcess X N) n)
          (2 : ℝ≥0∞) P ≤ R := by
    intro n
    exact
      durrett2019_theorem_4_5_1_runningAbsMax_eLpNorm_two_le_of_integral_sq_le
        (P := P) (ℱ := ℱ) (X := stoppedProcess X N) hStopped
        (hStopped_memLp_two n) (hStopped_sq_le n)
  exact
    durrett2019_runningAbsMax_ae_bddAbove_of_eLpNorm_bound
      (P := P) (ℱ := ℱ) (X := stoppedProcess X N) hStopped
      (p := (2 : ℝ≥0∞)) (C := R) hp_ne_zero hR_lt hbound
...
/--
Durrett 2019, Theorem 4.5.2 stopped maximal-estimate assembly with automatic
bounded-running-maximum side condition.

The stopped terminal square estimates both give the maximal `lintegral` bound
and discharge the a.s. boundedness needed to identify the canonical running
supremum.
-/
theorem durrett2019_theorem_4_5_2_stopped_exists_ae_tendsto_of_terminal_integral_sq_le_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X : ℕ -> Ω -> ℝ} {N : Ω -> ℕ∞} {C : ℝ}
...
/--
Durrett 2019, Theorem 4.5.2 threshold-stopped maximal-estimate assembly with
automatic bounded-running-maximum side condition.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_terminal_integral_sq_le_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a C : ℝ}
  ...
  exact
    durrett2019_theorem_4_5_2_stopped_exists_ae_tendsto_of_terminal_integral_sq_le_auto_bdd
      ...
      (durrett2019_theorem_4_5_2_firstPredictableAbove_isStoppingTime
        (ℱ := ℱ) (A := A) a hA_predictable)
      hX_memLp_two hC_nonneg hStopped_sq_le
...
/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from the
stopped increasing-process bound and stopped square identity, with automatic
bounded-running-maximum side condition.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_stopped_increasing_le_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
...
/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from the
initial increasing-process bound and stopped square identity, with automatic
bounded-running-maximum side condition.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_initial_le_and_square_identity_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
...
/--
Durrett 2019, Theorem 4.5.2 stopped square/increasing-process integral
identity from a predictable-part identification.

For a stopped martingale whose stopped square has predictable part identified
with a stopped increasing process `A`, the canonical square-process identity
from Theorem 4.5.1 gives the desired ordinary integral identity.
-/
theorem durrett2019_theorem_4_5_2_stopped_square_integral_eq_of_predictablePart_ae_eq
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {N : Ω -> ℕ∞}
    (hX : Martingale X ℱ P) (hN : IsStoppingTime ℱ N)
    (hN_zero_le : ∀ ω, (0 : ℕ∞) ≤ N ω)
    (hX_memLp_two : ∀ n, MemLp (X n) (2 : ℝ≥0∞) P)
    (hX0 : X 0 =ᵐ[P] 0)
    (hPredictablePart_eq_A :
      ∀ n,
        predictablePart (fun k ω => stoppedProcess X N k ω ^ 2) ℱ P n
          =ᵐ[P] stoppedProcess A N n) :
    ∀ n,
      (∫ ω, stoppedProcess X N n ω ^ 2 ∂P) =
        ∫ ω, stoppedProcess A N n ω ∂P := by
  have hStopped : Martingale (stoppedProcess X N) ℱ P :=
    durrett2019_theorem_4_2_9_martingale_stoppedProcess hX hN
  have hStopped_memLp_two :
      ∀ n, MemLp (stoppedProcess X N n) (2 : ℝ≥0∞) P := by
    intro n
    exact memLp_stoppedProcess (ι := ℕ) hN hX_memLp_two n
  have hStopped_zero : stoppedProcess X N 0 =ᵐ[P] 0 := by
    filter_upwards [hX0] with ω hX0ω
    have hstop : stoppedProcess X N 0 ω = X 0 ω :=
      stoppedProcess_eq_of_le (u := X) (τ := N) (i := 0) (ω := ω) (hN_zero_le ω)
    simpa [hstop] using hX0ω
  intro n
  calc
    (∫ ω, stoppedProcess X N n ω ^ 2 ∂P)
        = ∫ ω,
            predictablePart (fun k ω => stoppedProcess X N k ω ^ 2) ℱ P n ω ∂P :=
      durrett2019_theorem_4_5_1_square_integral_eq_predictablePart_square_of_initial_zero
        (P := P) (ℱ := ℱ) (X := stoppedProcess X N)
        hStopped hStopped_memLp_two hStopped_zero n
    _ = ∫ ω, stoppedProcess A N n ω ∂P :=
      integral_congr_ae (hPredictablePart_eq_A n)

/--
Durrett 2019, Theorem 4.5.2 threshold time starts after time `0`.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_zero_le
    {Ω : Type*} {A : ℕ -> Ω -> ℝ} {a : ℝ} :
    ∀ ω, (0 : ℕ∞) ≤
      durrett2019_theorem_4_5_2_firstPredictableAbove A a ω := by
  intro ω
...
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_square_integral_eq_of_predictablePart_ae_eq
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
  ...
  durrett2019_theorem_4_5_2_stopped_square_integral_eq_of_predictablePart_ae_eq
    ...
    (durrett2019_theorem_4_5_2_firstPredictableAbove_isStoppingTime
      (ℱ := ℱ) (A := A) a hA_predictable)
    (durrett2019_theorem_4_5_2_firstPredictableAbove_zero_le (A := A) (a := a))
    hX_memLp_two hX0 hPredictablePart_eq_A

/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from the
initial increasing-process bound and the stopped predictable-part identity.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_initial_le_and_predictablePart_identity
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
  ...
  durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_initial_le_and_square_identity
    (P := P) (ℱ := ℱ) (X := X) (A := A) (a := a)
    hX hA_predictable hX_memLp_two hA_int hA0_le
...
/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from the
initial increasing-process bound and stopped predictable-part identity, with
automatic bounded-running-maximum side condition.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_initial_le_and_predictablePart_identity_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
...
/--
Durrett 2019, Theorem 4.5.2 stopped predictable-part identification from a
Doob-decomposition certificate.

This is the uniqueness step: if `(X^N)^2 - A^N` is a martingale and `A^N` is
the predictable zero-start finite-variation part, then the canonical
predictable part of `(X^N)^2` is `A^N` at every fixed time.
-/
theorem durrett2019_theorem_4_5_2_stopped_predictablePart_eq_of_square_minus_increasing_martingale
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {N : Ω -> ℕ∞}
    ...
    (hSquareMinus :
      Martingale
        (fun n ω => stoppedProcess X N n ω ^ 2 - stoppedProcess A N n ω)
        ℱ P)
    (hA_predictable : IsStronglyPredictable ℱ (stoppedProcess A N))
    (hA0 : A 0 = 0)
    (hA_stopped_int : ∀ n, Integrable (stoppedProcess A N n) P) :
    ∀ n,
      predictablePart (fun k ω => stoppedProcess X N k ω ^ 2) ℱ P n
        =ᵐ[P] stoppedProcess A N n := by
  let Y : ℕ -> Ω -> ℝ := fun n ω => stoppedProcess X N n ω ^ 2
  let M : ℕ -> Ω -> ℝ := fun n ω => Y n ω - stoppedProcess A N n ω
  let B : ℕ -> Ω -> ℝ := stoppedProcess A N
  have hM : Martingale M ℱ P := by
    simpa [M, Y] using hSquareMinus
  have hB_predictable : IsStronglyPredictable ℱ B := by
    simpa [B] using hA_predictable
  have hB0 : B 0 = 0 := by
    ext ω
    have hstop : stoppedProcess A N 0 ω = A 0 ω :=
      stoppedProcess_eq_of_le (u := A) (τ := N) (i := 0) (ω := ω) (hN_zero_le ω)
    simp [B, hstop, congrFun hA0 ω]
  have hB_int : ∀ n, Integrable (B n) P := by
    intro n
    simpa [B] using hA_stopped_int n
  have hdecomp : M + B = Y := by
    ext n ω
    simp [M, B, Y]
  intro n
  have huniq :=
    durrett2019_theorem_4_3_2_doob_decomposition_canonical_unique
      (μ := P) (ℱ := ℱ) (X := Y) (M := M) (A := B)
      hM hB_predictable hB0 hB_int hdecomp n
  simpa [Y, B] using huniq.2

/--
Durrett 2019, Theorem 4.5.2 threshold predictable-part identification from a
stopped square-minus-increasing-process martingale certificate.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_predictablePart_eq_of_square_minus_increasing_martingale
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
    (hA_shift_predictable : StronglyAdapted ℱ (fun n ω => A (n + 1) ω))
    (hA_int : ∀ n, Integrable (A n) P)
    (hSquareMinus :
      Martingale
        (fun n ω =>
  ...
  have hN :
      IsStoppingTime ℱ (durrett2019_theorem_4_5_2_firstPredictableAbove A a) :=
    durrett2019_theorem_4_5_2_firstPredictableAbove_isStoppingTime
      (ℱ := ℱ) (A := A) a hA_shift_predictable
  have hA_stopped_int :
      ∀ n,
        Integrable
          (stoppedProcess A
            (durrett2019_theorem_4_5_2_firstPredictableAbove A a) n) P := by
    intro n
    exact integrable_stoppedProcess (ι := ℕ) hN hA_int n
  exact
...
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_square_minus_increasing_martingale
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
  ...
  durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_initial_le_and_predictablePart_identity
    (P := P) (ℱ := ℱ) (X := X) (A := A) (a := a)
    hX hA_shift_predictable hX_memLp_two hA_int hX0 hA0_le
    (durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_predictablePart_eq_of_square_minus_increasing_martingale
      (P := P) (ℱ := ℱ) (X := X) (A := A) (a := a)
...
/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from a
stopped Doob-decomposition certificate, with automatic bounded-running-maximum
side condition.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_square_minus_increasing_martingale_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
...
certificate from the source square-minus-increasing martingale.

Stopping commutes with the pointwise expression `X_n^2 - A_n`, so optional
stopping gives the stopped certificate consumed by the Doob-decomposition
bridge.
-/
theorem durrett2019_theorem_4_5_2_stopped_square_minus_increasing_martingale_of_source
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {N : Ω -> ℕ∞}
    ...
    Martingale
      ...
      ℱ P := by
  have hStopped :
      Martingale (stoppedProcess (fun n ω => X n ω ^ 2 - A n ω) N) ℱ P :=
    durrett2019_theorem_4_2_9_martingale_stoppedProcess hSquareMinus hN
  have hEq :
      stoppedProcess (fun n ω => X n ω ^ 2 - A n ω) N =
        fun n ω => stoppedProcess X N n ω ^ 2 - stoppedProcess A N n ω := by
    ext n ω
    simp [stoppedProcess]
  simpa [hEq] using hStopped

/--
Durrett 2019, Theorem 4.5.2 threshold stopped square-minus-increasing
martingale certificate from the source square-minus-increasing martingale.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_square_minus_increasing_martingale_of_source
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
...
/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from the
source square-minus-increasing martingale plus stopped predictability of the
increasing process.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_source_square_minus_martingale
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
  ...
  durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_square_minus_increasing_martingale
    ...
    hStoppedA_predictable hBdd

/--
Durrett 2019, Theorem 4.5.2 threshold stopped-convergence bridge from the
source square-minus-increasing martingale plus stopped predictability of the
increasing process, with automatic bounded-running-maximum side condition.
-/
theorem durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_source_square_minus_martingale_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {a : ℝ}
...
If `A_{n+1}` is almost surely bounded by a finite terminal variable `A∞`, then
almost every path belongs to one of the countably many threshold events
`∀ n, A_{n+1} <= k^2`.
-/
theorem durrett2019_theorem_4_5_2_threshold_cover_of_ae_le_terminal
    {Ω : Type*} [MeasurableSpace Ω] {P : Measure Ω}
    {A : ℕ -> Ω -> ℝ} {Ainf : Ω -> ℝ}
    (hA_le_Ainf : ∀ᵐ ω ∂P, ∀ n, A (n + 1) ω ≤ Ainf ω) :
    ∀ᵐ ω ∂P, ω ∈ (Set.univ : Set Ω) ->
      ∃ k : ℕ, ω ∈ {ω : Ω | ∀ n, A (n + 1) ω ≤ (k : ℝ) ^ 2} := by
  filter_upwards [hA_le_Ainf] with ω hle _hmem
...
This is the countable-threshold assembly step: each stopped threshold process
converges, each threshold event survives its stopping time, and the finite
terminal bound covers almost every path by such a threshold event.
-/
theorem durrett2019_theorem_4_5_2_exists_ae_tendsto_of_source_square_minus_martingale_cover
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {Ainf : Ω -> ℝ}
  ...
  have hcover :
      ∀ᵐ ω ∂P, ω ∈ (Set.univ : Set Ω) -> ∃ k, ω ∈ E k := by
    simpa [E] using
      durrett2019_theorem_4_5_2_threshold_cover_of_ae_le_terminal
        (P := P) (A := A) (Ainf := Ainf) hA_le_Ainf
  have hStopped :
      ∀ k, ∀ᵐ ω ∂P, ∃ z : ℝ,
    ...
    simpa [N] using
      durrett2019_theorem_4_5_2_firstPredictableAbove_stopped_exists_ae_tendsto_of_source_square_minus_martingale
        ...
        hX hA_shift_predictable hSquareMinus hX_memLp_two hA_int hX0 hA0 hA0_le
        (hStoppedA_predictable (k : ℝ)) (hBdd (k : ℝ))
  ...
  have hresult :=
    durrett2019_theorem_4_5_2_exists_ae_tendsto_of_stopped_event_cover
      (P := P) (X := X) (N := N) (FiniteVar := (Set.univ : Set Ω)) (E := E)
      hcover hStopped hSurvive
  filter_upwards [hresult] with ω hω
  exact hω trivial

/--
Durrett 2019, Theorem 4.5.2 event-cover convergence from the source
square-minus-increasing martingale certificates, with automatic
bounded-running-maximum side conditions for all thresholds.
-/
theorem durrett2019_theorem_4_5_2_exists_ae_tendsto_of_source_square_minus_martingale_cover_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {Ainf : Ω -> ℝ}
...
`A_n -> A∞`; this wrapper converts that into the threshold-cover domination
used by the countable assembly theorem.
-/
theorem durrett2019_theorem_4_5_2_exists_ae_tendsto_of_source_square_minus_martingale_monotone_terminal
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {Ainf : Ω -> ℝ}
            ...
            (Set.range fun m =>
              durrett2019_runningAbsMax
                (stoppedProcess X
                  (durrett2019_theorem_4_5_2_firstPredictableAbove A a)) m ω)) :
    ∀ᵐ ω ∂P, ∃ z : ℝ, Tendsto (fun n => X n ω) atTop (𝓝 z) := by
  have hA_le_all : ∀ n, A n ≤ᵐ[P] Ainf :=
    durrett2019_ae_le_of_ae_monotone_tendsto_atTop
      (P := P) (A := A) (Ainf := Ainf) hA_mono hA_tendsto
  have hA_le_Ainf : ∀ᵐ ω ∂P, ∀ n, A (n + 1) ω ≤ Ainf ω :=
    ae_all_iff.2 fun n => hA_le_all (n + 1)
  exact
    durrett2019_theorem_4_5_2_exists_ae_tendsto_of_source_square_minus_martingale_cover
      (P := P) (ℱ := ℱ) (X := X) (A := A) (Ainf := Ainf)
      hX hA_shift_predictable hSquareMinus hX_memLp_two hA_int hX0 hA0
      hA_le_Ainf hStoppedA_predictable hBdd

/--
Durrett 2019, Theorem 4.5.2 monotone terminal source wrapper, with automatic
bounded-running-maximum side conditions for all threshold-stopped processes.
-/
theorem durrett2019_theorem_4_5_2_exists_ae_tendsto_of_source_square_minus_martingale_monotone_terminal_auto_bdd
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {X A : ℕ -> Ω -> ℝ} {Ainf : Ω -> ℝ}
...
/--
Durrett 2019, Theorem 4.5.3 `L^2` support: multiplying an `L^p` function by
an a.e. strongly measurable real factor bounded by one in absolute value
preserves `L^p` membership.
-/
theorem durrett2019_memLp_mul_of_abs_le_one
    {Ω : Type*} [MeasurableSpace Ω] {P : Measure Ω} {p : ℝ≥0∞}
    {U Y : Ω -> ℝ}
    (hY : MemLp Y p P)
    (hU_sm : AEStronglyMeasurable U P)
    (hU_abs_le : ∀ᵐ ω ∂P, |U ω| ≤ 1) :
    MemLp (fun ω => U ω * Y ω) p P := by
  refine hY.of_le_mul (c := (1 : ℝ)) ?hsm ?hbound
...
is square-integrable as soon as the original martingale process is
square-integrable and `1 <= f(A_n)`.
-/
theorem durrett2019_theorem_4_5_3_reciprocal_comp_transform_memLp_two_of_process_memLp
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} {ℱ : Filtration ℕ mΩ}
    {A X : ℕ -> Ω -> ℝ} {f : ℝ -> ℝ}
    (hA_predictable : IsStronglyPredictable ℱ A)
  ...
  refine
    memLp_finsetSum' (μ := P) (p := (2 : ℝ≥0∞)) (s := Finset.range n)
      (f := fun k : ℕ =>
        fun ω : Ω => (f (A (k + 1) ω))⁻¹ * (X (k + 1) ω - X k ω))
      ?_
  intro k _hk
  have hdiff : MemLp (fun ω => X (k + 1) ω - X k ω) (2 : ℝ≥0∞) P :=
    (hX_memLp_two (k + 1)).sub (hX_memLp_two k)
  let Hk : Ω -> ℝ := fun ω => (f (A (k + 1) ω))⁻¹
  have hH_sm : AEStronglyMeasurable Hk P := by
    ...
    simpa [Hk, one_div] using hrec.aestronglyMeasurable
  have hH_abs_le : ∀ᵐ ω ∂P, |Hk ω| ≤ 1 := by
    refine ae_of_all P fun ω => ?_
    have hnonneg : 0 ≤ Hk ω :=
      inv_nonneg.mpr (zero_le_one.trans (hf_one_le (k + 1) ω))
    have hle : Hk ω ≤ 1 :=
      inv_le_one_of_one_le₀ (hf_one_le (k + 1) ω)
    simpa [Hk, abs_of_nonneg hnonneg] using hle
  exact
...
/--
Durrett 2019, Theorem 4.5.3 increment-square integrability support.
-/
theorem durrett2019_theorem_4_5_3_increment_sq_integrable_of_process_memLp
    {Ω : Type*} [MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P]
    {X : ℕ -> Ω -> ℝ}
    (hX_memLp_two : ∀ k, MemLp (X k) (2 : ℝ≥0∞) P) :
    ∀ k : ℕ, Integrable (fun ω => (X (k + 1) ω - X k ω) ^ 2) P := by
  intro k
  exact
    durrett2019_integrable_sq_of_memLp_two
      (P := P) ((hX_memLp_two (k + 1)).sub (hX_memLp_two k))

/--
Durrett 2019, Theorem 4.5.3 scaled-square integrability support for the
reciprocal transform `H_n = f(A_n)^{-1}`.
-/
theorem durrett2019_theorem_4_5_3_reciprocal_comp_scaled_sq_integrable_of_process_memLp
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] {ℱ : Filtration ℕ mΩ}
    {A X : ℕ -> Ω -> ℝ} {f : ℝ -> ℝ}
    (hA_predictable : IsStronglyPredictable ℱ A)
...
/--
Durrett 2019, Theorem 4.5.3 finite random clock-integrability support.

For a continuous reciprocal-square clock integrand, the random clock integral
is measurable because it is the difference of two continuous primitives
evaluated at predictable clock times.  The global tail-integral bound then
gives a uniform bound, hence integrability on a finite measure space.
-/
theorem durrett2019_theorem_4_5_3_finite_clock_integral_integrable_of_tail_integral_bound
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P]
    {ℱ : Filtration ℕ mΩ} {f : ℝ -> ℝ} {A : ℕ -> Ω -> ℝ} {C : ℝ}
    (hA_predictable : IsStronglyPredictable ℱ A)
    ...
    (htail_bound :
      ∫ t in Set.Ici (0 : ℝ), (f t)⁻¹ ^ 2 ∂volume ≤ C) :
    ∀ N : ℕ, Integrable (fun ω => ∫ t in A 0 ω..A N ω, (f t)⁻¹ ^ 2) P := by
  intro N
  let g : ℝ -> ℝ := fun t => (f t)⁻¹ ^ 2
  let F : ℝ -> ℝ := fun x => ∫ t in (0 : ℝ)..x, g t
  have hg_cont : Continuous g := by
    simpa [g] using hfinvSq_cont
  have hF_cont : Continuous F := by
    have huncurry :
        Continuous (Function.uncurry (fun _x : ℝ => fun t : ℝ => g t)) := by
      simpa [Function.uncurry] using hg_cont.comp continuous_snd
    simpa [F] using
      (intervalIntegral.continuous_parametric_intervalIntegral_of_continuous
        (μ := volume) (a₀ := (0 : ℝ))
        (f := fun _x : ℝ => fun t : ℝ => g t)
        huncurry (s := fun x : ℝ => x) continuous_id)
  have hA0N : ∀ ω : Ω, A 0 ω ≤ A N ω := by
    intro ω
    induction N with
    | zero =>
    ...
    | succ N ih =>
        exact ih.trans (hA_mono_step N ω)
  have hA0_sm_sub : StronglyMeasurable[ℱ 0] (A 0) :=
    hA_predictable.stronglyAdapted 0
  have hAN_sm_sub : StronglyMeasurable[ℱ N] (A N) :=
    hA_predictable.stronglyAdapted N
  have hA0_sm : StronglyMeasurable (A 0) :=
    hA0_sm_sub.mono (ℱ.le 0)
  have hAN_sm : StronglyMeasurable (A N) :=
    hAN_sm_sub.mono (ℱ.le N)
  have hbase_sm :
      AEStronglyMeasurable (fun ω : Ω => F (A N ω) - F (A 0 ω)) P :=
    ((hF_cont.comp_stronglyMeasurable hAN_sm).sub
      (hF_cont.comp_stronglyMeasurable hA0_sm)).aestronglyMeasurable
  have hclock_eq :
      (fun ω : Ω => F (A N ω) - F (A 0 ω)) =ᵐ[P]
        fun ω => ∫ t in A 0 ω..A N ω, (f t)⁻¹ ^ 2 := by
    refine ae_of_all P fun ω => ?_
    have h0N : IntervalIntegrable g volume (0 : ℝ) (A N ω) :=
      hg_cont.intervalIntegrable _ _
    have h00 : IntervalIntegrable g volume (0 : ℝ) (A 0 ω) :=
      hg_cont.intervalIntegrable _ _
    simpa [F, g] using
      (intervalIntegral.integral_interval_sub_left
        (μ := volume) (f := g) (a := (0 : ℝ)) (b := A N ω) (c := A 0 ω)
        h0N h00)
  have hclock_sm :
      AEStronglyMeasurable
        (fun ω : Ω => ∫ t in A 0 ω..A N ω, (f t)⁻¹ ^ 2) P :=
    (aestronglyMeasurable_congr hclock_eq).mp hbase_sm
  have hbounded :
      ∀ᵐ ω ∂P, ‖∫ t in A 0 ω..A N ω, (f t)⁻¹ ^ 2‖ ≤ C := by
    refine ae_of_all P fun ω => ?_
    have hclock_nonneg :
        0 ≤ ∫ t in A 0 ω..A N ω, (f t)⁻¹ ^ 2 := by
      exact intervalIntegral.integral_nonneg (hA0N ω) fun t _ht => sq_nonneg ((f t)⁻¹)
    have hclock_le :
        ∫ t in A 0 ω..A N ω, (f t)⁻¹ ^ 2 ≤ C :=
      durrett2019_theorem_4_5_3_clock_interval_le_tail_integral_bound
        (f := f) (a := A 0 ω) (b := A N ω) (C := C)
        (hA0_nonneg ω) (hA0N ω) htail_int htail_bound
    exact (Real.norm_of_nonneg hclock_nonneg).trans_le hclock_le
...
conditional-variance endpoint.  The remaining assumptions are the genuine
source side conditions: clock monotonicity/interval estimates, integrability,
and the reciprocal-transform `MemLp` obligations.
-/
theorem durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_integral_clock_bound
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {A X : ℕ -> Ω -> ℝ} {f : ℝ -> ℝ} {C : ℝ}
...
automatic `L^2` side conditions.

Compared with
`durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_integral_clock_bound`,
this wrapper replaces the transform `MemLp`, scaled-square integrability, and
increment-square integrability hypotheses by the source assumption that the
martingale process is square-integrable at every time.
-/
theorem durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_integral_clock_bound_of_process_memLp
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {A X : ℕ -> Ω -> ℝ} {f : ℝ -> ℝ} {C : ℝ}
  ...
  exact
    durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_integral_clock_bound
      ...
      (durrett2019_theorem_4_5_3_reciprocal_comp_transform_memLp_two_of_process_memLp
        (P := P) (ℱ := ℱ) (A := A) (X := X) (f := f)
        hA_predictable hf_cont hf_one_le hX_memLp_two)
...
This is currently the most source-shaped compiled endpoint for Theorem 4.5.3:
the transform `MemLp`, scaled-square integrability, increment-square
integrability, and uniform integrated clock-bound assumptions are all derived
from reusable hypotheses.  The remaining explicit side conditions are the
variance-ratio integrability and finite-clock-integrability assumptions.
-/
theorem durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_tail_integral_bound_of_process_memLp
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {A X : ℕ -> Ω -> ℝ} {f : ℝ -> ℝ} {C : ℝ}
  ...
  exact
    durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_integral_clock_bound_of_process_memLp
      (P := P) (ℱ := ℱ) (A := A) (X := X) (f := f) (C := C)
      hX hX0 hX_memLp_two hA_predictable hf_cont hA_mono_step hf_mono
      hf_interval_one_le hf_int hb_increment_nonneg hb_atTop hratio_int
      hcond_le hclock_int
      (durrett2019_theorem_4_5_3_integrated_clock_bound_of_tail_integral_bound
        (P := P) (f := f) (A := A) (C := C)
        hA0_nonneg hA_mono_step htail_int hclock_int htail_bound)
...
/--
Durrett 2019, Theorem 4.5.3 source-facing tail-integral route with automatic
`L^2`, variance-ratio integrability, and finite random clock-integrability
side conditions.

This wrapper removes the final explicit integrability side condition from the
V216/V217/V218 source route.  The remaining assumptions are theorem-facing
clock monotonicity/range conditions, interval estimates for `f`, clock-process
integrability, the conditional square estimate, and the global tail integral
bound.
-/
theorem durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_tail_integral_bound_of_process_memLp_clock_integrable_auto_clock
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] [IsProbabilityMeasure P]
    {ℱ : Filtration ℕ mΩ} [SigmaFiniteFiltration P ℱ]
    {A X : ℕ -> Ω -> ℝ} {f : ℝ -> ℝ} {C : ℝ}
  ...
  exact
    durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_of_reciprocal_comp_condExp_tail_integral_bound_of_process_memLp_clock_integrable
      (P := P) (ℱ := ℱ) (A := A) (X := X) (f := f) (C := C)
      hX hX0 hX_memLp_two hA_predictable hA_int hf_cont hA0_nonneg
      hA_mono_step hf_mono hf_interval_one_le hf_int hb_increment_nonneg
      hb_atTop hcond_le
...
/--
Durrett 2019, Theorem 4.5.5 canonical nonnegative version of the cumulative
conditional probability clock.

Each conditional-probability increment is clipped to its positive part.  This
process is pointwise monotone and agrees a.e. at every finite time with
`durrett2019_theorem_4_5_5_conditionalProbabilitySum`.
-/
noncomputable def durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    (P : Measure Ω) (ℱ : Filtration ℕ mΩ) (B : ℕ -> Set Ω) (n : ℕ) : Ω -> ℝ :=
  ∑ k ∈ Finset.range n,
    fun ω : Ω => max (P[(B (k + 1)).indicator (1 : Ω -> ℝ) | ℱ k] ω) 0
...
/--
Durrett 2019, Theorem 4.5.5 canonical denominator-increment display.

The clipped cumulative conditional-probability denominator has one-step
increment `max (E(1_{B_{k+1}} | F_k)) 0`.
-/
theorem durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_increment_eq
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} {ℱ : Filtration ℕ mΩ} (B : ℕ -> Set Ω) (k : ℕ) :
    (fun ω : Ω =>
      durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B (k + 1) ω -
        durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B k ω) =
      fun ω : Ω =>
        max (P[(B (k + 1)).indicator (1 : Ω -> ℝ) | ℱ k] ω) 0 := by
  ext ω
  simp [durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum,
    Finset.sum_range_succ]
...
/--
Durrett 2019, Theorem 4.5.5 automatic conditional-variance domination for the
canonical clipped conditional-probability clock.

This is the same Bernoulli conditional-variance estimate as the raw clock, with
the final comparison passed through `p <= max p 0`.
-/
theorem
    ...
    ∀ k : ℕ,
      P[(fun ω : Ω =>
        ...
        ≤ᵐ[P]
          fun ω : Ω =>
            durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B (k + 1) ω -
              durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B k ω := by
  intro k
  have hraw :=
    durrett2019_theorem_4_5_5_martingalePart_condExp_square_le_conditionalProbabilitySum_increment_auto
      (P := P) (ℱ := ℱ) (B := B) hB_meas k
  have hraw_increment :=
    durrett2019_theorem_4_5_5_conditionalProbabilitySum_increment_eq
      (P := P) (ℱ := ℱ) B k
  have hclipped_increment :=
    durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_increment_eq
      (P := P) (ℱ := ℱ) B k
  filter_upwards [hraw] with ω hrawω
  rw [congrFun hclipped_increment ω]
  rw [congrFun hraw_increment ω] at hrawω
  exact hrawω.trans (le_max_left _ _)
...
/--
Durrett 2019, Theorem 4.5.5 source support: the clipped cumulative
conditional probability clock is predictable.
-/
theorem durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_predictable
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} {ℱ : Filtration ℕ mΩ} (B : ℕ -> Set Ω) :
    IsStronglyPredictable ℱ
      (durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B) := by
  refine IsStronglyPredictable.of_measurable_add_one ?h0 ?hsucc
  · simpa [durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum] using
      (stronglyMeasurable_zero : StronglyMeasurable[ℱ 0] (0 : Ω -> ℝ))
  · intro n
    refine Finset.stronglyMeasurable_sum _ fun k hk => ?_
    have hkn : k ≤ n := Finset.mem_range_succ_iff.mp hk
    have hp :
        StronglyMeasurable[ℱ n]
          (P[(B (k + 1)).indicator (1 : Ω -> ℝ) | ℱ k]) :=
      stronglyMeasurable_condExp.mono (ℱ.mono hkn)
    exact
      (continuous_id.max continuous_const :
        Continuous fun x : ℝ => max x (0 : ℝ)).comp_stronglyMeasurable hp

/--
Durrett 2019, Theorem 4.5.5 source support: finite-time integrability of the
clipped cumulative conditional probability clock.
-/
theorem durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_integrable
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} [IsFiniteMeasure P] {ℱ : Filtration ℕ mΩ}
    {B : ℕ -> Set Ω}
    (hB_meas : ∀ n, MeasurableSet (B n)) :
    ∀ n,
      Integrable
        (durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B n) P := by
  intro n
  dsimp [durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum]
  exact
    integrable_finsetSum' (μ := P) (Finset.range n)
      (f := fun k : ℕ =>
        fun ω : Ω => max (P[(B (k + 1)).indicator (1 : Ω -> ℝ) | ℱ k] ω) 0)
      fun k _hk => by
        ...
        simpa using
          (integrable_condExp
            (μ := P) (m := ℱ k)
            (f := (B (k + 1)).indicator (1 : Ω -> ℝ))).pos_part

/--
Durrett 2019, Theorem 4.5.5 source support: the clipped cumulative conditional
probability clock starts nonnegative.
-/
theorem durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_zero_nonneg
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} {ℱ : Filtration ℕ mΩ} (B : ℕ -> Set Ω) :
    ∀ ω : Ω,
      0 ≤ durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B 0 ω := by
  intro ω
  simp [durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum]

/--
Durrett 2019, Theorem 4.5.5 source support: the clipped cumulative conditional
probability clock is pointwise monotone.
-/
theorem durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_mono_step
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} {ℱ : Filtration ℕ mΩ} (B : ℕ -> Set Ω) :
    ∀ k : ℕ, ∀ ω : Ω,
      durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B k ω ≤
        durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B (k + 1) ω := by
  intro k ω
  have hinc :=
    congrFun
      (durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_increment_eq
...
/--
Durrett 2019, Theorem 4.5.5 source support: at each finite time, the clipped
clock agrees almost everywhere with the raw conditional-probability clock.
-/
theorem durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_ae_eq_conditionalProbabilitySum
    {Ω : Type*} [mΩ : MeasurableSpace Ω]
    {P : Measure Ω} {ℱ : Filtration ℕ mΩ} (B : ℕ -> Set Ω) :
    ∀ n : ℕ,
      durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B n
        =ᵐ[P]
      durrett2019_theorem_4_5_5_conditionalProbabilitySum P ℱ B n := by
  intro n
  induction n with
  | zero =>
      exact ae_of_all P fun ω => by
        simp [durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum,
          durrett2019_theorem_4_5_5_conditionalProbabilitySum]
  | succ n ih =>
      let I : Ω -> ℝ := (B (n + 1)).indicator (1 : Ω -> ℝ)
      have hI_nonneg : 0 ≤ᵐ[P] I :=
        ae_of_all P fun ω => by
          by_cases hω : ω ∈ B (n + 1) <;> simp [I, hω]
      have hp_nonneg : 0 ≤ᵐ[P] P[I | ℱ n] :=
        condExp_nonneg (μ := P) (m := ℱ n) hI_nonneg
      have hstep :
          (fun ω : Ω => max (P[I | ℱ n] ω) 0) =ᵐ[P] P[I | ℱ n] := by
        filter_upwards [hp_nonneg] with ω hpω
        exact max_eq_left hpω
      filter_upwards [ih, hstep] with ω hihω hstepω
      calc
        durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B (n + 1) ω
            =
          durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B n ω +
            max (P[I | ℱ n] ω) 0 := by
              simp [durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum,
                Finset.sum_range_succ, I]
        _ =
          durrett2019_theorem_4_5_5_conditionalProbabilitySum P ℱ B n ω +
            P[I | ℱ n] ω := by
              rw [hihω, hstepω]
        _ =
          durrett2019_theorem_4_5_5_conditionalProbabilitySum P ℱ B (n + 1) ω := by
              simp [durrett2019_theorem_4_5_5_conditionalProbabilitySum,
                Finset.sum_range_succ, I]

/--
Durrett 2019, Theorem 4.5.5 source support: the clipped and raw clocks agree
almost everywhere for every finite time simultaneously.
-/
theorem
...
/--
Durrett 2019, Theorem 4.5.5 source support: infinite-clock divergence for the
raw clock transfers to the clipped pointwise-monotone representative.
-/
theorem
  ...
  filter_upwards
    [hA_atTop_on,
      durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_ae_eq_conditionalProbabilitySum_all
        (P := P) (ℱ := ℱ) B] with ω hrawω heqω hD
  exact Tendsto.congr' (Eventually.of_forall fun n => (heqω n).symm) (hrawω hD)
...
/--
Durrett 2019, Theorem 4.5.5 infinite-clock source wrapper from adapted events
using the canonical pointwise-monotone conditional-probability clock.

The theorem is stated with the textbook raw denominator.  Internally it applies
Theorem 4.5.3 to the clipped representative, then transfers the conclusion
back to the raw clock by finite-time a.e. equality.
-/
theorem
  ...
  have hB_meas : ∀ n, MeasurableSet (B n) :=
    durrett2019_theorem_4_5_5_measurableSet_of_adapted (ℱ := ℱ) hB
  let A : ℕ -> Ω -> ℝ :=
    durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum P ℱ B
  let X : ℕ -> Ω -> ℝ :=
    martingalePart (MeasureTheory.BorelCantelli.process B) ℱ P
  let f : ℝ -> ℝ := fun t : ℝ => max t (1 : ℝ)
  have hf_cont : Continuous f := by
  ...
  have hf_one_le : ∀ t : ℝ, 1 ≤ f t := by
    intro t
    exact le_max_right t (1 : ℝ)
  have hA_atTop_clipped :
      ∀ᵐ ω ∂P, ω ∈ InfiniteVar -> Tendsto (fun n : ℕ => A n ω) atTop atTop := by
    simpa [A] using
      durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_atTop_on_of_conditionalProbabilitySum
        (P := P) (ℱ := ℱ) (B := B) (D := InfiniteVar) hA_atTop_on
  have hcond_le :
      ∀ k : ℕ,
        P[(fun ω => (X (k + 1) ω - X k ω) ^ 2) | ℱ k] ≤ᵐ[P]
          fun ω => A (k + 1) ω - A k ω := by
    simpa [A, X] using
      durrett2019_theorem_4_5_5_martingalePart_condExp_square_le_nonnegativeConditionalProbabilitySum_increment_auto
        (P := P) (ℱ := ℱ) (B := B) hB_meas
  have hnormalized_clipped :
      ∀ᵐ ω ∂P, ω ∈ InfiniteVar ->
        Tendsto (fun n : ℕ => X n ω / f (A n ω)) atTop (𝓝 0) :=
    durrett2019_theorem_4_5_3_normalized_process_ae_tendsto_zero_on_of_reciprocal_comp_condExp_tail_integral_bound_of_process_memLp_clock_integrable_auto_clock_global_mono
      (P := P) (ℱ := ℱ) (A := A) (X := X) (f := f) (C := (2 : ℝ))
      (InfiniteVar := InfiniteVar)
      (by
        simpa [X] using
          durrett2019_example_4_3_3_borel_cantelli_process_martingale
      ...
      hA_atTop_clipped
      hcond_le
      (by simpa [f] using durrett2019_theorem_4_5_5_max_one_inv_sq_integrableOn_Ici)
      (by simpa [f] using durrett2019_theorem_4_5_5_integral_max_one_inv_sq_Ici_le_two)
  filter_upwards
    [hnormalized_clipped,
      durrett2019_theorem_4_5_5_nonnegativeConditionalProbabilitySum_ae_eq_conditionalProbabilitySum_all
        (P := P) (ℱ := ℱ) B] with ω hnormalizedω heqω hInfinite
  refine Tendsto.congr' ?_ (hnormalizedω hInfinite)
  exact Eventually.of_forall fun n => by
    simp [A, X, f, heqω n]
...
/--
Durrett 2019, Theorem 4.5.5 event-cover ratio assembly from adapted events
using the canonical pointwise-monotone infinite-clock representative.

Compared with
`durrett2019_theorem_4_5_5_ratio_tendsto_one_on_of_finite_or_adapted_conditionalProbabilitySum_clock_auto_tail`,
this wrapper removes the raw pointwise monotonicity assumption.  The conclusion
is still stated with the textbook raw conditional-probability denominator.
-/
theorem
  ...
  have hinfinite_atTop :
    ...
    filter_upwards [hdenom_atTop_on] with ω hdenomω hω
    exact hdenomω hω.1
  have hinfinite_normalized_inter :
      ∀ᵐ ω ∂P, ω ∈ D ∩ InfiniteVar ->
        Tendsto
          (fun n : ℕ =>
            martingalePart (MeasureTheory.BorelCantelli.process B) ℱ P n ω /
          ...
          atTop (𝓝 0) :=
    durrett2019_theorem_4_5_5_martingalePart_max_one_normalized_on_of_adapted_conditionalProbabilitySum_clock_canonical_auto_tail
      (P := P) (ℱ := ℱ) (B := B) (InfiniteVar := D ∩ InfiniteVar)
      hB hinfinite_atTop
  have hinfinite_normalized :
      ∀ᵐ ω ∂P, ω ∈ D -> ω ∈ InfiniteVar ->
        Tendsto
          (fun n : ℕ =>
            martingalePart (MeasureTheory.BorelCantelli.process B) ℱ P n ω /
              max
                ...
                (1 : ℝ))
          atTop (𝓝 0) := by
    filter_upwards [hinfinite_normalized_inter] with ω hω hD hInfinite
    exact hω ⟨hD, hInfinite⟩
  exact
    durrett2019_theorem_4_5_5_ratio_tendsto_one_on_of_finite_or_max_one_normalized
      (P := P) (ℱ := ℱ) (B := B) (D := D)
...
/--
Durrett 2019, Theorem 4.5.5 source support: on the limsup event, the raw
conditional-probability clock diverges.

This is the exact use of the conditional Borel-Cantelli theorem needed to turn
the theorem-facing ratio wrapper into a statement on `limsup B atTop`.
-/
theorem
  ...
  filter_upwards
    [durrett2019_theorem_4_3_4_conditional_borel_cantelli
      (μ := P) (ℱ := ℱ) (B := B) hB] with ω hBC hlimsup
  simpa [durrett2019_theorem_4_5_5_conditionalProbabilitySum] using
    hBC.mp hlimsup

/--
Durrett 2019, Theorem 4.5.5 infinite-clock ratio wrapper from adapted events.

On any event where the raw cumulative conditional-probability clock diverges,
the Borel-Cantelli event count divided by that clock tends to one.
-/
theorem
  ...
  have hmartingale_max_ratio_on :
      ∀ᵐ ω ∂P, ω ∈ D ->
        Tendsto
          ...
          atTop (𝓝 0) :=
    durrett2019_theorem_4_5_5_martingalePart_max_one_normalized_on_of_adapted_conditionalProbabilitySum_clock_canonical_auto_tail
      (P := P) (ℱ := ℱ) (B := B) (InfiniteVar := D) hB hdenom_atTop_on
  exact
    durrett2019_theorem_4_5_5_ratio_tendsto_one_on_of_martingalePart_max_one_normalized
      (P := P) (ℱ := ℱ) (B := B) (D := D)
      hdenom_atTop_on hmartingale_max_ratio_on

/--
Durrett 2019, Theorem 4.5.5 final infinite-clock ratio wrapper on the limsup
event.

For adapted events, on the event that the events occur infinitely often, the
event count divided by the cumulative conditional probabilities tends to one.
The denominator divergence is supplied by conditional Borel-Cantelli, and the
martingale estimate is supplied by the V233 canonical square-clock route.
-/
theorem
