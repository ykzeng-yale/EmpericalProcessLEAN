...
2. construct a `SuppliedRealMiddleCDFPartitionChain` for every bounded
   interval and positive radius, then feed the compiled cutpoint-chain consumer
   into the middle-partition-to-GC handoff;
...
- advance Durrett Theorem 2.4.9 by constructing
  `SuppliedRealMiddleCDFPartitionChain` objects for arbitrary bounded intervals
  and positive radii, beyond the compiled one-cell/two-cell/right-append
  consumers; or
