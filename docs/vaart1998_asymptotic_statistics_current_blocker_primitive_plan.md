...
1. Move one layer closer to a model-facing finite-dimensional statement:
   connect the random observation-sequence endpoint to a concrete
...
(`Add Vaart random observation-sequence source endpoint`).

The latest theorem-sized packet removes the direct indexed observation
measurability and indexed-family sequence-law fields by deriving them from a
single measurable random observation sequence and its infinite-product law.
...
random observation-sequence endpoint to a concrete model-specialized Theorem
