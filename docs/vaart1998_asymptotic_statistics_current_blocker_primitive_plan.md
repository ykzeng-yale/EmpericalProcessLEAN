...
measurability for the Gaussian limit representative `Z`; and only the textbook
...
of `Z` via `measurable_pi_lambda`, and rebuilds vector-valued
derivative-transform integrability via `Integrable.of_eval`. It rebuilds the
...
or full `Z` measurability, unless a source theorem specifically demands those
displays.

Immediate predecessor endpoint:
`vaart1998_theorem_5_41_zEstimator_scaledEstimator_handoff_of_empiricalAverage_scoreTransformCoordinateMomentSource_scoreLawSource_derivativeAtTheta0TransformSource_zCoordinateMeasurableSource_vDerivativeTransformMeanSource_derivativeTransformCoordinateIntegrableSource_derivativeTransformCoordinateMeasurableSource_scoreTransformCoordinateMeasurableSource_zSampleObservationScoreCenteredProductCovarianceSource_zSampleMeanSource_observationSequenceEvalSource_jointMeasurableSource_nonnegativeEnvelopeSource_unscaledScoreSource_fixedTheta0Source_coordinateSumRootSource_univSmoothnessSource_observationEnvelopeMeanSource_observationTransformDisplaySource_observationRandomSequenceTransformMomentSource_observationSamplePathSource_observationEnvelopeAverageSource_observationScoreCovarianceSource_zSampleCoordinateMeanSource_derivativeBasisMatrixActionSource_zSampleMeanSource_scoreVectorMeanSource_scoreLawMeanSource_zGaussianMemLpSource_zLawCovarianceBilinSource_zLawMeanSource_derivativeLawVectorIntegrableSource_scoreLawVectorMomentSource_coordinateProjectionSource_derivativeTableVectorScoreDirectSource_scoreLawCovarianceMomentSource_scoreVectorDisplaySource_estimatingMapContDiffTaylorSource_pointwiseSmoothnessSource_populationBasisMatrixActionSource_pointwiseDerivativeMatrixActionSource_measurableSource_rawRootSource_estimatorDefinitionSource_vectorScoreCommonLawScoreCLT_absorbingSource_envelope`.

This endpoint uses the same coordinate score-transform moment and
centered-product covariance route, but keeps the vector-valued observation
derivative-transform mean identity as an assumption. The current endpoint
rebuilds that field from coordinatewise observation basis-action mean
identities.
