...
The terminal-sample lower-growth adapters
`vdVWSelectedTruncatedPositiveRadiusEmpiricalL1CoveringNumberCard_eventually_ge_of_eventually_terminal_empiricalL1Index_packingNumber` and
...
plus the final terminal-sample consumer
...
hypotheses.
The infinite-packing source surface is also compiled:
...
by choosing the deterministic lower bound `n` from eventual terminal
`Metric.packingNumber = ⊤`.
The nonterminal companion
...
chooses the same lower bound from eventual `samplePath`-level
`Metric.packingNumber = ⊤`.
...
The matching nonterminal final consumer
`VdVWTheorem243TextbookAlignedConclusion.of_productPairChebyshev_countable_finiteCenter_failure_tails_halfScale_selected_truncated_quarterRadius_firstLevel_of_eventually_singleSample_ae_empiricalL1Index_coveringNumber_of_set_countable`
uses the already compiled `samplePath`-level covering-number adapter, so
callers can state the lower bound before terminal-sample transport.
Direct `Metric.coveringNumber = ⊤` adapters are now available as well, with
the nonterminal final consumer
...
feeding lower bound `n` from eventual `samplePath`-level infinite covering.
...
The projective-limit FDD law-extensionality layer now also works across
different source probability spaces:
`processLaw_eq_iff_forall_finiteDimensional_eq_twoMeasure`,
`identDistrib_iff_forall_finiteDimensional_identDistrib_twoMeasure`,
...
This removes the same-domain restriction from the law-uniqueness component of
VdV&W 1.4.8, but it is still not the arbitrary-index weak-convergence
converse.
...
`WeakConvergence.lean` now defines `VdVWProbabilityLaw` as the
`ProbabilityMeasure.map` law of an a.e.-measurable map and exposes
`VdVWRandomElementTight`, `VdVWRandomElementSeparable`,
`VdVWRandomElementPreTight`, and `VdVWRandomElementPolish` as law-backed
random-element predicates.  The compiled law-facing wrappers
`vdVW132_randomElement_preTight_iff_separable`,
...
`vdVW132_polishSpace_randomElement_tight` transfer the measure-level Lemma
1.3.2 package to a.e.-measurable random elements.
...
| 1.3.2 | Lemma | `..._1-100.md:582` | local-layer/mathlib-foundation: measure-level separability, pre-tightness, and Polishness predicates compiled; pre-tight/separable equivalence, tight-to-pre-tight/separable, complete-space tight/pre-tight/separable equivalences, and Polish-measure tightness now compiled through `vdVW132_complete_probabilityMeasure_tight_preTight_separable_equiv` and `vdVW132_probabilityMeasure_tight_of_polish`; law-facing a.e.-measurable random-element wrappers now compiled through `VdVWProbabilityLaw`, `vdVW132_complete_randomElement_tight_preTight_separable_equiv`, and `vdVW132_randomElement_tight_of_polish`; exact nonmeasurable/arbitrary-map variants still pending |
...
| 1.3.12 | Lemma | `..._1-100.md:768` | local-layer/mathlib-foundation: finite-measure bounded-continuous uniqueness, separating star-subalgebra uniqueness, and tight separating star-subalgebra weak-convergence criterion compiled; exact arbitrary-map/nonmeasurable variant pending |
...
| 1.10.2 | Lemma | `..._1-100.md:1409` | local-layer/mathlib-foundation: measurable common-domain part (ii) now named as `vdVW1102_ii_tendstoInDistribution_of_vdVWConvergesInOuterProbability`, plus signed bounded-continuous arbitrary-map consequence `vdVW1102_ii_to_signedBoundedContinuousArbitraryMap_aemeasurable`; full arbitrary-map/measurable-cover version remains pending |
...
| 1.12.2 | Theorem | `..._1-100.md:1718` | local-layer/mathlib-foundation: measure-level bounded-Lipschitz determining direction and testing consequence compiled as `vdVW1122_weakConvergenceProbabilityMeasures_of_forall_bounded_lipschitz_integral_tendsto` and `vdVW1122_forall_bounded_lipschitz_integral_tendsto_of_weakConvergenceProbabilityMeasures`; exact arbitrary-map/nonmeasurable version pending |
| 1.12.4 | Theorem | `..._1-100.md:1751` | local-layer/mathlib-foundation: measure-level Levy-Prokhorov distance criterion compiled as `vdVW1124_weakConvergenceProbabilityMeasures_iff_levyProkhorovDist_tendsto_zero`, plus both directions; exact arbitrary-map/nonmeasurable bounded-Lipschitz metric theorem pending |
...
3. Promote Chapter 2 primitive infrastructure: covering/packing semimetric
   ...
   The inverse-square source route is now compiled via
   ...
   They now also package directly into
   `VdVWTheorem243TextbookAlignedConclusion` through
   `VdVWTheorem243TextbookAlignedConclusion.of_productPairChebyshev_countable_finiteCenter_failure_tails_halfScale_selected_truncated_quarterRadius_firstLevel_of_eventually_ae_cardinality_ge`
   and
   `VdVWTheorem243TextbookAlignedConclusion.of_productPairChebyshev_countable_finiteCenter_failure_tails_halfScale_selected_truncated_quarterRadius_firstLevel_of_eventually_singleSample_ae_cardinality_ge`.
