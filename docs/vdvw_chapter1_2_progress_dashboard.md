...
2026-05-03 update: the random empirical-cover witness is now consumed by
`ae_prod_vdVWWeightedClassSupremum_le_finiteNetHoeffdingUpper_add_of_randomEmpiricalCovers_rademacherSigns`.
The remaining blocker is the product-a.e. pointwise finite-center Hoeffding
predicate, or an expectation-level replacement compatible with the existing
expected-maximal API, followed by entropy-to-convergence.
...
NEXT       Product-a.e. pointwise Hoeffding predicate for selected random covers, or expectation-level replacement, then entropy-to-convergence.
...
Latest pushed proof-layer verification is for `ea4833e`; dirty-tree
verification for the random empirical-cover handoff is recorded in the current
automation run.
