...
Route from `Live In-Thread Goal Prompt V139` in
...
the square martingale, deterministic variance clock, stopped-value
integrability, and `var(S_n)` endpoint feeding the compiled bounded-increment
small-ball wrapper.
...
bounded-increment overshoot/source wrapper now also compile.  The next target
is the remaining exact Exercise 4.4.6 source instantiation.
...
first-exit/small-ball assembly, and the bounded-increment overshoot/source
wrapper now compile.  Aim at one forward textbook packet: instantiate the square
martingale, deterministic variance clock, stopped-value integrability, and
`var(S_n)` endpoint for the exact Exercise 4.4.6 source wrapper, or move to a
nearby Chapter 4.4 theorem-sized packet if that route is blocked.
