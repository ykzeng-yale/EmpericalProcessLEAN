...
Route from `Live In-Thread Goal Prompt V208` in
...
Mathlib-shaped conditional-variance pull-out core.  The next target is to
instantiate this lemma with `H = (f(A_{k+1}))^{-1}` and then prove the
deterministic integral comparison for `b_n = f(A_n)`, using V204-V208 as
closed support.
