...
Route from `Live In-Thread Goal Prompt V222` in
...
The next target is either an event-local `{A∞ = ∞}` conclusion from an a.e.
clock limit, or the next martingale theorem if the global clock-divergence
endpoint is enough for the current route.
