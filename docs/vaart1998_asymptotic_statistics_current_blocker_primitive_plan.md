...
`‖observationSecondDerivative x‖ <= |envelope x|`. It also removes the
separate `secondDerivative` kernel from the endpoint surface by instantiating
it as `fun _ _ x => observationSecondDerivative x`; the second derivative
Taylor identity is now stated directly against `observationSecondDerivative`.
...
joint measurability route, and observation-level second-derivative
measurability/envelope route, but still keeps a separate `secondDerivative`
kernel and a pointwise identification with `observationSecondDerivative`. The
current endpoint removes that kernel by definitional instantiation.
