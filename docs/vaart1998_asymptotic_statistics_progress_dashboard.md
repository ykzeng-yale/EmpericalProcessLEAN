...
Move next to the raw-root wrapper and remove its `hScaledEstimator` hypothesis
by calling the absorbing raw-score-CLT endpoint, then continue upward through
estimator-substitution wrappers only while each packet remains theorem-sized.
Otherwise prove the next
