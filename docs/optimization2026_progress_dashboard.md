...
- Current manual frontier after the Chapter 12 finite sampled rate packet,
  ...
  final smooth/non-smooth weighted stochastic averaged-iterate wrappers:
...
conclude directly at the source-shaped averaged iterate.
...
averaged-iterate/Jensen bridge and final source-shaped wrappers:
...
wrappers now apply that bridge to the main sampled endpoints.  Next prove any
exact source conditional-expectation/process packaging that is still needed,
then move to ASGD CLT material once SMPGD source packaging is complete enough.
