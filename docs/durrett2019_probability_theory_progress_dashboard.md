...
Route from `Live In-Thread Goal Prompt V100` in
...
trimmed dominating measure.  The next target is denominator-limit nonzeroness
and the remaining nonzero-lower-integral/product inputs.
...
and trimmed-prefix RN-ratio support.  Aim at one actual source-production
obligation: discharge the denominator-limit nonzero input, the canonical ratio
nonzero-lower-integral input, or the final product/nonzero hypotheses consumed
by the compiled branch assemblers.  Search `Measure.trim`, `rnDeriv_ne_top`,
`rnDeriv_pos`, `lintegral_ne_zero`, `withDensity`, and the existing canonical
limit-density endpoint.  Do not re-prove the compiled Chapter 4.3.5 or 4.3.8
assembly layers.
