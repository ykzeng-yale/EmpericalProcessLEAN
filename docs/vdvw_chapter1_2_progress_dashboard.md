...
`0 < epsilon <= μ.real univ`, all-positive-radius `N_[] < ∞` handoff, the
...
| Example 2.4.2 | Real half-line indicator bracket membership, endpoint integrability, `L1(P)` width identity, extended-real endpoint indicators/brackets for `-∞`/`∞`, extended-open-cell endpoint/width identities, adjacent-endpoint grid handoff, supplied finite-grid bridges, one-cell base grid and one-cell adjacent-endpoint base grid for radii above total mass, radius-monotonicity helpers for supplied real/extended/adjacent-endpoint grids, reduction of full endpoint-grid existence to `0 < epsilon <= μ.real univ`, all-positive-radius `N_[] < ∞` handoff, conditional half-line GC corollary from supplied grids, and conditional half-line GC corollary from adjacent endpoint grids | Distribution-dependent finite grid in the nontrivial radius range and exact empirical-CDF example report. |
...
ONGOING    Example 2.4.2: distribution-dependent grid in the nontrivial radius range after conditional EReal-grid GC handoff.
