...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after the 2026-05-05 Theorem 5.10 setup packet:
  local `main` is past `88178a2` and
  `StatInference/Optimization/Theorem510.lean` now compiles and is imported by
  `StatInference.lean`.  Chapter 5 CG is past the basic Krylov/termination setup,
  ...
  stationarity, and `p 0 = 0`.  The active target remains Theorem 5.10
  discrete AGD, but the next blocker is now the weighted two-point
  Lyapunov/telescoping assembly, not the lambda/theta setup.
...
line 1219.  `StatInference/Optimization/Theorem510.lean` now contains the
source `(3.3)` specialization from the existing Chapter 3 theorem plus the AGD
lambda recurrence/positivity/growth and telescope-alignment facts.  The next
packet should assemble the source weighted two-point inequality, finite
telescoping sum, and final `O(1 / N^2)` rate.  The CG modules
