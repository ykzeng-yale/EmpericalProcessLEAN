...
Route from `Live In-Thread Goal Prompt V108` in
...
wrapper no longer asks for pointwise coordinate finiteness.  The next target is
only source-shaped wrappers that remove the remaining ambient dichotomy when a
...
the closed no-top `tprod` textbook branch wrapper.  Aim at one actual
source-production obligation: remove only the remaining ambient dichotomy when
a direct mathlib or local route is visible.  Search
`withDensity`, `MutuallySingular`, `AbsolutelyContinuous`,
`LebesgueDecomposition`, top-set identities, tail zero-one support, and existing
product-measure dichotomy endpoints.
