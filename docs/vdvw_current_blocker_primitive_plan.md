...
- the practical route for Theorem 2.4.3 remains the theorem-local
  ...
  supply measurable cardinality and deterministic boundedness or a genuine
  variable-domain uniform-integrability/dominated-convergence replacement, then
  choose a shrinking deterministic empirical-cover radius from the all-radius
  entropy hypothesis, for example `coverRadius n = ((n + 1 : ℕ) : ℝ)⁻¹`, and
  feed its convergence to zero into the fixed-`M` consumer.
