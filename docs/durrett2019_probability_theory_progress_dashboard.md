...
Route from `Live In-Thread Goal Prompt V93` in
...
coordinate side-condition bridge also now compiles.  The next target is
supplying the source convergence hypotheses for the full-prefix and tail-block
likelihood limits needed for the remaining Kakutani criterion assembly.
