...
| `VdVWMeasurableCover.eventIndicatorOfToMeasurable` / `VdVWOuterExpectation_eq_lintegral_eventIndicatorCover` | definition/theorem | Lemma 1.2.3 measurable-cover direction for an event indicator in the probability-space setting | proved finite-measure event-indicator cover using mathlib `Measure.measure_toMeasurable_inter`; full arbitrary-map measurable-cover existence remains pending |
...
| `tendstoInMeasure_of_vdVWConvergesInOuterProbability` / `vdVWConvergesInOuterProbability_iff_tendstoInMeasure` | theorems | common-domain VdV&W outer-probability convergence is equivalent to mathlib convergence in measure | proved strict-to-nonstrict event bridge using an `epsilon / 2` argument |
| `tendstoInDistribution_of_vdVWConvergesInOuterProbability` | theorem | Lemma 1.10.2(ii), measurable common-domain version: outer-probability convergence implies weak convergence | proved via mathlib `TendstoInMeasure.tendstoInDistribution` |
...
| `empiricalAverage_samplePath_aemeasurable_of_coordinate` / `empiricalAverage_samplePath_aemeasurable_of_sample_aemeasurable` / `empiricalAverage_samplePath_aemeasurable_of_hasLaw` | theorems | empirical averages over finite sample paths are measurable finite sums | proved using mathlib `Finset.aemeasurable_fun_sum`, `AEMeasurable.div_const`, `Measurable.comp_aemeasurable`, and `HasLaw.aemeasurable` |
| `vdVW_theorem_2_4_1_outerProbabilityGlivenkoCantelli_of_countable_of_sample_aemeasurable_of_classFun_measurable` / `vdVW_theorem_2_4_1_outerProbabilityGlivenkoCantelli_of_countable_of_classFun_measurable` | theorems | direct outer-probability GC for countable classes with concrete sample/function measurability, and the stronger `HasLaw` plus measurable-class-function form | proved concrete measurability corollaries of Theorem 2.4.1 |
