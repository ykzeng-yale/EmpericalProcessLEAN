...
- Manual goal policy: the app-level `/goal` objective text cannot be edited
  ...
  work.
- Current manual frontier after focused Lean verification of the local Chapter
  8 APGD/FISTA packet in `StatInference/Optimization/Proximal.lean`, rebased
  over current `origin/main`: Chapter 8 now
  has focused-verified supplied-interface Theorem 8.6 declarations
  `IsChewi86APGDTrajectory`, its accessors/telescope alignment,
  `proximalGradient_gap_le_inner_form`,
  `chewi86_weighted_two_point_bound_telescope`,
  `chewi86_weighted_sum_bound`, and
  `chewi86_gap_le_two_beta_dist_sq_over_nat_sq`, reusing PGD `(8.1)` plus
  Chapter 5.10 lambda/energy telescope algebra.  Search-first result remains:
  no pinned mathlib direct proximal/Moreau/proximal-gradient theorem was found;
  useful reuse is local first-order/smooth model APIs, Chapter 3 recurrence,
  and Chapter 5.10 acceleration algebra.  Latest pushed Optimization frontier
  is `5479054` (`Add Chewi theorem 8.6 APGD layer`), now followed on
  `origin/main` by other agents' empirical-process commits.  The local Chapter
  9/10 substrate now focused-checks in `Fenchel.lean` and `Bregman.lean`:
  finite-valued Fenchel conjugate certificates, Fenchel-Young/subgradient
  equivalences, weak double conjugation, Lemma 9.12's Lipschitz core,
  Bregman divergence, relative lower/upper models, and relative growth.
  Immediate gate: promoted build/scans/docs/commit/push.  Next aggressive
  theorem lane after promotion: `MirrorDescent.lean` for Theorem 10.9
  MPGD/OMD telescoping, not more Chapter 8 setup.
...
| Chapters 9-11 deterministic algorithms | local-layer-started | `StatInference/Optimization/Fenchel.lean`, `StatInference/Optimization/Bregman.lean` | The first finite-valued Chapter 9/10 substrate focused-checks. `Fenchel.lean` introduces `fenchelConjugateTerm`, `fenchelConjugate`, `IsFenchelUpperBound`, `IsFenchelConjugateValue`, `fenchelConjugate_isFenchelConjugateValue`, Fenchel-Young, equality iff primal subgradient, equality iff dual subgradient under supplied biconjugate equality, `fenchelDoubleConjugate_le`, `StrongSubgradientMonotoneOn`, and `fenchelGradient_lipschitz_of_strongSubgradientMonotone` for Lemma 9.12's core estimate. `Bregman.lean` introduces `bregmanDivergence`, `RelativelyStrongConvexOn`, `RelativelySmoothOn`, relative lower/upper model wrappers for Propositions 10.5/10.6, and `RelativelyStrongConvexOn.growth_of_stationary` for Lemma 10.7. Next lane is promoted build plus `MirrorDescent.lean` for MPGD/OMD telescoping. |
