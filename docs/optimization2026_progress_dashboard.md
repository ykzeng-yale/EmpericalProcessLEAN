...
- Current manual frontier: Chapter 5 CG is past the basic Krylov/termination
  ...
  `Theorem59.lean` now starts Theorem 5.9 with the strong-convex AGF friction,
  Lyapunov vector, Lyapunov energy, generic AGF gap derivative, energy-vector
  derivative, raw Lyapunov derivative, and zero-momentum vector identity.  The
  active target is simplifying the raw derivative to `L'_t <= -sqrt(alpha) L_t`
  and applying the existing scalar exponential-decay lemma.
...
Latest proof target: Chapter 5 acceleration.  The active AGF modules are
`StatInference/Optimization/Theorem58.lean` and
`StatInference/Optimization/Theorem59.lean`, anchored at Theorem 5.8 around
markdown line 1138 and Theorem 5.9 around line 1163.  The CG modules
