...
| Chapter 4 martingales | active/source-wrapper | `StatInference/ProbabilityTheory/Basic.lean`; `StatInference/ProbabilityTheory/ConditionalExpectation.lean`; `StatInference/ProbabilityTheory/Martingale.lean`; mathlib `Probability/ConditionalExpectation.lean` and `Probability/Martingale/*` | Chapter 4.1 conditional expectation is compiled through Theorem 4.1.15. Chapter 4.2 and Chapter 4.3 are reusable support. Chapter 4.4 now includes the maximal-inequality route through Theorem 4.4.6, Theorem 4.4.7 orthogonality, Theorem 4.4.8 conditional variance, Example 4.4.9 nonzero-limit support, Exercise 4.4.10 `L^2` and almost-sure convergence from square-summable increments, and Exercise 4.4.11 predictable-transform, deterministic Abel/Kronecker, Toeplitz constant/centered, centered-remainder, nonnegative-increment weighted-average/Kronecker, normalized increment-sum a.e., zero-initial/shifted normalized-process, bounded-variance corollary, and deterministic reciprocal-normalizer wrappers. Section 4.5.1 now has the finite Doob `L^2` `eLpNorm` bridge, automatic finite-max `L^2` membership, ordinary finite second-moment display, monotone iSup `lintegral` handoff, `runningAbsSup` square handoff, supplied `A∞` source bridge, canonical `E X_n^2 = E A_n` predictable-part identity, terminal monotone-limit handoff, and conditional-variance finite-sum display. V201 adds the Theorem 4.5.2 source square-minus stopped certificate, finite-terminal threshold cover, countable event-cover assembly, and monotone-terminal source wrapper. V202 derives the stopped running-maximum boundedness side condition from stopped terminal square estimates and adds no-`hBdd` source wrappers. V203 derives stopped predictability of `A^N` from `IsStronglyPredictable ℱ A` and adds no-manual-predictability source wrappers through the monotone-terminal auto endpoint. V204 adds event-local Theorem 4.5.2 source packaging on `FiniteVar`, matching the finite-variance event. V205 adds the Theorem 4.5.3 random-normalizer Kronecker bridge from transform convergence to `X_n / b_n -> 0`, plus reciprocal-transform predictability and bounds for `H_n = f(A_n)^{-1}`. V206 adds scaled-summability and reciprocal scaled-summability endpoints feeding that bridge directly. V207 adds the variance-ratio bridge and reciprocal variance-ratio endpoint. V208 adds the conditional-variance pull-out core. V209 instantiates that core with the reciprocal normalizer and exposes the source-facing conditional-variance endpoint. V210 adds the scalar deterministic interval comparison. V211 adds the finite pathwise clock-sum lift. V212 adds deterministic variance-ratio summability and total-bound wrappers from a uniform finite-clock-integral bound. V213 adds a.e. random pathwise summability and total-bound wrappers from a.e. random clock certificates. V214 adds the finite integrated/Fubini variance-ratio summability and total-bound package for `b_n = f(A_n)`. V215 feeds V214 into the V209 source endpoint. V216 removes the manual uniform integrated clock-bound input using the global tail-integral bound over `[0, ∞)`. V217 replaces transform `MemLp`, scaled-square integrability, and increment-square integrability side conditions by `∀ n, X_n ∈ L^2`. V218 replaces variance-ratio integrability by clock-process integrability `∀ n, Integrable (A n) P`. V219 removes finite random clock-integrability using the continuous primitive and tail bound. V220 removes interval-integrability, normalizer-increment, and reciprocal-square-continuity side conditions through source wrappers. V221 replaces per-interval lower-bound/no-zero/monotonicity and shifted random normalizer divergence assumptions by global `f` lower-bound/monotonicity/divergence plus a.s. clock divergence. V222 derives deterministic `f -> ∞` from `f >= 1`, monotonicity, and finite `∫_0^∞ f(t)^{-2} dt`. V223 adds the event-local infinite-clock endpoint for `∀ᵐ ω, ω ∈ InfiniteVar -> X_n / f(A_n) -> 0` from event-local clock divergence. V224 starts Theorem 4.5.5 with the conditional-probability denominator, martingale-part/count display, ratio endpoint, and finite-limit denominator bridge. V225 identifies the martingale increment, denominator increment, and source bridge from Bernoulli conditional variance to the dominated variance clock. V226 proves the Bernoulli conditional variance estimate and automatic dominated variance-clock endpoint. V227 adds the max-normalizer handoff and finite/infinite ratio assembly. V228 specializes the infinite-clock side to `f(t)=max t 1` and feeds V226 clock domination, leaving clock regularity and tail-integral source obligations explicit. Next target: discharge those explicit source side conditions, with special care around pointwise versus a.e. monotonicity of conditional expectations. |
