...
Route from `Live In-Thread Goal Prompt V154` in
...
Kronecker ratio certificates, plus the Toeplitz constant/centered split.  The
next target is proving the centered Toeplitz remainder from convergence of the
scaled partial sums, nonnegative normalizer increments, and divergence of the
normalizer, then feeding the weighted-average and Kronecker consumers before
the bounded-variance corollary.
The V143-V154 packets added `durrett2019_exercise_4_4_6_varianceClock_succ`,
...
`durrett2019_exercise_4_4_11_weighted_average_eq_constant_add_centered`, and
...
wrapper plus exact-denominator wrapper now also compile.  The next target is
the remaining exact Exercise 4.4.6 source instantiation.
...
| Chapter 4 martingales | active/source-wrapper | `StatInference/ProbabilityTheory/Basic.lean`; `StatInference/ProbabilityTheory/ConditionalExpectation.lean`; `StatInference/ProbabilityTheory/Martingale.lean`; mathlib `Probability/ConditionalExpectation.lean` and `Probability/Martingale/*` | Chapter 4.1 conditional expectation is compiled through Theorem 4.1.15. Chapter 4.2 now has martingale/submartingale/supermartingale adaptedness, integrability, conditional expectation identity/inequality, one-step, real constructor wrappers, Example 4.2.1 linear random-walk martingale/supermartingale/submartingale plus centered-display wrappers, Example 4.2.2 quadratic source and natural-random-walk wrappers, Example 4.2.3 product and normalized exponential martingale wrappers, Theorems 4.2.4/4.2.5 all-times and strict-index wrappers, Theorem 4.2.6 generic convex-image plus `|X_n|^p` wrappers, Theorem 4.2.7 increasing-convex plus positive-part/minimum consequences, Theorem 4.2.8 predictable-transform wrappers, Theorem 4.2.9 stopped-process wrappers, Theorem 4.2.10 upcrossing inequality wrappers, Theorem 4.2.11 direct L1/eLpNorm-bounded plus positive-part source convergence wrappers, Theorem 4.2.12 nonnegative-supermartingale convergence, integrable-limit, and Fatou expectation wrappers, Theorem 4.3.1 stopped-shifted, first-below, bounded-below, bounded-above, one-sided-bounded, range-form dichotomy, threshold-form oscillation, exact `EReal` liminf/limsup wrappers, Theorem 4.3.2 Doob-decomposition existence/formula and uniqueness wrappers, Example 4.3.3 plus Theorem 4.3.4 conditional Borel-Cantelli wrappers, the first Theorem 4.3.5 / Lemma 4.3.6 RN-derivative likelihood-ratio martingale and convergence wrappers, the Theorem 4.3.5 regular/singular decomposition identities, the density-ratio/top-set source assembly, the integral-representation to RN-derivative bridge, the generator-extension bridge, the bounded-convergence generator-production bridge, the trimmed-RN eventual restricted-density bridge, the `mu + nu` boundedness bridge, the real-to-`ENNReal` convergence-transfer endpoint, the bounded real-martingale limitProcess convergence bridge for the two natural `mu + nu` trimmed RN `toReal` sequences, the canonical finite nonnegative limit-density endpoint, the canonical-ratio endpoint, the denominator-side canonical-ratio top-null endpoint, the singular-support top-set endpoint, the canonical `toReal = dmu/dnu` endpoint, canonical-ratio real integrability, the quotient-limit convergence bridge, the full canonical-ratio real identity, the Example 4.3.7 finite-partition likelihood, finite-union, and generator-facing endpoint, the Theorem 4.3.8 finite-product likelihood measurability, rectangle set-integral, and finite-product `withDensity` endpoint, the infinite-product cylinder-likelihood measurability, finite-coordinate restriction `withDensity`, cylinder set-integral endpoint, finite/cylinder Hellinger factorization endpoints, zero-product Fatou/cylinder Hellinger-product endpoints, positive/zero branch assemblers, Hellinger-tail/Cauchy positive-product consumers, tail zero-set and lower-integral positive-branch consumers, finite-prefix/tail-block zero-set handoffs, pointwise finite/nonzero coordinate side-condition handoff, range-limit tail handoff, range-limit positive-branch consumers from full-prefix convergence, canonical-ratio range-limit positive consumers, the real-valued canonical-ratio Hellinger/product consumer, canonical prefix-filtration measurability/inclusion support, the canonical trimmed-prefix RN-ratio identity, the denominator-limit nonzero bridge, canonical prefix convergence from the trimmed-prefix ratio, the positive Hellinger-product wrapper with that convergence supplied, the positive-product finite-limit side-condition bridge, canonical product-tail/`tprod` positive-product wrappers with strict-positive product variants, the first canonical zero/positive product criterion wrapper, canonical `mu + nu` ratio measurability, the ENNReal full-prefix convergence upgrade, the closed zero/positive branch wrapper, the textbook `tprod` zero/positive branch wrapper, the no-coordinate-finiteness/no-ambient-dichotomy `tprod` branch wrapper, Lemma 4.3.9's normalized conditional-mean martingale bridge, Theorem 4.4.2 nonnegative/positive-part/total-positive-part Doob maximal inequality wrappers, the Example 4.4.3 squared-threshold/division/absolute-maximum variance-bound Kolmogorov wrappers, the full Theorem 4.4.4 positive-part and martingale maximal inequality route through final `p/(p-1)` `eLpNorm` wrappers, the full Theorem 4.4.6 route through a.s.-convergence, limit-process `MemLp`, supplied-domination, finite running maxima, canonical running supremum, a.s. boundedness from finite-maximal `eLpNorm` bounds, and final `L^p` convergence, plus Theorem 4.4.1 optional-stopping wrappers, Theorem 4.4.7 orthogonality and increment-increment wrappers, Theorem 4.4.8 conditional variance formula, Example 4.4.9 through the nonzero-limit endpoint, Exercise 4.4.5 conditional-variance variant, and Exercise 4.4.6 stopped-variance small-ball handoff, the finite first-exit/small-ball assembly, the bounded-increment overshoot/source wrapper, the square-martingale wrapper with automatic stopped integrability, the deterministic variance-clock wrapper, and the exact-denominator wrapper. Next target: instantiate the square-martingale source from increments for the exact Exercise 4.4.6 source wrapper, or move to a nearby Chapter 4.4 theorem-sized packet if blocked. |
...
now closed support; the active frontier is Chapter 4.4 martingale maximal
inequalities.

Next proof packet: move past Durrett Theorem 4.3.8 Kakutani after the compiled
canonical-ratio, quotient-limit, Hellinger, tail-zero-set, prefix-filtration,
trimmed-prefix RN-ratio, denominator-limit nonzero, canonical prefix convergence,
positive-product finite-limit support, canonical product-tail/`tprod`
positive-product support, the first canonical zero/positive product criterion,
canonical ratio measurability, the ENNReal full-prefix convergence upgrade, and
the closed no-top/no-ambient-dichotomy `tprod` textbook branch wrapper, Lemma
4.3.9's normalized conditional-mean martingale bridge, and Theorem 4.4.2's
nonnegative/positive-part/total-positive-part Doob maximal inequality wrappers.
Example 4.4.3's squared-threshold Kolmogorov maximal wrapper also now compiles.
The division and absolute-maximum variance-bound displays now also compile.
Theorem 4.4.4's martingale absolute-maximum consequence bridge now compiles
from a supplied positive-part Lp maximal bound, and the p-th-power `lintegral`
source bridge now turns the textbook integration/Hölder estimate into the same
endpoint.  The layer-cake equality, pointwise Doob integrand bound, and Hölder
integral bound also compile.  The set-integral conversion and integrated Doob
threshold bound now compile as well.  The weighted/Fubini identification,
coefficient extraction, assembled Doob/Fubini/Hölder endpoint, scalar
cancellation lemma, finite `lintegral` estimate, and finite `eLpNorm` wrapper
now also compile.  The bounded-truncation layer through the per-cutoff
truncated `lintegral` estimate, monotone-convergence/iSup handoff, final
positive-part `eLpNorm` wrapper, and final martingale absolute-maximum
`eLpNorm` wrapper also compile.  The Theorem 4.4.6 bridge from a uniform
...
bound, `eLpNorm 2` handoff, `L^2` convergence endpoint, expectation handoff,
`E X = 1`, and nonzero-limit endpoint now compile.  Exercise 4.4.5's
conditional-variance variant now compiles.  Theorem 4.4.1 optional-stopping
wrappers, Exercise 4.4.6's stopped-variance small-ball handoff, the finite
first-exit/small-ball assembly, the bounded-increment overshoot/source wrapper,
and the square-martingale wrapper with automatic stopped integrability now
compile, along with the deterministic variance-clock and exact-denominator
wrappers.  Aim at one
forward textbook packet: instantiate the square-martingale source from
increments for the exact Exercise 4.4.6 source wrapper, or move to a nearby
Chapter 4.4 theorem-sized packet if that route is blocked.
