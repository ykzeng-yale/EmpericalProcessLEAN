...
Route from `Live In-Thread Goal Prompt V82` in
...
handoff also now compile.  The next target is the finite tail-product lower
bound from positive prefix/tail monotonicity and tail-event zero-one support
for Kakutani's dichotomy.
