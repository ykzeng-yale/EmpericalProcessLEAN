...
handling the terminal boundary/finite-vs-infinite issue for
`strongLowerBoundGeometricCandidate`, or by switching to a true `ℓ²` model
where the geometric vector is the exact zero-gradient minimizer.  Then prove
the geometric tail lower bound and convert that obstruction into the
logarithmic iteration-count statement.  In parallel,
