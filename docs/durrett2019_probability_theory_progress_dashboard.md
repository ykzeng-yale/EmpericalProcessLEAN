...
Route from `Live In-Thread Goal Prompt V204` in
...
event `FiniteVar`, matching the textbook's `{A∞ < ∞}` side.  The next target
is Durrett Theorem 4.5.3, using V204 as closed support.
